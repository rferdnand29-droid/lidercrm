...
(function(){
   ...
   *
   * CORREÇÃO 2026-08-07 (erros no console: "Presence iniciado" repetido
   * várias vezes, "Multiple GoTrueClient instances detected", blob: ...
   * ERR_FILE_NOT_FOUND repetido): existem DOIS sistemas de "app voltou a
   * ficar visível" totalmente independentes — este (v39,
   * setupAppResumeHandler) e outro em js/app.js (_lfSoftResumeSync),
   * cada um registrando seu PRÓPRIO listener de visibilitychange sem
   * saber da existência do outro. Num único evento real de "voltei pro
   * app", os dois podiam disparar reconexão, cada chamada de
   * initDB() criando um cliente Supabase novo (GoTrueClient) e
   * reiniciando presence/branding — dobrando (ou mais) o trabalho e
   * causando corrida na URL blob: do manifest (criada e revogada de
   * novo antes do navegador terminar de processar a anterior).
   * Em vez de remover um dos dois sistemas (cada um resolve um problema
   * diferente, mexer len risco reintroduzir o bug original de cada um),
   * um cooldown aqui — no PONTO ONDE OS DOIS CONVERGEM — garante que
   * reconexões muito próximas no tempo (qualquer combinação dos dois
   * disparando perto um do outro) resultem em só UMA chamada real a
   * initDB(), não importa quantos gatilhos dispararam. */
  var _lfSafeInitDBLastRun = 0;
  var LF_SAFE_INITDB_COOLDOWN_MS = 10000; // 10s — suficiente pra não duplicar, curto o bastante pra não atrasar reconexão de verdade
  window._lfSafeInitDB = function(){
    try{
      var now = Date.now();
      if(now - _lfSafeInitDBLastRun < LF_SAFE_INITDB_COOLDOWN_MS){
        log('_lfSafeInitDB ignorado — chamada recente demais (cooldown ' + LF_SAFE_INITDB_COOLDOWN_MS + 'ms), provável disparo duplicado de outro handler de resume');
        return;
      }
      _lfSafeInitDBLastRun = now;
