...
(function () {
  ...
  function _installPromiseAwarePollWrap() {
    ...
    window._chatPollNewMsgs = function () {
      ...
      try {
        ...
        return _wrappedPoll.apply(this, arguments);
      } catch (e) {
  ...
  function _watchdogUnstuck() {
    try {
      if (!window.LF_CHAT_CTX_SOUND_FIX) return;
      var fn = window.LF_CHAT_CTX_SOUND_FIX.isSuppressed;
      ...
      if (!_lastSuppressedAt) {
        _lastSuppressedAt = Date.now();
        return;
      }
      if (Date.now() - _lastSuppressedAt > SAFETY_MS) {
        // Está travado há mais de SAFETY_MS. Força um tick de poll,
        // que agenda o setTimeout(30ms) e libera.
        _lastSuppressedAt = 0;
        console.warn(TAG, 'supressão travada por >' + SAFETY_MS + 'ms — destravando');
        try { window._chatPollNewMsgs && window._chatPollNewMsgs(); } catch (_e) {}
      }
    } catch (_e) {}
  }
  setInterval(_watchdogUnstuck, 2000);
