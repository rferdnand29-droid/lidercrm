...
(function () {
  ...
  function _upsertOverdueNotif(a) {
    ...
    /* [FIX 20260818, reforçado 20260820] Guarda de card órfão: se a atividade aponta pra um lead/negócio
       que não existe mais no quadro — OU que já está em etapa terminal (descartado/fechado/convertido) —
       NÃO recria a notificação no sino. Descartar um negócio NÃO remove o card do array (só marca
       discarded:true e col:'noshow'), então uma checagem de "existe?" sozinha não bastava: o card
       "existia" mesmo já descartado, e a notificação continuava voltando. Mesmo critério usado em
       lf-fix-tab-dot-negocios-ownership-v1-20260820.js para a bolinha da aba. */
    if (a.clientId && a.board && typeof window.getKBFor === 'function') {
      try {
        var _TERMINAL = ['desc','noshow','conv','desist','fechado'];
        var _arr = window.getKBFor(a.board, window.S.userId) || [];
        var _card = null;
        for (var _i = 0; _i < _arr.length; _i++) {
          if (_arr[_i] && _arr[_i].id === a.clientId) { _card = _arr[_i]; break; }
        }
        if (!_card) return false;
        if (_card.discarded === true) return false;
        if (_TERMINAL.indexOf(String(_card.col || '')) >= 0) return false;
      } catch (_e) { /* em caso de erro, segue o fluxo original */ }
    }

    /* [FIX 20260820-2] Nunca ressuscita/reabre notificação de atividade que
       NÃO é mais minha ou cujo dono mudou. O sintoma reportado: o sino fica
       piscando com uma atividade "Atrasada" de um lead que já foi transferido
       pra outro consultor — a atividade continua no cache local de atividades
       do usuário atual (não foi limpa na transferência), então este patch a
       reencontrava a cada tick e recriava a notificação, reabrindo como não
       lida mesmo depois de "Marcar todas como lidas".
       Guarda: se a atividade tem um responsável registrado (userId/_ownerId/
       by) que NÃO é o usuário logado, não é problema meu → não notifica. */
    var me = window.S.userId;
    var actOwner = a.userId || a._ownerId || null;
    if (actOwner && String(actOwner) !== String(me)) return false;
    ...
    if (idx === -1) {
      ...
      var n = list[idx];
      /* [FIX 20260820-2] NUNCA reabre uma notificação que o usuário já leu.
         Antes: se n.lida era true (usuário marcou como lida), este bloco a
         reabria (n.lida=false) toda vez que a atividade continuava atrasada
         — que é exatamente o sintoma do sino piscando sem parar. Agora só
         ATUALIZA o texto/hora se ainda estiver não lida; uma vez lida, fica
         lida até a atividade ser resolvida (done) — aí _markDoneActNotifRead
         garante a limpeza. */
