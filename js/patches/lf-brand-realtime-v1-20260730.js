...
(function(root){
  ...
  var POLL_MS   = 20000;                   // 20 s — barato via ETag/304
  var FAST_MS   = 1500;                    // reforço 1.5 s logo após salvar
  ...
  function paintLogo(logoData){
    ...
    var alt = logoData ? 'Logo do CRM' : 'Líder Financeira e Investimentos';
  ...
  function paintTitle(nameText){
    var t = (nameText||'LIDER CRM').replace(/\s+/g,' ').trim();
  ...
  function paintManifest(brand){
    try{
      ...
      var manifest = {
        name:       brand.name || 'LIDER CRM',
        short_name: (brand.name || 'LIDER CRM').slice(0,12),
        start_url:  origin,
        scope:      origin,
        display:    'standalone',
  ...
  function paintCapacitorName(nameText){
    try{
      ...
      if(App && typeof App.setName === 'function'){
        App.setName({ name: nameText || 'LIDER CRM' }).catch(function(){});
      }
    }catch(_e){}
  }
  ...
  function _tick(){
    ...
    }).catch(function(e){ WARN('poll falhou (ignorado)', e && e.message); });
  }
  ...
  function bootBrand(){
    ...
    }).catch(function(e){ WARN('boot fetch falhou', e && e.message); });

    // 3) Setup canal + polling contínuo
    _setupChannel();
  ...
  if(typeof _origAdmResetCRMName === 'function'){
    root.admResetCRMName = function(){
      ...
      _saveRemote({ name:'LIDER CRM', nameImg:null }).then(function(){
