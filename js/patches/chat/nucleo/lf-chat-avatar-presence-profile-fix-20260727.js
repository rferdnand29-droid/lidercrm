...
(function(){
  ...
  function renderNodeAvatar(el, uid, name, color){
    if(!el) return;
    var pic = getPic(uid);
    /* CORREÇÃO 2026-08-07 (relatado: "foto de perfil instável, às vezes
       dando glitchs"): esta função é chamada de novo a cada 15s
       (setInterval mais abaixo) mesmo sem NADA ter mudado — e sempre
       reconstruía o <img> do zero (el.innerHTML = ...), forçando o
       navegador a redecodificar a mesma imagem repetidamente. Numa
       imagem base64 de até ~900KB (ver compressImageFile), redecodificar
       do zero a cada 15s é perceptível — um pisca/flash periódico,
       batendo com "instável, às vezes glitcha". Agora só reconstrói o
       DOM se a foto realmente mudou desde a última vez (compara a
       própria string base64/URL guardada como data-attribute) — sem
       mudança, não mexe em nada, sem decodificação repetida à toa. */
    if(pic){
      if(el.dataset && el.dataset.lfPicSrc===pic) return; // já está mostrando essa foto, não refaz
      el.style.background = 'transparent';
      el.innerHTML = '<img src="'+escAttr(pic)+'" alt="" style="width:100%;height:100%;border-radius:50%;object-fit:cover;display:block">';
      if(el.dataset) el.dataset.lfPicSrc = pic;
      return;
    }
    if(el.dataset && el.dataset.lfPicSrc==='') return; // já está mostrando a letra, não refaz
    el.innerHTML = '';
    el.textContent = String(name||'?').charAt(0).toUpperCase();
    if(color) el.style.background = color;
    if(el.dataset) el.dataset.lfPicSrc = '';
  }
