...
(function(global){
  ...
  /* ==================================================================
   * FIX 1 — força ocultação do logo/brand do login mesmo se o markup
   *          for re-renderizado dinamicamente
   * ================================================================== */
  (function purgeLoginBrand(){
    function purge(){
      try{
        var scope = D.getElementById('login-screen');
        if (!scope) return;
        scope.querySelectorAll('.lmon, .lbr, .crm-brand-name, .lf-logo-img')
