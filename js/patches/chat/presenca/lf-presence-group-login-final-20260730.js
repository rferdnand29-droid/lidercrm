(function(global){
  ...
  function ensureRawRequest(){
    ...
    client.rawRequest = function(path, method, body){
      ...
      }).then(function(r){ return r.text().then(function(t){ var d=null; try{ d=t?JSON.parse(t):null; }catch(_e){ d=t; } return { ok:r.ok, status:r.status, data:d, headers:r.headers }; }); });
    };
  }
  function apiReq(path, method, body, keepalive){
    ...
    return fetch(path, {
      ...
      return r.text().then(function(t){ var d=null; try{ d=t?JSON.parse(t):null; }catch(_e){ d=t; } return { ok:r.ok, status:r.status, data:d, headers:r.headers }; });
    }).catch(function(err){
  ...
  function refreshOnlineCache(){
    return apiReq('/api/v1/users/online', 'GET', null, false).then(function(res){
      ...
      refreshPresenceUI();
      return next;
    }).catch(function(){ return presCache; });
  }
  ...
  global.addEventListener('focus', function(){ if (meUid()) { installPresence(); refreshOnlineCache(); } }, true);
