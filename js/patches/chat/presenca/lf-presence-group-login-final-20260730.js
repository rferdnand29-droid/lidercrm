(function(global){
  ...
  /* [FIX 20260903] O fallback lia o token cru do localStorage sem checar a
     validade: com a sessão vencida, cada heartbeat/last-seen saía com um
     Bearer morto e voltava 401 (era essa a enxurrada de
     "POST /api/v1/users/last-seen 401" no console). Token expirado agora
     conta como ausente — beatPresence/markLastSeen simplesmente não
     disparam até a sessão ser renovada. */
  function getWorkerToken(){
    return safe(function(){
      var ss = global.LiderCRM && global.LiderCRM.api && global.LiderCRM.api.httpClient && global.LiderCRM.api.httpClient.session;
      if (ss && typeof ss.isValid === 'function' && !ss.isValid()) return '';
      ...
      if (raw){
        ...
        var expiresAt = Number(obj && obj.expiresAt) || 0;
        if (obj && obj.token && (!expiresAt || expiresAt > Date.now() + 5000)) return String(obj.token);
        return '';
      }
