 ...
 *    STATUS_NOSHOW.
 * 4) [FIX 20260820] Instala um hook em _finalizeSaveKBCard (js/kanban.js):
 *    o modal "Editar" de um card também troca a Etapa (o <select id="kb-col">
 *    do modal), mas SALVA de um jeito completamente diferente de mover no
 *    board — escreve cx.col direto e chama saveKBFor(), sem nunca passar
 *    por _kbMoveCard. Por isso o Hook 1 nunca disparava nesse caminho: o
 *    Bingo só ficava em dia na próxima reconciliação de boot (login/reload
 *    da página). CAUSA RAIZ do bug relatado ("às vezes demora"/"às vezes
 *    não aparece" ao mover pra Presencial ou Remarcar): depende de qual
 *    dos dois jeitos a pessoa usou pra mover o card — arrastar/clicar na
 *    etapa no board (Hook 1, sincroniza na hora) sempre funcionava; editar
 *    pelo modal "Editar" e trocar a Etapa por lá (nunca sincronizava até o
 *    próximo login) não.
 * 5) Reconciliação no boot: varre uma vez todos os cards de Negócios do
 ...
 * 6) Ajusta o filtro da aba "normal" do Bingo (renderTable em
...
(function(global){
  'use strict';
  if (global.__lfFixBingoSyncV1) return;
  global.__lfFixBingoSyncV1 = true;
  ...
  /* CORREÇÃO 2026-08-03: antes de criar um registro novo no Bingo, respeita
     a "lápide" de exclusão manual (window._bingoIsDismissed, definida em
     js/clientes.js junto de confirmDC). Sem isso, apagar um cliente do
     Bingo pelo botão de lixeira não "grudava" — a reconciliação de boot (ou
     o próprio hook de movimento) recriava o registro na próxima vez que o
     card ainda estivesse parado em AG Vídeo/Presencial. A função já expira
     sozinha a lápide se o card mudou de etapa depois da exclusão. */
  function _isDismissed(uid,card){
    try{return typeof global._bingoIsDismissed==='function'&&global._bingoIsDismissed(uid,card);}
  ...
  function syncNegocioToBingo(card,ownerUid,newCol){
    ...
    try{
      ...
      if(!cli){
        ...
        if(_isDismissed(uid,card))return; // excluído manualmente do Bingo; card não mudou desde então
  ...
  function reconcileBingoFromNegocios(){
    ...
    try{
      ...
      negs.forEach(function(card){
        ...
        // CORREÇÃO (2026-08-02): só aplica o mapeamento coluna→status pra
        // registro RECÉM-criado aqui. Antes, um registro que JÁ EXISTIA
        // também passava por _applyMapping toda vez que o app abria — e
        // como o card de Negócios muitas vezes continua "parado" em
        // agvid/presencial mesmo depois do cliente ser marcado No-Show/
        // Atendido pelo Bingo (os dois nem sempre se movem juntos), essa
        // reconciliação forçava o status de volta pra Agendado (normal)
        // toda vez, mesmo sem nenhuma mudança real ter acontecido — é
        // isso que fazia lead já no-show (ou nem agendado de verdade)
        // reaparecer como Agendado. O hook em tempo real (_hookMoveCard,
        // abaixo) continua aplicando o mapeamento normalmente quando o
        // card REALMENTE muda de coluna — só a reconciliação de boot
        // parou de reescrever status de registro já existente.
        if(isNewCli && _applyMapping(cli,card.col))mutated=true;
      });
  ...
  function _hookMoveCard(){
    ...
    global._kbMoveCard=function(cardId,board,uid,newCol,silent,bulk,dropIndex){
      var _oldCol=null;
      try{
        if(board==='negocios'&&typeof global.getKBFor==='function'){
          var _before=(global.getKBFor(board,uid)||[]).find(function(x){return x&&x.id===cardId;});
          if(_before)_oldCol=_before.col;
        }
      }catch(_e){}
      var card=orig.apply(this,arguments);
      try{
        // Só sincroniza se a coluna REALMENTE mudou. _kbMoveCard também é
        // usado pra reordenar dentro da mesma coluna (arrastar pra cima/
        // baixo na mesma etapa, ver dropIndex) — sem essa checagem, uma
        // simples reordenação em agvid/presencial reaplicava o mapeamento
        // e resetava o status do Bingo pra Agendado de novo.
        if(card&&board==='negocios'&&card.col!==_oldCol){
  ...
  /* Hook 4 [FIX 20260820]: _finalizeSaveKBCard (js/kanban.js) — chamado ao
     salvar o modal "Editar" de um card. Esse caminho NUNCA passa por
     _kbMoveCard: escreve cx.col direto no array e chama saveKBFor(). Por
     isso o Hook 1 nunca disparava aqui — o Bingo só ficava em dia na
     próxima reconciliação de boot (login/reload). Isso explica o bug
     relatado: mover pra Presencial/Remarcar arrastando ou clicando na
     etapa (Hook 1) sempre sincronizava na hora; fazer a mesma troca de
     etapa pelo modal "Editar" não sincronizava até o próximo login —
     parecia "às vezes demora"/"às vezes não aparece" porque dependia de
     qual dos dois jeitos a pessoa usou, sem nenhuma diferença visível
     pra quem está mexendo no card. */
  function _hookFinalizeSaveKBCard(){
    if(typeof global._finalizeSaveKBCard!=='function'){
      _warn('_finalizeSaveKBCard não encontrado — hook do modal Editar ignorado');
      return;
    }
    if(global._finalizeSaveKBCard.__lfBingoSyncWrapped)return;
    var orig=global._finalizeSaveKBCard;
    global._finalizeSaveKBCard=function(board,editId,arr){
      var _oldCol=null;
      try{
        if(board==='negocios'&&editId&&Array.isArray(arr)){
          var _before=arr.find(function(x){return x&&x.id===editId;});
          if(_before)_oldCol=_before.col;
        }
      }catch(_e){}
      var ret=orig.apply(this,arguments);
      try{
        if(board==='negocios'&&editId&&Array.isArray(arr)){
          // arr é o MESMO array (e os MESMOS objetos, por referência) que
          // orig acabou de mutar — reler agora já reflete a nova coluna,
          // sem precisar buscar de novo em storage.
          var card=arr.find(function(x){return x&&x.id===editId;});
          if(card&&card.col!==_oldCol){
            var ownerUid=global._kbEditOwnerUid
              ||(typeof global.activeUID==='function'?global.activeUID(board):null)
              ||(global.S&&global.S.userId);
            syncNegocioToBingo(card,ownerUid,card.col);
          }
        }
      }catch(e){_warn('hook _finalizeSaveKBCard',e);}
      return ret;
    };
    global._finalizeSaveKBCard.__lfBingoSyncWrapped=true;
    _log('hook _finalizeSaveKBCard instalado');
  }
