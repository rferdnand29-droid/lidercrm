...
function saveKB(b,list){
  ...
  /* [FIX 20260907-KANBAN409-CIRURGICO] A gravacao remota agora passa pela MESMA fila serial por
     documento usada por saveKBFor — dois PUTs do mesmo board nunca saem
     em voo juntos (era exatamente isso que gerava o loop PUT->409). A
     gravacao local otimista em cima continua identica. */
  if(wc||(DB_MODE==='firebase'&&db)){_kbRemoteEnqueue(b,S.userId,wc,function(){});}
  return localOk;
}

/* [FIX 20260907-KANBAN409-CIRURGICO]
   Causa-raiz do loop "PUT /api/v1/kanban/list 409 (Conflict)" e do tremor
   das etapas: polling (15s), SSE (lf-realtime-kanban) e BroadcastChannel
   disparavam _syncKBRemoteBG ao mesmo tempo; cada ciclo chamava saveKBFor
   e varios PUTs do MESMO documento (board+uid) saiam em voo juntos. Todos
   mandavam a mesma versao — do segundo em diante, 409; cada um fazia o
   seu GET de recuperacao e re-PUTava, e o ciclo recomeçava, repintando o
   quadro a cada volta (renderKBLocal no caminho do 409).
   Correcao: UMA fila serial por documento. Enquanto um PUT esta em voo,
   novos saveKBFor apenas marcam "tem pendente"; ao concluir, a fila rele
   o localStorage (sempre a lista mais nova — o patch de tombstones ja
   injeta deletedIds nela antes de gravar) e envia um unico PUT. O PUT
   bem-sucedido atualiza o cache de versao do worker-client (ETag), entao
   o PUT seguinte ja sai com a versao certa: sem 409, sem loop, sem
   repaint em cadeia. Idempotente: marcador checado na aplicacao. */
var _kbRemoteQ=(window.__lfKbRemoteQ=window.__lfKbRemoteQ||{});
function _kbRemoteEnqueue(b,uid,wc,done){
  var key=b+'::'+uid;
  var st=_kbRemoteQ[key]||(_kbRemoteQ[key]={inflight:false,pending:false,waiters:[]});
  if(typeof done==='function')st.waiters.push(done);
  st.pending=true;
  if(st.inflight)return;
  st.inflight=true;
  function _flushWaiters(ok,info){
    var ws=st.waiters.splice(0);
    ws.forEach(function(f){try{f(ok,info);}catch(_e){}});
  }
  function _step(attempt){
    var wcNow=_kbWorkerClient();
    var useFirebase=(!wcNow&&DB_MODE==='firebase'&&db);
    if(!wcNow&&!useFirebase){st.inflight=false;st.pending=false;_flushWaiters(true,null);return;}
    st.pending=false;
    var payload=getKBFor(b,uid);
    var p=wcNow?wcNow.saveKanbanList(b,uid,payload)
               :db.collection('kb_'+b).doc(uid).set({list:payload,ts:Date.now()});
    syncBusy();
    p.then(function(r){
      syncOk();
      if(st.pending){_step(1);return;}
      st.inflight=false;_flushWaiters(true,r);
    }).catch(function(e){
      if(e&&(e.versionConflict||e.status===409)){
        /* 409 real (outro dispositivo gravou): o worker-client ja
           sincronizou a versao no cache. Puxa o doc atual, aplica o
           merge padrao e, se o merge local ainda divergir do servidor,
           reenfileira UM envio com a versao nova — em vez de martelar
           PUT->409. Se convergiu, encerra sem repintar em loop. */
        syncErr();
        try{
          if(wcNow&&typeof wcNow.kanbanList==='function'){
            wcNow.kanbanList(b,uid).then(function(doc){
              var server=(doc&&doc.list)||[];
              var merged=(typeof _mergeKeepLocalOnly==='function')?_mergeKeepLocalOnly(server,getKBFor(b,uid)):server;
              ss(kbKeyFor(b,uid),merged);
              var stillDiff=true;
              try{stillDiff=(typeof window._lfListsEqualById==='function')?!window._lfListsEqualById(server,merged):true;}catch(_e){}
              if(stillDiff)st.pending=true;
              try{if(typeof renderKBLocal==='function')renderKBLocal(b);}catch(_re){}
              if(st.pending){_step(1);return;}
              st.inflight=false;_flushWaiters(false,e);
            }).catch(function(){st.inflight=false;_flushWaiters(false,e);});
            return;
          }
        }catch(_ce){}
        st.inflight=false;_flushWaiters(false,e);
      ...
      if(attempt<2){setTimeout(function(){_step(attempt+1);},1500);return;}
      st.inflight=false;
      syncErr();
      try{
        if(wcNow&&window.LiderCRM&&window.LiderCRM.offline&&window.LiderCRM.offline.retryQueue){
          window.LiderCRM.offline.retryQueue.enqueue({
            method:'PUT',
            path:'/kanban/list?board='+encodeURIComponent(b)+'&uid='+encodeURIComponent(uid),
            body:{uid:uid,list:getKBFor(b,uid)},
...
function saveKBFor(b,uid,list,onRemoteDone){
  ...
  /* [FIX 20260907-KANBAN409-CIRURGICO] saveKBFor agora so grava local + avisa as outras abas
     (acima) e ENFILEIRA a gravacao remota na fila serial por documento:
     enquanto houver PUT em voo, novas chamadas so marcam pendencia; o
     envio seguinte rele o localStorage e leva sempre a lista mais nova. */
  if(wc||(DB_MODE==='firebase'&&db)){_kbRemoteEnqueue(b,uid,wc,_done);}
...
/* [FIX 20260908-STAGE-PICKER-LEADS] Na aba Leads (não-ADM) a lista-base é
   esta função (render desktop kanban.js:~970, lista mobile ~3981 e o picker
   _kbBaseListForSummary ~4097) — diferente de Negócios, que usa getKBFor
   direto. Se o cache do pool "livre" do servidor estiver ausente/corrompido
   (boot offline, storage cheio), qualquer exceção aqui derrubava o picker e
   o render inteiros — o botão "escolher etapa" parecia morto só em Leads.
   Agora a função inteira é à prova de falha: em qualquer erro devolve ao
   menos os leads do próprio usuário. */
function _collectLivrePoolForUser(uid){
  try{return _collectLivrePoolForUserUnsafe(uid);}catch(_e){
    try{return getKBFor('leads',uid)||[];}catch(_e2){return [];}
  }
}
function _collectLivrePoolForUserUnsafe(uid){
  var seen={};
  var out=[];
...
function toggleKBLateFilter(board,btn){
  ...
  /* [FIX 20260907] Bug 2 causa 2.4 — ao ATIVAR o filtro "Atrasadas",
     o cache local das atividades dos colegas pode estar velho (só era
     atualizado ao abrir o painel ADM), e conclusões feitas por eles em
     outro dispositivo não tinham chegado — o card aparecia atrasado sem
     atividade real atrasada. Agora, ao ligar o filtro, refrescamos do
     servidor (loadAllActivitiesAdmin já respeita a guarda de permissão:
     sem visão global, só recarrega a própria lista) e re-renderizamos
     quando o refresh conclui. Com o filtro desligado, comportamento
     inalterado. */
  if(_kbOnlyLate[board]&&typeof loadAllActivitiesAdmin==='function'){
    try{
      loadAllActivitiesAdmin(function(){
        try{
          if(_kbOnlyLate[board]){
            renderKBLocal(board);
...
function _kbHasOverdueLinkedActivity(card, ownerUid, board){
  ...
  /* [FIX 20261101] Bug 2 causa 2.1 — o espelho card.activities pode estar
     desatualizado (conclusão feita por outro dispositivo/aba, conclusão de
     outro usuário, falha de saveKBFor, corrida de sync). Antes bastava a
     store central não achar nada atrasado para o fallback disparar
     "Atrasada" a partir do espelho velho. Agora o fallback só é usado se
     a store central estiver indisponível para o uid — se ela existe e
     respondeu (mesmo que com lista vazia), confiamos nela como fonte de
     verdade e ignoramos o espelho legado. */
  if(typeof getActivitiesLocalFor === 'function' && uid) return false;
  ...
  for(var i=0; i<acts.length; i++){
    ...
    /* [FIX 20261101] Bug 2 causas 2.1/2.2/2.3 — endurece o fallback
       legado com os mesmos guardas do caminho novo:
       - _lfIsRecentlyDone (persistente, 7 dias) contra corrida de sync;
       - _doneLocalAt (flag local otimista);
       - orfã / etapa terminal / card descartado;
       - board obrigatório (atividades sem board não devem casar com
         qualquer quadro por default — Causa 2.2);
       - vínculo ao card por clientId (a.clientId === card.id). */
    if(a._doneLocalAt) continue;
    if(a.id && typeof window._lfIsRecentlyDone==='function' && window._lfIsRecentlyDone(a.id)) continue;
    if(!a.id) continue;
    if(!a.scheduledAt) continue;
    if(!a.board) continue; /* Causa 2.2: sem board não casa com quadro nenhum */
    if(board && a.board !== board) continue;
    if(a.clientId && String(a.clientId) !== String(card.id || '')) continue;
    if(typeof _isActivityOrphanOrInactive==='function' && _isActivityOrphanOrInactive(a, uid)) continue;
...
function assumeLead(cardId,board,ownerUid){
  ...
  wc.kanbanClaimLivre(cardId,uid).then(function(claimed){
    ...
    /* [FIX 20260907] Bug 2 causa 2.3 — migra as atividades vinculadas ao
       card da loja do dono ANTERIOR para a loja do NOVO dono. Antes, ao
       assumir o lead as atividades ficavam na loja do consultor antigo:
       o filtro "Atrasadas" olhava a loja do dono novo (vazia) e caía no
       fallback do espelho card.activities (desatualizado) → falso
       "Atrasada". Best-effort: se a loja antiga não estiver em cache
       neste dispositivo, o merge por id do Worker (atividades-controller,
       mesmo fix) converge na próxima sync de qualquer dos dois donos. */
    try{
      if(typeof getActivitiesLocalFor==='function'&&typeof lfSaveActivitiesFor==='function'){
        var _oldActs=getActivitiesLocalFor(uid);
        var _linked=_oldActs.filter(function(a){return a&&String(a.clientId||'')===String(cardId);});
        if(_linked.length){
          var _remain=_oldActs.filter(function(a){return !(a&&String(a.clientId||'')===String(cardId));});
          lfSaveActivitiesFor(uid,_remain);
          var _nowISO=new Date().toISOString();
          var _newActs=getActivitiesLocalFor(S.userId);
          _linked.forEach(function(a){
            var _cp=Object.assign({},a,{userId:S.userId,updatedAt:_nowISO});
            delete _cp._pending;delete _cp._doneLocalAt;
            _newActs.push(_cp);
          });
          lfSaveActivitiesFor(S.userId,_newActs);
        }
      }
    }catch(_eMigrate){console.warn('[kb] migração de atividades no assumir falhou (best-effort)',_eMigrate);}
...
function openStagePicker(board,cardId,ownerUid){
  ...
  if(!listEl)return;
  /* [FIX 20260908-STAGE-PICKER-LEADS] As opções do picker devem vir SEMPRE
     do pipeline do próprio board. Antes kbCols('negocios') podia herdar
     _kbDetOwnerUid de um detalhe aberto de outro board: com dono do cargo
     Administrativo, as opções saíam com IDs adm_* (pipeline inexistente em
     Leads / pipeline errado em Negócios), e ao escolher, o card ia para uma
     coluna que nenhum render exibe — "nada acontecia" / o card sumia. */
  var cols=(typeof KB_LEADS_COLS!=='undefined'&&board==='leads')?KB_LEADS_COLS
    :((typeof KB_NEG_COLS!=='undefined'&&board==='negocios')?KB_NEG_COLS:kbCols(board));
...
function _spSelect(colId){
  ...
  if(cardId){
    ...
    _mbStageFilter[board]=colId;
    /* [FIX 20260908-STAGE-PICKER-DESKTOP] No modo filtro (barra de etapas),
       antes só renderKBMobile era chamado — no desktop (onde a barra também
       existe: index.html #947/#1042) escolher uma etapa não mudava nada na
       tela, e o botão parecia quebrado. renderKBLocal já chama
       renderKBMobile por dentro quando isMobileView(); no desktop, redesenha
       a barra de resumo (e futuros filtros de etapa) direto do cache local. */
    renderKBLocal(board);
    if(typeof renderStageSummaryBar==='function')renderStageSummaryBar(board);
    if(typeof renderMobileChips==='function')renderMobileChips(board);
  }
