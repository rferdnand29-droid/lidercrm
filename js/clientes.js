...
function renderTable(list){
  ...
  if(_dashTab==='normal'&&typeof getKBFor==='function'){
    _bgByCard={};_bgByLead={};
    (getKBFor('negocios',_uidBg)||[]).forEach(function(n){
      if(!n)return;
      if(n.id)_bgByCard[n.id]=n.col;
      if(n.originalLeadId)_bgByLead[n.originalLeadId]=n.col;
    });
  }
  var filtered=list.filter(function(c){
    ...
    // REGRA DO BINGO (2026-08-03): só mostra em Agendados o cliente cujo
    // Negócio vinculado está atualmente em AG Vídeo ou Presencial. Sem
    // vínculo (lançado manual) não é afetado.
    if(_bgByCard){
      var _bCol=(c.sourceCardId&&_bgByCard[c.sourceCardId])||(c.sourceOriginalLeadId&&_bgByLead[c.sourceOriginalLeadId]);
      if(_bCol&&_bCol!=='agvid'&&_bCol!=='presencial')return false;
    }
...
function saveC(){
  ...
  list.push({
    id:'c'+Date.now()+'_'+Math.random().toString(36).slice(2,10),
    nome:n,
    steps:steps,
    stepDates:stepDates,
    data:now,
...
function confirmDC(){
  ...
  if(_delC&&(_delC.sourceCardId||_delC.sourceOriginalLeadId)&&typeof getKBFor==='function'){
    var _negs=getKBFor('negocios',S.userId)||[];
    var _linked=(_delC.sourceCardId&&_negs.find(function(n){return n&&n.id===_delC.sourceCardId;}))||
