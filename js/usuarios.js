...
function openDeptEditor(id){
  ...
  parentSel.innerHTML='<option value="">Nenhum — departamento raiz</option>'+all.filter(function(x){return x.id!==id;}).map(function(x){return '<option value="'+_htmlAttr(x.id)+'"'+(d&&d.parentId===x.id?' selected':'')+'>'+eH(x.nome)+'</option>';}).join('');
  var supSel=document.getElementById('dept-sup');
  supSel.innerHTML='<option value="">Selecione...</option>'+users.map(function(u){return '<option value="'+_htmlAttr(u.id)+'"'+(d&&d.supervisorId===u.id?' selected':'')+'>'+eH(u.nome)+'</option>';}).join('');
  var adjSel=document.getElementById('dept-sup-adj');
  adjSel.innerHTML='<option value="">Nenhum</option>'+users.map(function(u){return '<option value="'+_htmlAttr(u.id)+'"'+(d&&d.supervisorAdjId===u.id?' selected':'')+'>'+eH(u.nome)+'</option>';}).join('');
...
function showCred(uid,pw){
  ...
  // 2026-08-07: removida a leitura da caixa "Ativar acesso ao Painel ADM"
  // (eu-admin-check/k-admin-check) — acesso ADM agora vem exclusivamente
  // de departamento atribuído pelo Hudson, não mais de um toggle manual
  // por cargo. u.admExtra deixa de ser lido/gravado por aqui.
  openM('mo-k');
}
...
function saveCredCargo(){
  ...
  // 2026-08-07: removida a leitura/gravação de admExtra via checkbox —
  // ver comentário na função que preenche este modal, acima.
...
function openEditUser(uid){
  ...
  // 2026-08-07: removida a leitura da caixa "Ativar acesso ao Painel ADM"
  // — ver comentário na função equivalente do modal de credenciais.
  openM('mo-edit-user');
}
...
function saveEditUser(){
  ...
  // 2026-08-07: removida a leitura/gravação de admExtra via checkbox —
  // acesso ADM agora vem exclusivamente de departamento atribuído pelo
  // Hudson (ver js/patches/scope/lf-cargo-only-via-departamento-v1-20260804.js).
