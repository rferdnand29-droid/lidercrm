...
/* ------------------------------------------------------------------
 * MULTI-SUPERVISOR / MULTI-ADJUNTO (2026-08-20):
 * Um departamento agora aceita MAIS DE UM supervisor e MAIS DE UM
 * supervisor adjunto. Modelo novo (arrays): dept.supervisorIds e
 * dept.supervisorAdjIds. Os campos legados supervisorId/supervisorAdjId
 * (escalares) CONTINUAM sendo gravados com o PRIMEIRO selecionado de
 * cada grupo — é o que garante que consumidores antigos (cards da
 * Estrutura, patches de escopo, worker) não quebrem.
 * _deptSupIds/_deptAdjIds leem os dois formatos (array novo ou escalar
 * antigo), então departamentos criados antes desta mudança seguem
 * funcionando sem migração manual.
 * ------------------------------------------------------------------ */
function _deptSupIds(dept){
  var ids=[];
  (Array.isArray(dept&&dept.supervisorIds)?dept.supervisorIds:[]).forEach(function(x){if(x&&ids.indexOf(x)<0)ids.push(x);});
  if(!ids.length&&dept&&dept.supervisorId)ids.push(dept.supervisorId);
  return ids;
}
function _deptAdjIds(dept){
  var ids=[];
  (Array.isArray(dept&&dept.supervisorAdjIds)?dept.supervisorAdjIds:[]).forEach(function(x){if(x&&ids.indexOf(x)<0)ids.push(x);});
  if(!ids.length&&dept&&dept.supervisorAdjId)ids.push(dept.supervisorAdjId);
  return ids;
}

function _deptUserBelongs(dept,uid){
  return _deptSupIds(dept).indexOf(uid)>=0||_deptAdjIds(dept).indexOf(uid)>=0||(dept.memberIds||[]).indexOf(uid)>=0;
}

function _normalizeDeptRefsForUsers(dept,usersMap){
  if(!dept)return dept;
  var changed=false;
  var supervisorIds=_deptSupIds(dept).filter(function(uid){
    if(!uid||(usersMap&&!usersMap[uid])){changed=true;return false;}
    return true;
  });
  var adjSeen={},supervisorAdjIds=_deptAdjIds(dept).filter(function(uid){
    if(!uid||(usersMap&&!usersMap[uid])||supervisorIds.indexOf(uid)>=0||adjSeen[uid]){changed=true;return false;}
    adjSeen[uid]=1;
    return true;
  });
  /* Legado escalar = primeiro de cada lista (mantém retrocompatibilidade
     com o que ainda lê supervisorId/supervisorAdjId). */
  var supervisorId=supervisorIds[0]||null;
  var supervisorAdjId=supervisorAdjIds[0]||null;
  if(dept.supervisorId!==supervisorId||dept.supervisorAdjId!==supervisorAdjId)changed=true;
  var supAdjMap={};
  supervisorIds.forEach(function(x){supAdjMap[x]=1;});
  supervisorAdjIds.forEach(function(x){supAdjMap[x]=1;});
  var seen={},src=Array.isArray(dept.memberIds)?dept.memberIds:[];
  var memberIds=src.filter(function(uid){
    if(!uid||supAdjMap[uid]||seen[uid])return false;
  ...
  if(!Array.isArray(dept.supervisorIds)||dept.supervisorIds.length!==supervisorIds.length)changed=true;
  if(!Array.isArray(dept.supervisorAdjIds)||dept.supervisorAdjIds.length!==supervisorAdjIds.length)changed=true;
  if(!changed)return dept;
  return Object.assign({},dept,{supervisorId:supervisorId,supervisorAdjId:supervisorAdjId,supervisorIds:supervisorIds,supervisorAdjIds:supervisorAdjIds,memberIds:memberIds,ts:Date.now()});
}
...
function _cleanupDepartmentsForRemovedUser(uid){
  ...
  var next=all.map(function(d){
    if(!d)return d;
    var base=d;
    if(_deptSupIds(d).indexOf(uid)>=0||_deptAdjIds(d).indexOf(uid)>=0||(d.memberIds||[]).indexOf(uid)>=0){
      base=Object.assign({},d,{
        supervisorIds:_deptSupIds(d).filter(function(x){return x!==uid;}),
        supervisorAdjIds:_deptAdjIds(d).filter(function(x){return x!==uid;}),
...
/* ------------------------------------------------------------------
 * FALLBACK AUTOMÁTICO POR ORIENTADOSIDS (patch 2026-07-27):
 *
 * Problema: getDepartmentVisibleUsers() dependia 100% de um cadastro
 * MANUAL em Estrutura (Departamentos: supervisorId/supervisorAdjId/
 * memberIds). Se ninguém criou o Departamento pra um Supervisor novo
 * (ou esqueceu de marcar um consultor em memberIds), a função caia na
 * linha "só ele mesmo" — a aba Time ficava vazia mesmo o cargo tendo
 * escopo:'team' em CARGO_CAPS. Sintoma reportado: supervisor não via
 * "Todos" nem consultores específicos (ex.: Rhuan) em Time.
 *
 * Solução (aditiva, sem tocar em Departamentos nem no backend):
 * reusa o campo u.orientadosIds, que já existe, já é editável na tela
 * de Credenciais/Editar usuário (k-orientados / eu-orientados) e já é
 * salvo por saveCredCargo()/saveEditUser() — só nunca era consultado
 * fora do antigo nível "Orientador". Agora serve como fallback: se o
 * Departamento não resolver ninguém além do próprio supervisor, usa
 * orientadosIds como lista de subordinados diretos. Departamento
 * continua tendo prioridade quando já está configurado corretamente
 * (nada muda pra quem já usa Estrutura).
 * ------------------------------------------------------------------ */
/* FIX (2026-08-03): nova primeira tentativa de resolução — via
 * team_id -> teams.departamento_id (LF_SCOPE_V2.departamentoOfUser),
 * a MESMA fonte que agora protege leads/negócios/clientes de verdade
 * no banco (RLS). Departamentos criados/atribuídos por
 * LF_DEPARTMENTS.assignUsersToDept() aparecem aqui automaticamente,
 * sem precisar configurar nada em Estrutura. O sistema antigo
 * (Estrutura/getDepartments()/orientadosIds) continua funcionando
 * como fallback pra quem já tinha configurado por lá — nada quebra
 * pra ninguém que já usava.
 */
function getDepartmentVisibleUsers(uid){
  ...
  if(typeof LF_SCOPE_V2!=='undefined' && typeof LF_SCOPE_V2.departamentoOfUser==='function'){
    var myDeptNovo=LF_SCOPE_V2.departamentoOfUser(uid);
    if(myDeptNovo){
      var deptVisibleNovo=allUsers.filter(function(u){
        return u.id===uid || LF_SCOPE_V2.departamentoOfUser(u.id)===myDeptNovo;
      });
      if(deptVisibleNovo.length>1)return deptVisibleNovo;
    }
  }

  var deptIds=(getDepartments()||[]).filter(function(d){return _deptUserBelongs(d,uid);}).map(function(d){return d.id;});
  var deptVisible=deptIds.length?allUsers.filter(function(u){
    if(u.id===uid)return true;
    return (getDepartments()||[]).some(function(d){return deptIds.indexOf(d.id)>=0&&_deptUserBelongs(d,u.id);});
  }):null;
  if(deptVisible&&deptVisible.length>1)return deptVisible;
  var me=getUser(uid);
  var orIds=(me&&Array.isArray(me.orientadosIds))?me.orientadosIds.filter(Boolean):[];
  if(orIds.length)return allUsers.filter(function(u){return u.id===uid||orIds.indexOf(u.id)>=0;});
  // [FIX 20260820] Se departamento nenhum está configurado no sistema
  // inteiro (nem Estrutura manual, nem team_id→departamento do
  // LF_SCOPE_V2 pra NINGUÉM), a funcionalidade de Departamentos
  // simplesmente não está em uso — cair pro "só eu mesmo" aqui não
  // respeita fronteira real nenhuma, é um acidente: "ninguém tem
  // departamento" virava "todo supervisor só vê a si mesmo", incluindo
  // gente que precisa ver o time inteiro pra trabalhar. Verifica se
  // ALGUÉM no sistema tem departamento resolvido (por qualquer das duas
  // fontes); se não, mostra todos — mesmo comportamento de antes da
  // funcionalidade de Departamentos existir. Se departamentos EXISTEM
  // (a empresa já usa) mas esse supervisor especificamente não está
  // atribuído a nenhum, mantém o mínimo seguro (só ele) — aí sim é uma
  // fronteira real sendo respeitada.
  try{
    var _hasAnyDeptConfigured=(getDepartments()||[]).length>0;
    if(!_hasAnyDeptConfigured && typeof LF_SCOPE_V2!=='undefined' && typeof LF_SCOPE_V2.departamentoOfUser==='function'){
      _hasAnyDeptConfigured=allUsers.some(function(u){return !!LF_SCOPE_V2.departamentoOfUser(u.id);});
    }
    if(!_hasAnyDeptConfigured)return allUsers;
  }catch(_e){}
  return deptVisible||allUsers.filter(function(u){return u.id===uid;});
}
...
function _deptCardHtml(d,all){
  var supsList=_deptSupIds(d).map(getUser).filter(Boolean);
  var adjsList=_deptAdjIds(d).map(getUser).filter(Boolean);
  var sup=supsList[0]||null;
  ...
  return '<div class="estr-card'+(mine?' mine':'')+(_deptSelectedId===d.id?' sel':'')+'" onclick="deptSelectCard(\''+d.id+'\')" tabindex="0" role="button" aria-label="Selecionar '+eH(d.nome||'departamento')+'">'
    ...
    +(sup?'<div class="estr-person-mini"><span class="estr-av" style="background:'+AVB[(sup.cor||0)%AVB.length]+'">'+eH(sup.nome.charAt(0))+'</span>'+eH(sup.nome)+(supsList.length>1?' <span style="color:var(--mu);font-size:.68rem">+'+(supsList.length-1)+'</span>':'')+'</div>':'<div class="estr-person-mini muted">Sem supervisor</div>')
    +'<div class="estr-card-foot"><span class="estr-chip">'+members.length+' colaborador'+(members.length===1?'':'es')+'</span>'
    +(adjsList.length?'<span class="estr-chip">Adjunto'+(adjsList.length>1?'s':'')+': '+eH(adjsList[0].nome.split(' ')[0])+(adjsList.length>1?' +'+(adjsList.length-1):'')+'</span>':'')
...
function deptSelectCard(id){
  ...
  var supUsers=_deptSupIds(d).map(getUser).filter(Boolean);
  var adjUsers=_deptAdjIds(d).map(getUser).filter(Boolean);
  var members=(d.memberIds||[]).map(getUser).filter(Boolean);
  var sups=supUsers.concat(adjUsers);
  ...
  panel.innerHTML='<div class="estr-panel-title">'+eH(d.nome)+'</div>'
    ...
    +(sups.length?'<div class="estr-panel-sec">Supervisores ('+sups.length+')</div>'+sups.map(function(u){return personRow(u,_deptSupIds(d).indexOf(u.id)>=0?'Supervisor':'Adjunto');}).join(''):'')
...
function openDeptEditor(id){
  ...
  // REMOVIDO (2026-08-20): campo 'Departamento pai (opcional)' retirado
  // do modal — todo departamento agora e raiz (parentId sempre null).
  /* Multi-seleção (2026-08-20): os <select> de supervisor/adjunto viraram
     múltiplos (markup trocado no HTML + re-estilo pelo patch
     lf-dept-multi-supervisores-20260820.js). Aqui basta manter as options
     e marcar TODOS os pré-selecionados vindos dos arrays
     supervisorIds/supervisorAdjIds (com fallback pros escalares legados). */
  var _selSupIds=d?_deptSupIds(d):[];
  var _selAdjIds=d?_deptAdjIds(d):[];
  var supSel=document.getElementById('dept-sup');
  supSel.innerHTML=users.map(function(u){return '<option value="'+_htmlAttr(u.id)+'"'+(_selSupIds.indexOf(u.id)>=0?' selected':'')+'>'+eH(u.nome)+'</option>';}).join('');
  var adjSel=document.getElementById('dept-sup-adj');
  adjSel.innerHTML=users.map(function(u){return '<option value="'+_htmlAttr(u.id)+'"'+(_selAdjIds.indexOf(u.id)>=0?' selected':'')+'>'+eH(u.nome)+'</option>';}).join('');
  var memWrap=document.getElementById('dept-members-list');
  memWrap.innerHTML=users.map(function(u){var checked=d&&(d.memberIds||[]).indexOf(u.id)>=0;return '<label style="display:flex;align-items:center;gap:8px;padding:5px 4px;font-size:.78rem;cursor:pointer"><input type="checkbox" value="'+_htmlAttr(u.id)+'" class="dept-member-cb"'+(checked?' checked':'')+'> '+eH(u.nome)+'</label>';}).join('');
...
function saveDept(){
  ...
  var parentId=null; // REMOVIDO (2026-08-20): 'Departamento pai' — hierarquia
  // pai/filho desativada; todo departamento salvo como raiz.
  ...
  var supervisorIds=Array.prototype.slice.call(document.getElementById('dept-sup').selectedOptions).map(function(o){return o.value;}).filter(Boolean);
  var supervisorAdjIds=Array.prototype.slice.call(document.getElementById('dept-sup-adj').selectedOptions).map(function(o){return o.value;}).filter(Boolean);
  ...
  var obj=_normalizeDeptRefsForUsers({id:id,nome:nome,parentId:parentId,supervisorIds:supervisorIds,supervisorAdjIds:supervisorAdjIds,memberIds:memberIds,ts:Date.now()},usersMap);
...
function createUser(){
  ...
  shSecure(pw).then(function(hash){
    var ncCheck=document.getElementById('nc-admin-check');
    var extra=ncCheck?ncCheck.checked:false;
    var newU={id:'u'+Date.now()+'_'+Math.random().toString(36).slice(2,5),nome:nome,email:email,cargo:cargo,ph:hash,data:data,role:'user',ativo:true,cor:users.length%5,admExtra:extra};
...
function adminResetPassword(uid){
  if(!hasAdminAccess()){toast('Sem permissão');return;}
  var u=getUser(uid);if(!u)return;
  var input=document.getElementById('k-reset-senha');
  var pw=(input&&input.value||'').trim();
  if(!pw||pw.length<8){toast('A nova senha precisa ter pelo menos 8 caracteres.');return;}
  var wc=(window.LiderCRM||{}).api&&(window.LiderCRM.api.workerClient);
  if(!wc||typeof wc.adminResetPassword!=='function'){toast('API não disponível para reset.');return;}
  // Desabilita o botão durante a requisição para evitar duplo clique
  var btn=document.querySelector('#k-reset-section button');if(btn)btn.disabled=true;btn&&btn.classList.add('disabled');
  wc.adminResetPassword({targetUserId:uid,newPassword:pw}).then(function(res){
    if(input)input.value='';
    if(btn){btn.disabled=false;btn.classList.remove('disabled');}
    toast('✅ Senha redefinida! O usuário já pode fazer login.');
    // Atualiza a mensagem de credenciais com a nova senha para o ADM copiar/enviar
    showCred(uid,pw);
  }).catch(function(err){
    if(btn){btn.disabled=false;btn.classList.remove('disabled');}
    toast('Erro ao redefinir: '+((err&&err.message)||'falha'));
  });
}
...
function confirmDU(){
  ...
  deleteUserDoc(delId);
  /* LF-CHAT-PURGE-USER-20260819: ao excluir o usuário, remove também as
     conversas e mensagens dele do bate-papo (local + servidor) — antes a
     conta sumia, mas o papo e as mensagens continuavam aparecendo. */
  try{ if(typeof window._chatPurgeUserEverywhere==='function'){ window._chatPurgeUserEverywhere(delId); } }catch(_eChatPurge){ console.warn('[usuarios] limpeza do bate-papo falhou', _eChatPurge); }
