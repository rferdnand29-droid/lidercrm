...
function renderDash(){
  ...
  loadCli(S.userId,function(list){
    var tot=list.length;
    var metr=_getClosingReconciliationMetrics(list,{uids:[S.userId]});
    var fec=metr.fechamentos;
    var ag=metr.agendamentos;
    var tx=metr.taxa;
...
function setPer(p,btn){_per=p;document.querySelectorAll('.pb').forEach(function(b){b.classList.remove('on');});if(btn)btn.classList.add('on');var dr=document.getElementById('dr');if(dr)dr.style.display=p==='custom'?'flex':'none';loadCli(S.userId,function(l){drawAnal(l,'krow','funil','psvg','pleg');drawNegKPIs(S.userId);});}

function renderAnal(){loadCli(S.userId,function(l){drawAnal(l,'krow','funil','psvg','pleg');drawNegKPIs(S.userId);});}
...
/* "Taxa de Conversão (Lead → Negócio)" — numerador: Leads cuja etapa é
   'conv' (Convertido). Mesmo padrão de data dos Negócios —
   stageEnteredAt na frente, por ser a data em que ENTROU na etapa
   'conv', não a última edição qualquer do registro. */
...
function drawAnal(list,kid,fid,svid,lgid){
  ...
  if(kEl)kEl.innerHTML=[
    '<div class="kc k1" title="Leads criados no board de Leads, com data de criação dentro do período selecionado"><div class="kv">'+leadsAdd+'</div><div class="kl">Leads Adicionados</div></div>',
    '<div class="kc k2" title="Negócios que chegaram em AG Vídeo ou Presencial (ou etapa posterior), dentro do período selecionado"><div class="kv">'+ag+'</div><div class="kl">Leads Agendados</div></div>',
    '<div class="kc k3" title="Negócios que entraram na etapa Fechado dentro do período selecionado"><div class="kv">'+fec+'</div><div class="kl">Negócios Fechados</div></div>',
    '<div class="kc k4" title="Leads Convertidos ÷ Leads Adicionados, no período selecionado"><div class="kv">'+txConv+'%</div><div class="kl">Taxa Conversão</div></div>'
  ].join('');
...
function drawNegKPIs(uid,elId){
  ...
  // [FIX 20260821] CALIBRAÇÃO — pedido explícito do usuário.
  // "Negócios Fechados" saiu daqui (ficava duplicado com o card de
  // mesmo nome na 1ª linha, só que sem filtro de período — confuso
  // ter os dois na tela ao mesmo tempo); entrou "Taxa Vídeo/Loja →
  // Ficha". "No-Show/Desistência" ficou mais claro no tooltip
  // (cobre quem não veio pro AG Vídeo/Presencial E quem veio no
  // Vídeo/Loja mas não fechou — as duas coisas caem na mesma etapa
  // 'noshow' no board de Negócios, não existe uma etapa separada só
  // pra "veio e desistiu"). Removida também a checagem de 'desist' —
  // não é uma etapa que existe de verdade no board (KB_NEG_COLS),
  // ficava ali sem nunca bater com nada.
  var noshow=arr.filter(function(c){return c.col==='noshow';}).length;
  ...
  var atendido=(typeof _countAtendidoNegocios==='function')?_countAtendidoNegocios([uid],null):0;
  var ficaOuAlem=(typeof _countFichaOuAlemNegocios==='function')?_countFichaOuAlemNegocios([uid],null):0;
  var txFicha=atendido>0?Math.round(ficaOuAlem/atendido*100):0;
  el.innerHTML=[
    '<div class="kc k2" title="Negócios que ainda não fecharam nem foram marcados como No-Show/Desistência"><div class="kv">'+emAndamento+'</div><div class="kl">Negócios Ativos</div></div>',
    '<div class="kc k1" title="Soma do valor de todos os Negócios fechados"><div class="kv" style="font-size:1.1rem">'+fmtBRL(valorTotal)+'</div><div class="kl">Valor Fechado</div></div>',
    '<div class="kc k4" title="Negócios que não vieram no AG Vídeo/Presencial, ou que vieram no Vídeo/Loja e não fecharam"><div class="kv">'+noshow+'</div><div class="kl">No-Show/Desistência</div></div>',
    '<div class="kc k3" title="Negócios que chegaram em Ficha (ou etapa posterior) ÷ Negócios que chegaram em Vídeo/Loja (ou etapa posterior)"><div class="kv">'+txFicha+'%</div><div class="kl">Taxa Vídeo/Loja → Ficha</div></div>'
  ].join('');
}
