...
function _countClosedNegocios(uids,range){
  ...
  return (uids||[]).reduce(function(sum,uid){
    ...
    return sum+arr.filter(function(c){
      ...
      if(!range)return true;
      var ref=c.closedAt||c.updatedAt||c.createdAt||c.data||null;
...
function _getClosingReconciliationMetrics(list,opts){
  ...
  steps[6]=fec;
  return {steps:steps,agendamentos:ag,fechamentosCli:fecCli,fechamentosKanban:fecKB,fechamentos:fec,taxa:ag>0?Math.round(fec/ag*100):0};
}
