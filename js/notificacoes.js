...
function _runAutomationEngineBoot(){
  ...
  /* CORREÇÃO 2026-08-06 (pedido do usuário): a regra "3 dias parado ->
     Lead Livre" (KB_STALE_TO_LIVRE_DAYS, js/kanban.js) já existe e já
     reseta o contador ao trocar de etapa — é exatamente a regra
     descrita. O problema real era só de GATILHO: ela só era checada
     quando alguém abria a aba Leads (_syncKBRemoteBG -> renderKB), então
     se ninguém abrisse por um tempo, leads ficavam parados além dos 3
     dias até alguém entrar na aba de novo. Reaproveitando este mesmo
     timer de 5 min (que já existe pra outra coisa), a checagem passa a
     rodar de forma confiável enquanto o app está aberto, sem depender
     de ninguém navegar pra Leads especificamente. */
  try{
    if(typeof _autoMoveStaleToLivre==='function'&&typeof getKBFor==='function'){
      _autoMoveStaleToLivre('leads',getKBFor('leads',S.userId),S.userId);
      var _pgLeads=document.getElementById('pg-leads');
      if(_pgLeads&&_pgLeads.classList.contains('on')&&typeof renderKBLocal==='function')renderKBLocal('leads');
    }
  }catch(_e){}
}
...
/* 2026-08-07 (pedido do usuário): "quero que notifique com o app fechado
   as movimentações de lead pra mim (quando alguém adiciona/transfere um
   lead pra mim), com o mesmo som já existente [do chat]". pushNotif()
   acima já cuidava do aviso DENTRO do app (sino) — nunca disparava a
   notificação de verdade (fora do app). Esta função faz a segunda
   parte, reaproveitando o MESMO endpoint /api/v1/push/send e o mesmo
   som já configurado no servidor pro chat (Fase 2) — não duplica nada
   novo do lado do Worker, só chama de mais lugares no cliente.
   Best-effort de propósito, igual _chatNotifyRecipients(): nunca lança
   erro nem bloqueia o fluxo que a chamou, mesmo sem push configurado. */
function sendRealPushNotif(toUid,title,body,data){
  if(!toUid||!S||!S.userId||toUid===S.userId)return;
  try{
...
window.sendRealPushNotif=sendRealPushNotif;
