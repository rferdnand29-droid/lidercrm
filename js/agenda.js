...
function saveLinkedActivityEdit(){
  ...
  _loadActivitiesForOwner(uid,function(list){
    ...
    act.overdueNotifiedAt=null; /* 2026-08-07: reagendar limpa o "já notificado", senão nunca mais avisa se atrasar de novo */
...
/* 2026-08-06 (pedido do usuário): "Concluir" direto de dentro do lead,
   ao lado de "Editar atividade" — ver linked-activities-view.js. Não
   reaproveita actItemClick() porque aquela função resolve o dono da
   atividade a partir de _actPanelLastList (a última lista renderizada
   no painel de Atividades) — que não existe/não está atualizada aqui
   dentro do card do lead. Aqui já sabemos o ownerUid certo de cara
   (veio de opts.ownerUid no HTML), então só falta a mesma confirmação
   não-bloqueante por toast (confirm() falha em iOS PWA standalone). */
function confirmLinkedActDone(actId,ownerUid){
  var t=document.getElementById('toast'),tm=document.getElementById('tmsg');
  if(!t||!tm)return;
  clearTimeout(t._tm);clearTimeout(t._confirmTm);
  tm.innerHTML='Marcar como concluída? <button id="toast-linked-act-done-btn" style="margin-left:8px;padding:2px 9px;border-radius:6px;border:none;background:var(--ok);color:#fff;font-size:.75rem;cursor:pointer;font-family:Outfit,sans-serif">Sim</button>';
  var btn=document.getElementById('toast-linked-act-done-btn');
  if(btn){
    btn.dataset.actId=actId;btn.dataset.ownerId=ownerUid||((S&&S.userId)||'');
    btn.addEventListener('click',function(){
      actConfirmDone(this.dataset.actId,this.dataset.ownerId);
    },{once:true});
  }
...
function updateActBadge(){
  ...
  /* CORREÇÃO 2026-08-07 (pedido do usuário): "aba agenda não deve mais
     notificar agendamentos atrasados com bolinha vermelha; leads e
     negócios, se houver atividade atrasada, bolinha vermelha surge".
     O sino ⏰ do topo (conceitualmente ligado à Agenda) contava TODAS
     as pendentes juntas — futuras e atrasadas. Agora ele só conta as
     que ainda NÃO venceram (unread && !done && não atrasada); atrasada
     vira sinal visual nas abas Leads/Negócios em vez disso. */
  var isLate=function(a){ return !!(a && !a.done && a.scheduledAt && a.scheduledAt<nowIso); };
  var unread=acts.filter(function(a){return !a.read&&!a.done&&!isLate(a);}).length;
  var badge=document.getElementById('act-badge'),bell=document.getElementById('act-bell');
  if(badge&&bell){
  ...
  try{ _updateBoardLateDots(acts.filter(isLate)); }catch(_e){}
}

/* 2026-08-07: bolinha vermelha nas abas Leads/Negócios (desktop: botões
   .nt gerados em buildNav(); mobile: item combinado "CRM" no rodapé) —
   acende quando existe pelo menos uma atividade atrasada vinculada
   àquele board especificamente. */
function _updateBoardLateDots(lateActs){
  var hasLeadsLate=(lateActs||[]).some(function(a){return a.board==='leads';});
  var hasNegLate=(lateActs||[]).some(function(a){return a.board==='negocios';});
  // Desktop: botões .nt sem id fixo — identifica pelo texto, igual goPage() já faz.
  document.querySelectorAll('#ntabs .nt').forEach(function(b){
    var txt=b.textContent.trim();
    var isLeadsBtn=(txt==='Leads');
    var isNegBtn=(txt.indexOf('gócio')>=0);
    if(!isLeadsBtn&&!isNegBtn)return;
    var on=isLeadsBtn?hasLeadsLate:hasNegLate;
    var dot=b.querySelector('.nav-late-dot');
    if(on&&!dot){
      dot=document.createElement('span');
      dot.className='nav-late-dot';
      dot.setAttribute('aria-label','Atividade atrasada');
      b.appendChild(dot);
    }else if(!on&&dot){
      dot.remove();
    }
  });
  // Mobile: item combinado "CRM" — acende se QUALQUER um dos dois tiver atraso.
  var mobileCrm=document.querySelector('.mbn-item[data-page="crm"]');
  if(mobileCrm){
    var mDot=mobileCrm.querySelector('.nav-late-dot');
    var mOn=hasLeadsLate||hasNegLate;
    if(mOn&&!mDot){
      mDot=document.createElement('span');
      mDot.className='nav-late-dot mbn-late-dot';
      mDot.setAttribute('aria-label','Atividade atrasada');
      mobileCrm.appendChild(mDot);
    }else if(!mOn&&mDot){
      mDot.remove();
    }
  }
  // Sub-abas Leads/Negócios dentro da tela CRM (.ctt-btn, mobile e desktop)
  document.querySelectorAll('.ctt-btn[data-board]').forEach(function(b){
    var board=b.getAttribute('data-board');
    var on=board==='leads'?hasLeadsLate:(board==='negocios'?hasNegLate:false);
    var dot=b.querySelector('.nav-late-dot');
    if(on&&!dot){
      dot=document.createElement('span');
      dot.className='nav-late-dot';
...
function applyActBulkDate(){
  ...
  var acts=getActivities();_actBulkSel.forEach(function(id){var a=acts.find(function(x){return x.id===id;});if(a){a.scheduledAt=dt;a.read=false;a.overdueNotifiedAt=null;}});
...
function buildLigGrid(){
  ...
  if(cnt===10){
    g.querySelectorAll('.lig-cell').forEach(function(c){c.classList.add('bingo');});
    setTimeout(function(){toast('🎉 BINGO! 10 ligações feitas e contabilizadas!');},300);
    /* CORREÇÃO 2026-08-07 (pedido do usuário): ao completar as 10,
       resetar sozinho em vez de ficar travado em 10/10 esperando
       clique manual em "Reiniciar". O atraso (2.2s) deixa o destaque
       visual das 10 células + o toast serem vistos antes de sumir —
       sem esse tempo, o "bingo" passaria despercebido. */
    setTimeout(function(){resetLig(true);},2200);
  }
}
...
function resetLig(silent){_ligMarked=[];saveLigToday([]);buildLigGrid();if(!silent)toast('Contador reiniciado');}
