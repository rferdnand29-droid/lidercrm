...
function renderObjBank(){
  ...
  function card(o){
    ...
    var actions='<div class="obj-card-actions">'
      +'<button class="obj-act-btn copy" onclick="event.stopPropagation();copyObjecaoBanco(\''+_jsSq(o.id)+'\')" title="Copiar">📋</button>'
      +(isAdm?'<button class="obj-act-btn edit" onclick="event.stopPropagation();admOpenEditObjecao(\''+_jsSq(o.id)+'\')" title="Editar">✏️</button><button class="obj-act-btn del" onclick="event.stopPropagation();admDeleteBancoObjecao(\''+_jsSq(o.id)+'\')" title="Excluir">✕</button>':'')
      +'</div>';
    return '<div class="obj-card"><div class="obj-card-hd"><div class="obj-card-q">'+escapeHtml(o.objecao)+(isCustom?'<span style="font-size:.6rem;color:var(--ok);margin-left:6px;background:rgba(27,138,94,.12);padding:1px 5px;border-radius:10px">✦ ADM</span>':'')+'</div><span class="canal-badge '+_htmlAttr(o.canal)+'">'+canalLbl[o.canal]+'</span>'+actions+'</div>'
      +'<div class="obj-cat">'+escapeHtml(o.categoria)+'</div>'
      ...
      +['iniciante','intermediario','experiente'].map(function(lv){
        ...
        return '<div class="obj-level"><div class="obj-level-lbl '+lv+'">'+lbl+'</div><div class="obj-level-txt">'+escapeHtml((o.respostas&&o.respostas[lv])||'')+'</div></div>';
      }).join('')
      +'</div></div>';
  }
  if(_objBankCanal){
