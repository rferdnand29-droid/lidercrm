...
(function () {
  ...
  function _openMedia(src, type, name) {
    ...
    if (src && !src.startsWith('data:')) {
      ...
      btnNova.addEventListener('click', function (e) {
        e.stopPropagation();
        // FIX (2026-08-02): window.open() direto não abre nada dentro do
        // WebView Android do Capacitor (a URL é descartada silenciosamente
        // — ver comentário completo em js/whatsapp.js, função
        // _lfOpenExternal, que já resolve isso pra outros lugares do CRM).
        if (typeof window._lfOpenExternal === 'function') window._lfOpenExternal(src);
  ...
  function _enhanceFiles(root) {
    root.querySelectorAll('a.chat-msg-file:not([data-lf-ok])').forEach(function (link) {
      link.dataset.lfOk = '1';
      var href = link.href || '';
      var rawName = (link.textContent || '').replace(/^[📎▶📄]\s*/u, '').replace(/\s*[↗👁]$/u, '').trim();
  ...
  document.addEventListener('contextmenu', function (e) {
    ...
    if (navEl && !navEl.closest('#lf-ctx') && !navEl.closest('#kb-ctx')) {
      ...
      if (page) {
        ...
        _showCtx(e.clientX, e.clientY, [
          ...
          '-',
          { icon: '🗗', label: 'Abrir em nova janela', fn: function () { _doOpenNewTab(page); } }
        ]);
        return;
      }
    }
    ...
    if (mbnEl) {
      ...
      if (dp && dp !== 'menu') {
        ...
        _showCtx(e.clientX, e.clientY, [
          ...
          '-',
          { icon: '🗗', label: 'Abrir em nova janela', fn: function () { _doOpenNewTab(page2); } }
        ]);
        return;
      }
    }
    ...
    if (imgEl) {
      ...
      _showCtx(e.clientX, e.clientY, [
        ...
        { icon: '🗗', label: 'Abrir em nova guia',  fn: function () {
          // FIX (2026-08-02): ver comentário completo no botão do overlay
          // acima — window.open() direto falha silenciosamente no
          // WebView Android do Capacitor.
          if (typeof window._lfOpenExternal === 'function') window._lfOpenExternal(imgSrc);
    ...
    if (fileLink) {
      ...
      var fname = (fileLink.textContent || '').replace(/^[📎▶📄]\s*/u, '').replace(/\s*[↗👁]$/u, '').trim();
      ...
      items2.push({ icon: '🗗', label: 'Abrir em nova guia', fn: function () {
        // FIX (2026-08-02): mesmo motivo dos outros 2 pontos deste arquivo.
        if (typeof window._lfOpenExternal === 'function') window._lfOpenExternal(fhref);
        else window.open(fhref, '_blank');
      } });
    ...
    if (kbCard) {
      ...
      if (kbBoard) {
        ...
        setTimeout(function () {
          ...
          btn.innerHTML = '🗗 Abrir aba em nova janela';
