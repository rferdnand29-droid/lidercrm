 ...
 *   - timeout de segurança de 15s (roda assim mesmo — o fetch-wrapper do
 *     lf-fix-raiz-token-quota injeta o Bearer se houver).
...
(function(global){
  ...
  global.LF_WHEN_WORKER_AUTH=function(fn){
    ...
    var to=setTimeout(function(){
      try{console.debug('[lf-when-worker-auth] timeout de '+TIMEOUT_MS+'ms — executando sem confirmação de token');}catch(_e){}
      run();
    },TIMEOUT_MS);
