...
(function(global){
  ...
  /* ── 2. Transparência de wallpaper (fonte ativa do bundle limpo)
     Mantemos nesta v1 os stubs/overrides necessários para não depender de
     um patch paralelo não carregado pelo HTML. O comportamento continua
     idempotente e com fallback mínimo (alpha 0.36/0.24, blur 2/1). */
  function _lfApplyWallpaperTransparency(hasWallpaper){
    if(global.__LF_WALLPAPER_TRANSP_V2__ && global._lfApplyWallpaperTransparency){
      global._lfApplyWallpaperTransparency(hasWallpaper);
      return;
    }
    ...
    if(!hasWallpaper){st.textContent='';document.documentElement.classList.remove('lf-has-wallpaper','lf-theme-dark','lf-theme-light');return;}
    var isDark=_lfIsThemeDark();
    var alpha=isDark?0.36:0.24;var bgRgb=isDark?'0, 0, 0':'255, 255, 255';var blur=isDark?2:1;
    st.textContent=
  ...
  /* ── 3. STUB DE DELEGAÇÃO _lfClearWallpaperTransparency ──────────── */
  function _lfClearWallpaperTransparency(){
    if(global.__LF_WALLPAPER_TRANSP_V2__ && global._lfClearWallpaperTransparency){
      global._lfClearWallpaperTransparency();
      return;
    }
    var st=document.getElementById('lf-wallpaper-transp-el');
