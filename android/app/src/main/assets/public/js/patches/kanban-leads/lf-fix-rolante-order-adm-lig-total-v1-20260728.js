...
(function(global){
  'use strict';
  if (global.__lfFixRolanteOrderAdmLigTotalV1) return;
  global.__lfFixRolanteOrderAdmLigTotalV1 = true;
  ...
  function _patchSortCardsForColumn(){
    ...
    global._sortCardsForColumn=function(cards){
      return (cards||[]).slice().sort(function(a,b){
        /* CORREÇÃO 2026-08-18 (Bug "tremelique" / etapas rolantes):
           A versão anterior usava (createdAt || updatedAt) como chave de
           desempate quando não havia manualOrder. Qualquer operação que
           bumpasse updatedAt (marcar bolinha, atividade, observação,
           mudança de etapa) movia o cartão de posição na coluna a cada
           re-render — visualmente o cartão "pula" e a coluna oscila
           (render → sort → save → render sem estabilizar). Além disso,
           cartões com createdAt idênticos (import em lote) sem
           manualOrder empatavam (return 0), reforçando reordenação
           instável.
           Nova prioridade:
           1) ambos com manualOrder → respeita a ordem manual;
           2) só um com manualOrder → o que NÃO tem (é novo, nunca foi
              movido) fica por CIMA — preserva "lead novo no topo";
           3) nenhum com manualOrder → createdAt DESC (mais recente
              primeiro). NUNCA usar updatedAt aqui — toda edição
              reordenaria o card (tremelique).
           4) tiebreak ESTÁVEL por id — elimina troca de posição em
              empate de timestamp. */
      ...
      if(lbl&&/Somat/i.test(lbl.textContent||'')){
        ...
        if(numEl){
          ...
          if(newTotal>oldVal){
            ...
            // META-CORRIGIDA (2026-08-18): horas acontecidas limitadas ao
            // expediente de 8h (8h→18h); meta = 80 ligações/dia por consultor
            // (10/hora × 8h). Antes: sem teto e numUsers*10.
            var _horasTrab=Math.min(8,isToday?Math.max(1,_horaAtual-8):8);
            var _media=(newTotal/_horasTrab).toFixed(isToday?1:0);
            var _metaDiaria=numUsers*80;
  ...
  function _patchRenderAdmLigacoes(){
    ...
    global.renderAdmLigacoes=function(){
      ...
      var _horaAtual=new Date().getHours();
      var _horaInicio=8;
      // META-CORRIGIDA (2026-08-18): expediente de 8h (8h→18h). Fora dele o
      // painel "trava" no limite do dia — sem teto, chegava a mostrar "70h".
      var _horasAcontecidas=Math.min(8,isToday?Math.max(0,_horaAtual-_horaInicio):8);
      ...
      // (10/hora × 8h). Antes: users.length*10.
