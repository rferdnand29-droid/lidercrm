/* lf-fix-loadcli-array-v1-20260819 */
if(typeof window!=='undefined' && !window.__lfSafeArr){
  window.__lfSafeArr=function(x){return Array.isArray(x)?x:[];};
}
/* =====================================================================
 * auth.js
...
function getCargoCaps(uid){
  try{
    ...
    // FIX-ADM-JWT-VENCE-CACHE (2026-08-18): pro PRÓPRIO usuário logado, os
    // claims do /login|/refresh guardados em S (S.role/S.cargoCodigo/S.admExtra)
    // são AUTORITATIVOS e vencem o cache local `lf6_u`. Sintoma original:
    // mesma conta abria o Painel ADM em um PC (cache já atualizado) e não
    // abria no outro (cache antigo travava o cargo sem privilégio), mesmo
    // com o Worker devolvendo o cargo certo no login. Para usuários OUTROS
    // que não o logado, seguimos com o caminho antigo baseado em `u.*`.
    if(S && S.userId===uid){
      if(S.role==='adm') return CARGO_CAPS.master;
      var sCodeSelf=_lfCoerceCargoCode(S.cargoCodigo)||_lfNormalizeCargoCode(S.cargo);
      if(!sCodeSelf) sCodeSelf=_lfNormalizeCargoCode(u.cargo); // fallback pra JWT antigo sem cargoCodigo
      var baseSelf=(sCodeSelf && CARGO_CAPS[sCodeSelf]) ? CARGO_CAPS[sCodeSelf] : CARGO_CAPS_DEFAULT;
      var admExtraSelf=(typeof S.admExtra==='boolean') ? S.admExtra
                       : (typeof u.admExtra==='boolean') ? u.admExtra : false;
      if(admExtraSelf && !baseSelf.adminUI){
        return { escopo:baseSelf.escopo, leads:baseSelf.leads, negocios:baseSelf.negocios,
                 foreign:baseSelf.foreign, stageGated:baseSelf.stageGated,
                 adminUI:true, supervisorUI:true };
      }
      return baseSelf;
    }
...
function hasAdminAccess(uid){
  ...
  // FIX-ADM-JWT-VENCE-CACHE (2026-08-18): defesa em profundidade — pro PRÓPRIO
  // usuário logado, os claims do JWT (S.role/S.cargoCodigo/S.admExtra) vencem
  // qualquer resultado que dependa do cache `lf6_u`. Fazemos essa checagem
  // ANTES do resto pra não deixar o cache antigo de outro PC travar o acesso.
  if(S && S.userId===uid){
    if(S.role==='adm') return true;
    if(S.admExtra===true) return true;
    var sCodeSelfAdm=_lfCoerceCargoCode(S.cargoCodigo)||_lfNormalizeCargoCode(S.cargo);
    if(sCodeSelfAdm && CARGO_CAPS[sCodeSelfAdm] && CARGO_CAPS[sCodeSelfAdm].adminUI) return true;
  }
...
function toggleAdminNote(selId,noteId){
  // Textos amarelos de cargo removidos do HTML (pedido do usuário) — 'note'
  // pode não existir mais; o guarda exige só o <select>, senão o checkbox
  // 'Ativar acesso ao Painel ADM' pararia de aparecer.
  var sel=document.getElementById(selId),note=document.getElementById(noteId);if(!sel)return;
  ...
  var tid = 'eu-admin-toggle';
  if(selId==='k-cargo') tid = 'k-admin-toggle';
  if(selId==='nc') tid = 'nc-admin-toggle';
  var adminToggle=document.getElementById(tid);
  if(adminToggle)adminToggle.style.display=(isAdmin||isSupervisor)?'block':'none';
  // Textos amarelos de descrição de cargo removidos (pedido do usuário) —
  // a nota informativa não é mais exibida para nenhum cargo.
...
function loadCli(uid,cb){
  var _cb=function(v){try{return cb(window.__lfSafeArr(v));}catch(e){return cb([]);}};
  var localList=window.__lfSafeArr(getCliLocal(uid));
  _cb(localList);
  ...
  function applyServerList(server){
    server=window.__lfSafeArr(server);
    var merged=_mergeKeepLocalOnly(server,window.__lfSafeArr(getCliLocal(uid)));
    ss(ck(uid),merged);
    if(merged.length!==server.length)saveCli(uid,merged);
    // [FIX 20260828] Antes comparava JSON.stringify(array) direto — sensível
    // à ORDEM, e o merge reconstrói a lista na ordem do servidor (diferente
    // da ordem local acumulada), causando falso positivo de "mudou" e
    // re-render desnecessário (piscar/tremer) mesmo sem mudança real de
    // conteúdo. window.__LF_PERF_R4 nunca chegou a ser definido em lugar
    // nenhum do código — sempre caía no fallback problemático.
    if(!window._lfListsEqualById(window.__lfSafeArr(getCliLocal(uid)),merged))_cb(merged);
  }
  if(cfg&&cfg.useWorkerApi&&wc&&typeof wc.clientesList==='function'){
    wc.clientesList(uid).then(function(doc){applyServerList((doc&&doc.list)||[]);}).catch(function(e){console.warn("[auth] clientesList falhou",e);_cb([]);});
  }else if(DB_MODE==='firebase'&&db){
    db.collection('clientes').doc(uid).get().then(function(d){
      applyServerList(d.exists?(d.data().list||[]):[]);
    }).catch(function(e){console.warn("[auth] loadCli firebase falhou",e);_cb([]);});
  }
}
