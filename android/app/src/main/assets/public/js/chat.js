...
function renderChatList(){
  ...
  container.innerHTML = convs.map(function(c){
    ...
    return '<div class="chat-conv-item'+(c.id===_chatCurrentConv?' active':'')+'" data-conv-id="'+_jsSq(c.id)+'" onclick="openChatConv(\''+_jsSq(c.id)+'\')">'+
      '<div class="chat-conv-avatar" style="background:'+color+'">'+escapeHtml(avatar)+onlineDot+'</div>'+
...
function openChatConv(convId){
  ...
  if(headerEl){
    ...
    headerEl.innerHTML =
      ...
      '<div class="chat-conv-hd-name">'+escapeHtml(name)+'</div>'+
...
function renderChatMsgs(convId){
  ...
  function _renderOne(m){
    ...
    var senderName = '';
    if(!isMe && m.senderName) senderName = '<div class="chat-msg-sender">'+escapeHtml(m.senderName)+'</div>';
    ...
    if(m.attachmentName){
      ...
      if(isAudio && src){
        ...
        attachHTML = '<audio class="chat-msg-audio" controls preload="metadata" src="'+_htmlAttr(src)+'" data-audio-src="'+_htmlAttr(src)+'"></audio>';
      } else if(isImg && src){
        attachHTML = '<img class="chat-msg-img" src="'+_htmlAttr(src)+'" alt="'+escapeHtml(m.attachmentName)+'" loading="lazy">';
      } else if(src){
        attachHTML = '<a href="'+_htmlAttr(src)+'" target="_blank" rel="noopener" class="chat-msg-file">📎 '+escapeHtml(m.attachmentName)+' ↗</a>';
      } else {
        attachHTML = '<div class="chat-msg-file">📎 '+escapeHtml(m.attachmentName)+'</div>';
      }
    }
    // FIX #12: reações
    var reactionsHTML = '';
    if(m.reactions && Object.keys(m.reactions).length){
      ...
      reactionsHTML = '<div class="chat-msg-reactions">' +
        Object.keys(counts).map(function(e){ return '<span class="chat-msg-reaction" title="'+counts[e]+' reações">'+escapeHtml(e)+' '+counts[e]+'</span>'; }).join('') +
      '</div>';
    }
    ...
    if(m.replyToId && m.replyMeta){
      ...
      replyHTML =
        '<div class="chat-msg-reply" onclick="return chatJumpToMsg(event,\''+_jsSq(m.replyToId)+'\')">' +
          '<div class="chat-msg-reply-name">'+escapeHtml(m.replyMeta.fromName || 'Mensagem')+'</div>' +
          '<div class="chat-msg-reply-text">'+escapeHtml(_rPrev)+'</div>' +
        '</div>';
    }
    return '<div class="chat-msg'+(isMe?' me':' them')+(m.pinned?' pinned':'')+'" data-msg-id="'+_htmlAttr(m.id)+'">'+
      senderName+
      fwdTag+
      replyHTML+
...
function chatForwardMsg(msgId){
  ...
  host.innerHTML = '<div style="background:var(--bg2,#1a1e26);color:var(--tx,#eee);border-radius:12px;padding:16px;max-width:400px;width:100%;max-height:70vh;overflow:auto;font-family:Outfit,sans-serif">'
    ...
    + convs.map(function(c){
        ...
        return '<button class="chat-fwd-target" data-conv="'+_htmlAttr(c.id)+'" style="display:block;width:100%;text-align:left;padding:10px 12px;margin:4px 0;background:rgba(195,154,45,.06);border:1px solid rgba(195,154,45,.15);border-radius:8px;color:inherit;cursor:pointer;font-size:.85rem">'+(c.isGroup?'👥 ':'💬 ')+escapeHtml(nm)+'</button>';
      }).join('')
...
function chatShowConvInfo(){
  ...
  var infoHTML = '<div style="background:var(--bg2,#1a1e26);color:var(--tx,#eee);border-radius:12px;padding:20px;max-width:420px;width:100%;max-height:80vh;overflow:auto;font-family:Outfit,sans-serif">'
    + '<h3 style="margin:0 0 12px 0;font-size:1.05rem">'+escapeHtml(_chatOtherUserName(conv))+'</h3>';
  if(user){
    if(user.cargo)     infoHTML += '<div style="margin:6px 0;font-size:.85rem"><strong>Cargo:</strong> '+escapeHtml(user.cargo)+'</div>';
    if(user.telefone)  infoHTML += '<div style="margin:6px 0;font-size:.85rem"><strong>Telefone:</strong> <a href="tel:'+_htmlAttr(user.telefone)+'" style="color:var(--al,#c39a2d)">'+escapeHtml(user.telefone)+'</a></div>';
    if(user.email)     infoHTML += '<div style="margin:6px 0;font-size:.85rem"><strong>Email:</strong> '+escapeHtml(user.email)+'</div>';
    if(user.obs)       infoHTML += '<div style="margin:6px 0;font-size:.85rem"><strong>Observações:</strong> '+escapeHtml(user.obs)+'</div>';
  }
  if(conv.isGroup){
  ...
  if(medias.length){
    infoHTML += '<div class="chat-media-grid" style="display:grid;grid-template-columns:repeat(auto-fill,minmax(80px,1fr));gap:6px;margin-top:6px">'
      + medias.slice(-30).reverse().map(function(m){
          ...
          var mid  = _jsSq(m.id || '');
          var midAttr = _htmlAttr(m.id || '');
          var href = src ? ('href="'+_htmlAttr(src)+'" target="_blank" rel="noopener"') : '';
          var common = 'data-msg-id="'+midAttr+'" '+href+
                       ' onclick="return chatJumpToMsg(event,\''+mid+'\')"'+
                       ' title="Clique: ir para a mensagem • Botão direito / Ctrl+clique: abrir em nova aba"';
          if(isImg && src){
            return '<a '+common+' style="display:block"><img src="'+_htmlAttr(src)+'" style="width:100%;height:70px;object-fit:cover;border-radius:6px" alt=""></a>';
          }
          return '<a '+common+' style="display:flex;align-items:center;justify-content:center;background:rgba(195,154,45,.08);padding:8px;border-radius:6px;font-size:.65rem;text-align:center;color:inherit;text-decoration:none">📎 '+escapeHtml(m.attachmentName.slice(0,15))+'</a>';
        }).join('')
      + '</div>';
  } else {
...
function chatStartReply(msgId){
  ...
  if(box){
    ...
    box.innerHTML =
      '<div class="chat-reply-head">↩ Respondendo a '+escapeHtml(_chatReplyTo.fromName || 'mensagem')+'</div>' +
      '<div class="chat-reply-body">'+escapeHtml(_chatReplyTo.text || _chatReplyTo.attachmentName || '📎 Anexo')+'</div>' +
      '<button type="button" class="chat-reply-cancel" onclick="chatCancelReply()" aria-label="Cancelar resposta">✕</button>';
    // [FIX 20260821] posiciona o preview EXATAMENTE colado acima da barra
    // de digitar, medindo a altura real dela no momento (mais confiável
    // que um valor fixo "chutado" — a barra não tem altura 100% fixa,
    // varia com a área segura do aparelho).
    try{
      var inputArea=document.getElementById('chat-input-area');
      if(inputArea)box.style.bottom=inputArea.getBoundingClientRect().height+'px';
    }catch(_e){}
  }
  // [FIX 20260821] avisa o CSS que o preview de resposta está visível —
  // ver nota em css/chat/chat.css (#chat-reply-preview). Sem isso, o
  // espaço reservado pras mensagens (#chat-msgs) não cresce quando esse
  // preview aparece, e a última mensagem visível fica coberta por ele.
  if(document.body)document.body.classList.add('lf-chat-reply-active');
...
function chatCancelReply(){
  ...
  if(document.body)document.body.classList.remove('lf-chat-reply-active');
}
...
function _chatUploadBinaryViaWorker(name, dataUrl, meta, conv){
  ...
  /* LF-API-UNIFICADA: upload binário agora passa pelo cliente de API
     unificado (auth Bearer automática, retry de rede/transitório, base
     nativa do Capacitor) em vez de fetch cru com token montado na mão. */
  var _api = (window.LiderCRM && window.LiderCRM.api) || {};
  var _doUpload = (_api.authedRequest || window.lfApiRequest).bind(_api);
  _doUpload('/api/v1/upload/binary', {
    method: 'POST',
    retries: 2,
    timeout: 60000,
    headers: {
  ...
  .then(function(r){
    if(!r.ok){
      throw new Error('Worker upload '+r.status+': '+(r.message || (typeof r.data==='string'?r.data:'') || 'falha'));
    }
    var json = r.data;
...
function _chatSyncMobileLayout(forceBottom){
  ...
  // [FIX 20260821] Antes: page.style.height=liveH+'px' (sem !important).
  // Essa conta é medida ao vivo (topo real do elemento, altura real da
  // bottom-nav) — mais precisa que a fórmula em CSS baseada só na
  // variável --vvh (lf-consolidated-mobile.css). Só que a regra CSS
  // usa !important, e um estilo inline SEM !important nunca vence um
  // !important de folha de estilos — então esse cálculo, apesar de
  // todo o histórico de ajuste registrado no comentário acima, nunca
  // chegava a valer de verdade: a página sempre ficava com a altura
  // calculada em CSS, não com esta. Usar setProperty com 'important'
  // deixa este cálculo (mais confiável) vencer de verdade, assim que
  // rodar — a regra CSS continua sendo usada como valor inicial, antes
  // do JS ter a chance de medir e ajustar.
  page.style.setProperty('height',liveH+'px','important');
  page.style.setProperty('min-height',liveH+'px','important');
...
function _chatNotifyRecipients(msg, conv){
  ...
  try{
    /* LF-API-UNIFICADA: push/send via cliente unificado (auth automática,
       retry transitório) — best-effort, silencioso como antes. */
    var _api2 = (window.LiderCRM && window.LiderCRM.api) || {};
    var _req = _api2.authedRequest || window.lfApiRequest;
    if(_req){
      _req('/api/v1/push/send',{
        method:'POST',
        retries: 1,
        body:{
    ...
    listHTML =
      ...
      + users.map(function(u){
          ...
          return '<label class="chat-new-item" style="cursor:pointer">'
            ...
            +'<div class="chat-new-avatar" style="background:'+color+'">'+escapeHtml(av)+'</div>'
            +'<div class="chat-new-info"><div class="chat-new-name">'+escapeHtml(nome)+'</div><div class="chat-new-role">'+escapeHtml(u.cargo||'Consultor')+'</div></div>'
          +'</label>';
        }).join('')
      +'</div>';
  } else {
...
function _chatRenderAddMemberList(){
  ...
  modalBody.innerHTML = '<h2>➕ Adicionar participante</h2>'
    + '<div style="font-size:.78rem;color:var(--mu);margin-bottom:12px">Grupo: <b>'+escapeHtml(gname)+'</b> · Selecione um usuário para adicionar ('+users.length+' disponíveis):</div>'
    + listHTML
...
function chatDeleteConv(convId){
  ...
  function _persistDeleteRemote(){
    if(S && S.userId && typeof _chatRemoveInboxEntryForUsers==='function'){
      _chatRemoveInboxEntryForUsers(convId, [S.userId]).catch(function(){});
    }
  }
...
/* União por id entre a lista do servidor e a local: começa do servidor
   (preserva o que outros aparelhos escreveram) e sobrepõe/acrescenta o
   local (mensagens novas locais entram; metadados locais como read/pinned/
   edited prevalecem). Uma mensagem apagada "só pra mim" (removida da lista
   LOCAL, mas ainda presente no servidor) NÃO desaparece do resultado —
   ela vem do servidor e é reenviada como estava, então continua existindo
   para os outros participantes. Sem isso, apagar uma mensagem aqui também
   apagava ela do papo da outra pessoa (o documento é compartilhado e
   _chatSyncMsg sobrescrevia ele inteiro com a lista local, já sem a msg). */
function _chatMergeMsgsForSync(serverMsgs,localMsgs){
  var map={},order=[];
  (Array.isArray(serverMsgs)?serverMsgs:[]).forEach(function(m){
    if(m&&m.id){map[m.id]=m;order.push(m.id);}
  });
  (Array.isArray(localMsgs)?localMsgs:[]).forEach(function(m){
    if(!(m&&m.id))return;
    if(!map[m.id])order.push(m.id);
    map[m.id]=m;
  });
  return order.map(function(id){return map[id];}).filter(Boolean).sort(function(a,b){
    return (Date.parse(a&&a.ts)||0)-(Date.parse(b&&b.ts)||0);
  });
}
...
function _chatSyncMsg(msg){
  ...
  try{
    ...
    if(conv)conv = _chatNormalizeConv(conv);
    function _buildPayload(msgsForConv){
      var payload = {
        id: msg.convId,
    ...
    var localMsgs=_chatGetMsgs(msg.convId);
    if(root && root.config && root.config.useWorkerApi && wc && typeof wc.putConfig === 'function'){
      var _getFn=(typeof wc.getConfig==='function')?wc.getConfig('chat_conv_'+msg.convId).catch(function(){return null;}):Promise.resolve(null);
      _getFn.then(function(serverDoc){
        var serverMsgs=(serverDoc&&Array.isArray(serverDoc.msgs))?serverDoc.msgs:[];
        var merged=serverMsgs.length?_chatMergeMsgsForSync(serverMsgs,localMsgs):localMsgs;
        return wc.putConfig('chat_conv_'+msg.convId,_buildPayload(merged));
      }).catch(function(e){ console.warn('[chat] sync falhou', e); });
      if(conv) _chatSyncConvIndex(conv, msg).catch(function(){});
    } else if(DB_MODE === 'firebase' && db){
      var _docRef=db.collection('config').doc('chat_conv_' + msg.convId);
      _docRef.get().then(function(snap){
        var serverDoc=(snap&&snap.exists)?snap.data():null;
        var serverMsgs=(serverDoc&&Array.isArray(serverDoc.msgs))?serverDoc.msgs:[];
        var merged=serverMsgs.length?_chatMergeMsgsForSync(serverMsgs,localMsgs):localMsgs;
        return _docRef.set(_buildPayload(merged));
      }).catch(function(e){ console.warn('[chat] firebase sync falhou', e); });
      if(conv) _chatSyncConvIndex(conv, msg).catch(function(){});
    }
    ...
    if(root && root.config && root.config.useWorkerApi && wc && typeof wc.getConfig === 'function'){
      _chatPullInboxConvs().then(function(){
        ...
        return wc.getConfig('chat_conv_' + c.id).then(function(doc){
          ...
          var incoming = (doc.msgs || []).filter(function(m){ return m && m.id && !known.has(m.id) && !_chatIsMsgDeleted(m.id); });
          ...
          try{
            if(payload && payload.new && payload.new.data){
              ...
              if(doc && doc.msgs && Array.isArray(doc.msgs)){
                ...
                var known     = new Set((localMsgs||[]).map(function(m){ return m&&m.id; }));
                /* 2026-08-06: mesma proteção do poll — ver comentário lá. */
                var incoming  = doc.msgs.filter(function(m){ return m&&m.id&&!known.has(m.id)&&!_chatIsMsgDeleted(m.id); });
                if(incoming.length){
...
function initChatPage(){
  ...
  if(typeof loadUsersDB === 'function'){
    try{
      // [FIX 20260820] renderChatList() reconstrói a lista inteira do zero
      // (innerHTML=...map(...)), recriando TODOS os <img> de avatar mesmo
      // quando o dado não mudou nada. Antes, esta chamada assíncrona
      // sempre re-renderizava incondicionalmente logo depois da síncrona
      // (linha abaixo) já ter rodado — dois renders em sequência, cada um
      // recriando os avatares do zero, é exatamente o "piscar" relatado ao
      // entrar no Papo. Agora só re-renderiza se a lista de usuários
      // realmente mudou entre as duas chamadas.
      var _chatUsersSnapBefore = (typeof getUsers==='function') ? JSON.stringify(getUsers()) : null;
      loadUsersDB(function(freshList){
        try{
          var _chatUsersSnapAfter = JSON.stringify(freshList || (typeof getUsers==='function' ? getUsers() : null));
          if(_chatUsersSnapAfter !== _chatUsersSnapBefore) renderChatList();
        }catch(_e){ renderChatList(); }
      });
...
function _chatRegisterPushDevice(){
  ...
  _chatGetPushToken(platform).then(function(tokenData){
    ...
    /* LF-API-UNIFICADA: push/register via cliente unificado (auth
       automática, retry transitório) — mesmo contrato de resposta. */
    var _api3 = (window.LiderCRM && window.LiderCRM.api) || {};
    return (_api3.authedRequest || window.lfApiRequest)('/api/v1/push/register', {
      method: 'POST',
      retries: 2,
      body: {
...
function _chatUnregisterPushDevice(){
  ...
  /* LF-API-UNIFICADA: unregister via cliente unificado (auth automática,
     retry transitório) — mesmo contrato de resposta. */
  var _api4 = (window.LiderCRM && window.LiderCRM.api) || {};
  (_api4.authedRequest || window.lfApiRequest)('/api/v1/push/register', {
    method: 'DELETE',
    retries: 1,
    body: { token: token },
  }).then(function(){
...
if(!window.__LF_CHAT_MOBILE_LAYOUT_BOUND__){
  ...
  if(window.visualViewport){
    window.visualViewport.addEventListener('resize',function(){setTimeout(function(){_chatSyncMobileLayout(true);},20);},{passive:true});
    window.visualViewport.addEventListener('scroll',function(){setTimeout(function(){_chatSyncMobileLayout(true);},20);},{passive:true});
  }
}
...
function _chatPushSelfTest(){
  ...
  /* LF-API-UNIFICADA: selftest via cliente unificado — authedJson mantém
     o contrato {status, json} que a UI abaixo já consome. */
  var _api5 = (window.LiderCRM && window.LiderCRM.api) || {};
  (_api5.authedJson || window.lfApiRequest)('/api/v1/push/selftest',{ method:'POST' })
  .then(function(r0){ return {status:r0.status, json:(r0.json!==undefined?r0.json:r0.data)}; })
  .then(function(r){
    var data=(r.json && r.json.data) || {};
    var steps=data.steps || [];
    var stepsHTML=steps.map(function(s, i){
      var icon = s.ok ? '✅' : '❌';
      var detail = s.detail!=null ? '<div style="font-size:.72rem;color:var(--mu);margin-top:2px;word-break:break-word">'+escapeHtml(typeof s.detail==='string'?s.detail:JSON.stringify(s.detail))+'</div>' : '';
      return '<div style="padding:8px 0;border-bottom:1px solid rgba(255,255,255,.06)">'
        +'<div style="font-size:.85rem">'+icon+' <b>Passo '+(i+1)+':</b> '+escapeHtml(s.step)+'</div>'
        +detail
      +'</div>';
    }).join('');
    ...
    host.querySelector('div').innerHTML = '<h3 style="margin:0 0 12px 0;font-size:1rem">🔔 Resultado do teste de push</h3>'
      + (r.status!==200 ? '<div style="color:var(--rl,#ef4444);font-size:.85rem;margin-bottom:10px">HTTP '+r.status+' — '+escapeHtml((r.json&&r.json.error&&r.json.error.message)||'erro desconhecido')+'</div>' : '')
      + stepsHTML + overall
    ...
    host.querySelector('div').innerHTML = '<h3 style="margin:0 0 12px 0;font-size:1rem">🔔 Resultado do teste de push</h3>'
    + '<div style="color:var(--rl,#ef4444);font-size:.85rem">Não consegui nem chamar o servidor: '+escapeHtml(String(err&&err.message||err))+'</div>'
      + '<button onclick="document.getElementById(\'chat-selftest-modal\').remove()" style="width:100%;margin-top:16px;padding:10px;background:rgba(195,154,45,.1);border:1px solid rgba(195,154,45,.25);color:inherit;border-radius:8px;cursor:pointer;font-size:.85rem">Fechar</button>';
  });
}
...
/* =====================================================================
 * LF-CHAT-PURGE-USER-20260819
 * Ao excluir um usuário, agora some TAMBÉM tudo o que era dele no Papo:
 * conversas 1:1 (somem do bate-papo de todos os participantes), as
 * mensagens dessas conversas, a participação dele em grupos e a caixa de
 * entrada remota (chat_inbox_<uid>). Antes, confirmDU() apagava só o
 * cadastro — conversa e mensagens continuavam no servidor e seguiam
 * aparecendo para os demais usuários.
 * ===================================================================== */
function _chatRemotePurgeDm(convId, parts, deletedUid){
  /* Lápide no doc da conversa + remoção da entrada na inbox de cada
     participante. Best-effort: sem worker/firebase, só resolve. */
  convId=String(convId||'').trim();
  if(!convId) return Promise.resolve();
  var root=window.LiderCRM;
  var wc=root&&root.api&&root.api.workerClient;
  var useWorker=!!(root&&root.config&&root.config.useWorkerApi&&wc&&typeof wc.getConfig==='function'&&typeof wc.putConfig==='function');
  var useFb=(typeof DB_MODE!=='undefined'&&DB_MODE==='firebase'&&(typeof db!=='undefined'&&db));
  if(!useWorker&&!useFb) return Promise.resolve();
  function _getCfg(name){
    if(useWorker) return wc.getConfig(name).catch(function(){return null;});
    return db.collection('config').doc(name).get().then(function(s){return s&&s.exists?(s.data()||{}):null;}).catch(function(){return null;});
  }
  function _putCfg(name,payload){
    if(useWorker) return wc.putConfig(name,payload).catch(function(){});
    return db.collection('config').doc(name).set(payload).catch(function(){});
  }
  var now=new Date().toISOString();
  var jobs=[_putCfg('chat_conv_'+convId,{
    id:convId, deleted:true, msgs:[], participants:[], participantNames:{},
    updatedAt:now, deletedAt:now, deletedReason:'user-removed', deletedUid:deletedUid||''
  })];
  _chatNormalizeParticipants(parts||[]).forEach(function(p){
    jobs.push(_getCfg(_chatInboxDocName(p)).then(function(doc){
      var payload=doc&&typeof doc==='object'?doc:{};
      var list=Array.isArray(payload.list)?payload.list:[];
      var next=list.filter(function(item){ return !(item&&item.id===convId); });
      if(next.length===list.length&&doc==null) return null;
      payload.list=next; payload.ts=Date.now();
      return _putCfg(_chatInboxDocName(p), payload);
    }));
  });
  return Promise.all(jobs);
}

function _chatPurgeUserEverywhere(uid){
  uid=String(uid||'').trim();
  if(!uid) return Promise.resolve(false);
  var convs=_chatGetConvs();
  var affected=convs.filter(function(c){ return c && _chatParticipantsInclude(c.participants, uid); });
  var dmIds=[], dmParts={}, groupConvs=[];
  affected.forEach(function(c){
    if(c.isGroup){
      var parts=_chatNormalizeParticipants(c.participants).filter(function(p){return p!==uid;});
      var admins=_chatNormalizeParticipants(c.admins).filter(function(p){return p!==uid;});
      if(parts.length<=1){ dmIds.push(c.id); dmParts[c.id]=c.participants; }
      else{
        c.participants=parts; c.admins=admins;
        if(c.participantNames&&c.participantNames[uid])delete c.participantNames[uid];
        groupConvs.push(c);
      }
    }else{
      dmIds.push(c.id); dmParts[c.id]=c.participants;
    }
  });
  /* 1) Limpeza LOCAL imediata: lista de conversas + mensagens + tela aberta */
  _chatSaveConvs(convs.filter(function(c){ return c && dmIds.indexOf(c.id)<0; }));
  dmIds.forEach(function(id){ try{ localStorage.removeItem(CHAT_MSG_PREFIX+id); }catch(_e){} });
  if(_chatCurrentConv && dmIds.indexOf(_chatCurrentConv)>=0){
    _chatCurrentConv=null;
    try{ if(typeof chatBackToList==='function') chatBackToList(); }catch(_e){}
  }
  try{ if(typeof renderChatList==='function') renderChatList(); }catch(_e){}
  try{ if(typeof renderChatPage==='function') renderChatPage(); }catch(_e){}
  /* 2) Limpeza REMOTA */
  var jobs=[];
  dmIds.forEach(function(id){ jobs.push(_chatRemotePurgeDm(id, dmParts[id], uid)); });
  var root=window.LiderCRM;
  var wc=root&&root.api&&root.api.workerClient;
  var useWorker=!!(root&&root.config&&root.config.useWorkerApi&&wc&&typeof wc.getConfig==='function'&&typeof wc.putConfig==='function');
  var useFb=(typeof DB_MODE!=='undefined'&&DB_MODE==='firebase'&&(typeof db!=='undefined'&&db));
  if(useWorker||useFb){
    groupConvs.forEach(function(conv){
      var op=(useWorker?wc.getConfig('chat_conv_'+conv.id).catch(function(){return null;})
                        :db.collection('config').doc('chat_conv_'+conv.id).get().then(function(s){return s&&s.exists?(s.data()||{}):null;}).catch(function(){return null;}));
      jobs.push(op.then(function(prev){
        var payload=Object.assign({}, prev||{}, {
          id:conv.id, isGroup:true, name:conv.name||((prev||{}).name)||'',
          participants:conv.participants, admins:conv.admins,
          participantNames:conv.participantNames||{},
          updatedAt:new Date().toISOString()
        });
        return useWorker?wc.putConfig('chat_conv_'+conv.id,payload).catch(function(){})
                        :db.collection('config').doc('chat_conv_'+conv.id).set(payload).catch(function(){});
      }));
      /* tira o excluído da inbox dele para este grupo */
      jobs.push(_chatRemoveInboxEntryForUsers(conv.id,[uid]).catch(function(){}));
    });
    /* zera a caixa de entrada remota do usuário excluído */
    jobs.push((useWorker?wc.putConfig(_chatInboxDocName(uid),{list:[],ts:Date.now()}).catch(function(){})
                        :db.collection('config').doc(_chatInboxDocName(uid)).set({list:[],ts:Date.now()}).catch(function(){})));
  }
  return Promise.all(jobs).then(function(){ return true; });
}
window._chatPurgeUserEverywhere=_chatPurgeUserEverywhere;

/* Reação nos OUTROS dispositivos: quando a lista de usuários recarrega e
   alguém que existia antes sumiu (foi excluído em outro aparelho), purga
   localmente as conversas 1:1 com esse "fantasma" E já limpa o servidor,
   para a conversa não voltar no próximo pull da inbox. */
var _chatPrevUserIds=null;
window.addEventListener('crm:users-updated', function(){
  try{
    var users=(typeof getUsers==='function')?getUsers():[];
    if(!Array.isArray(users)||!users.length){ _chatPrevUserIds=null; return; }
    var cur={}; users.forEach(function(u){ if(u&&u.id) cur[String(u.id)]=true; });
    if(_chatPrevUserIds){
      Object.keys(_chatPrevUserIds).forEach(function(prevId){
        if(cur[prevId]) return;
        var convs=_chatGetConvs();
        var gone=convs.filter(function(c){ return c && !c.isGroup && _chatParticipantsInclude(c.participants, prevId); });
        if(!gone.length) return;
        var goneIds=gone.map(function(c){return c.id;});
        _chatSaveConvs(convs.filter(function(c){ return c && goneIds.indexOf(c.id)<0; }));
        gone.forEach(function(c){
          try{ localStorage.removeItem(CHAT_MSG_PREFIX+c.id); }catch(_e){}
          _chatRemotePurgeDm(c.id, c.participants, prevId);
        });
        if(_chatCurrentConv && goneIds.indexOf(_chatCurrentConv)>=0){
          _chatCurrentConv=null;
          try{ if(typeof chatBackToList==='function') chatBackToList(); }catch(_e){}
        }
        try{ if(typeof renderChatList==='function') renderChatList(); }catch(_e){}
