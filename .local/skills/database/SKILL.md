...
For Databricks Apps with Lakebase support, use this database normally during development. Replit exposes it through the same database tools and PostgreSQL environment variables as other projects. When the user publishes, Replit natively provisions or reuses a production Lakebase database in Databricks and binds it to the deployed app. Do not create a separate Lakebase instance.
