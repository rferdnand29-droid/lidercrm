---
name: testing
description: Run automated UI tests against your application using a Playwright-based testing subagent. Use it rarely. Each run drives a real browser for several minutes, so reserve it for significant changes to critical user journeys that cheaper checks cannot confirm; finishing a feature, an edit, or a plan step is not by itself a reason to run one.
---
...
A test run drives a real browser for several minutes while the user waits, and most changes never need one. Default to not launching a tester, and make the change earn it:
...
- **Don't launch a tester for anything you can check yourself**: a visual or copy tweak with your screenshot tool, a backend change with `curl`, a crash fix with the workflow logs. If you can verify it directly, do not engage a tester.
- **Do launch one only when the change is significant**: a user journey where a silent failure would be costly -- sign-in, payments, data the user could lose, a flow the user called out as critical -- materially changed by this work and beyond what a screenshot, a `curl`, or the logs can confirm. Finishing a feature does not clear that bar on its own, and most completed work does not clear it at all. When a change does, run a single pass over its changed flows; individual edits and plan steps never earn their own run.
...
1. **Test one flow per message, one tester per unit of work**: Keep each message focused on a single user journey, and cover the unit's other changed flows with follow-ups to the same tester -- do not launch a fresh tester or a separate full pass per flow
...
At most one tester can run at a time; reuse existing testers (with `sendFollowup`) when the earlier context is useful.
...
- A significant change to a critical user journey -- sign-in, payments, checkout, data the user could lose -- that cheaper checks cannot confirm
...
- Routine completed features that a screenshot, a `curl`, or the workflow logs can confirm -- finishing a feature is not by itself a reason to test
- Re-verifying flows that nothing has invalidated, re-testing after a fix you are confident in, or checking off individual edits and plan steps mid-task
