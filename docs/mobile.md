...
`app.html`, `app-lite.html`, `404.html`, `css/`, `js/`, `src/`,
...
- `allowNavigation` já libera `*.liderfinanceira.com`, `*.supabase.co`
  e `*.backblazeb2.com` — se um novo domínio de API/CDN for adicionado
  no futuro, precisa entrar nessa lista ou o WebView bloqueia a
  navegação silenciosamente.
...
## Diferença entre `index.html` e `app.html` em contexto mobile

**Confirmado (2026-08-02)**: o Capacitor carrega **`index.html`** — `webDir: "."` sem `server.url` customizado no `capacitor.config.json` faz o WebView servir a pasta como um servidor estático comum, que por convenção universal serve `index.html` pra `/`. Os dois têm o mesmo conjunto de patches funcionais desde a reorganização de 2026-08-01; a única diferença remanescente de propósito são os arquivos exclusivos da tela de login (vídeo de fundo animado) — só em `index.html`, ou seja, **o app nativo recebe o vídeo de login**, ao contrário do que a especulação anterior deste documento sugeria. Se o vídeo pesar demais em WebView nativo (bateria/memória), isso precisaria ser testado num device real — não dá pra confirmar performance de rendering de vídeo em WebView a partir deste ambiente.
