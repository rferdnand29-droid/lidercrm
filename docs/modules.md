...
Todos carregados por `index.html`; `app.html` é um espelho gerado
identicamente.
...
- `src/shared/utils/` — `namespace.js` (ativo, base do padrão `LiderCRM.utils.*`).
  Os helpers opcionais sem consumidores foram removidos; ver
...
```
Se não aparecer em `index.html`, o arquivo existe no disco mas **não está
conectado**. O único arquivo fora do entrypoint permitido pelo contrato é
`js/shared/data-worker.js`, instanciado dinamicamente por
`js/shared/popup-state.js`. `diagnostics/crash-reporter.js` é carregado no
boot. Veja também
`docs/load-order.md` antes de adicionar um novo `<script>`.
