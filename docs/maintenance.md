...
3. Adicionar a tag `<script>` em **`index.html` E `app.html`**, na
   mesma posição relativa (ordem de carregamento importa — ver
   `docs/architecture.md`). Verificar se `app-lite.html` também precisa
   (hoje ele só linka pros outros dois, não carrega scripts próprios).
...
- **`js/hash.js` órfão**: achado desta auditoria, ver
  `docs/ai-guide.md` § "Achados desta auditoria" — decisão de
  remover ou não é do time, não desta reorganização.
