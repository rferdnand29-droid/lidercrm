...
## 4. Offline / sync — **duas filas de retry paralelas e independentes**

Achado desta auditoria (não documentado antes): existem **dois sistemas
de retry-queue diferentes**, com chaves de `localStorage` diferentes,
que não se comunicam entre si:

| | `src/core/offline/retry-queue.js` | `src/modules/sync/runtime/retry-queue-sync.js` |
|---|---|---|
| Chave de storage | `lidercrm_retry_queue_v1` | `lf_retry_q_v1` |
...
Não são a mesma fila e **não devem ser assumidas como intercambiáveis**.
Antes de "consertar" um bug de sync, confirmar qual das duas está
envolvida (log de `localStorage.getItem('lidercrm_retry_queue_v1')` vs
`'lf_retry_q_v1')`). Ver `docs/ai-guide.md`.
