...
function _isRootAdm(raw){
  ...
  // LF-AUTHZ-ROOTADM-FIX-20260824: sub do ADM raiz pode ser namespaced
  // (ex.: 'adm_root_2026'). Sem isso o ADM caía no ramo de cargos altos sem
  // departamento e recebia CAPS_BASIC_CONSULTOR (escopo:'self'), gerando 403
  // em todo GET/PUT cross-owner de /api/v1/kanban/list.
  if(typeof raw.sub === 'string' && /^adm(_|$)/i.test(raw.sub)) return true;
  if(raw.adm_root === true) return true;
  return false;
}
