...
// CORS-FIX (2026-08-24): adicionado 'https://localhost' — origem do WebView
// no Capacitor 6/7 com androidScheme: "https". Sem ela, o preflight do login
// era negado no app Android (funcionava só no PC, mesma origem).
...
function isOriginAllowed(origin, cfg) {
  ...
  // AUDITORIA-FINAL-10 (2026-08-01, item 2.3): em produção (cfg.ENV==='production',
  // setado explicitamente em Cloudflare Pages > Environment variables), o
  // wildcard '*' passa a NEGAR em vez de permitir — força configurar
  // ALLOWED_ORIGINS de verdade. Sem essa env var setada, comportamento
  // idêntico ao anterior (permissivo) — rollout é opt-in, não automático.
  const raw = (cfg && cfg.ALLOWED_ORIGINS) || '*';
  if (raw === '*') {
    if (cfg && cfg.ENV === 'production') return false;
    return true;
  }
