...
/**
 * Igual a resolveTeamMemberIds, mas um nível acima: "departamento"
 * contém várias teams (ver sql/10-schema-departamentos.sql — a relação
 * é team.departamento_id, não 1:1 com team). Resolve:
 *   sub -> team_id -> teams.departamento_id -> todas as teams desse
 *   departamento -> todos os usuários dessas teams.
 * Fail-closed em cada passo: qualquer erro, ou usuário/time sem
 * departamento resolvido, retorna null — quem chama trata null como
 * "não achou departamento", nunca como "sem filtro".
 */
export async function resolveDepartmentMemberIds(cfg, sub) {
  if (!sub) return null;
  try {
    const me = await findUserByLegacyId(cfg, sub);
    const teamId = me && me.team_id;
    if (!teamId) return null;
    const { rows: myTeamRows } = await selectFrom(cfg, 'teams', {
      filters: { id: 'eq.' + teamId },
      select: 'id,departamento_id',
      limit: 1,
    });
    const deptId = myTeamRows && myTeamRows[0] && myTeamRows[0].departamento_id;
    if (!deptId) return null;
    const { rows: deptTeamRows } = await selectFrom(cfg, 'teams', {
      filters: { departamento_id: 'eq.' + deptId },
      select: 'id',
      limit: 500,
    });
    const teamIds = (deptTeamRows || []).map((t) => t.id).filter(Boolean);
    if (!teamIds.length) return null;
    const { rows: userRows } = await selectFrom(cfg, 'users', {
      filters: { team_id: 'in.(' + teamIds.join(',') + ')' },
      select: 'legacy_id',
      limit: 500,
    });
    const ids = (userRows || []).map((u) => u.legacy_id).filter(Boolean);
