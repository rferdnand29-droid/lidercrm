...
export async function verifyJwtHS256(token, secret) {
  const payload = await verifyJwtSignatureHS256(token, secret);
  const now = Math.floor(Date.now() / 1000);
  if (payload.exp && payload.exp < now) throw new Error('JWT_EXPIRED');
  return payload;
}

// [FIX 20260903] Verificação SEM checar `exp`. Usada exclusivamente pelo
// refresh com janela de tolerância (ver middlewares/auth.js): a assinatura
// continua sendo obrigatória — só a validade temporal é avaliada por quem
// chama, que impõe a própria janela de graça.
export async function verifyJwtSignatureHS256(token, secret) {
  ...
  return JSON.parse(base64UrlDecodeToString(bodyB64));
}
