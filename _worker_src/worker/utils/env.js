...
const DEFAULTS = Object.freeze({
  ...
  // Integração Evolution API: somente o Worker conhece a chave.
  // Vazio = recurso desativado; o frontend abre o WhatsApp normalmente.
  EVOLUTION_BASE_URL: '',
  EVOLUTION_API_KEY: '',
...
export function readEnv(env) {
  ...
  Object.keys(DEFAULTS).forEach((k) => {
    const v = env[k];
    provided[k] = v !== undefined && v !== null && v !== '';
  ...
  cfg._providedEnv = provided;
  cfg._productionConfigErrors = productionConfigErrors(cfg);

  return cfg;
}

/**
 * Produção não pode iniciar silenciosamente usando defaults de desenvolvimento.
 * O health check pode continuar público para explicar a configuração; o
 * api-handler usa esta lista para bloquear login e demais rotas com 503.
 */
export function productionConfigErrors(cfg) {
  if (!cfg || cfg.ENV !== 'production') return [];

  const provided = cfg._providedEnv || {};
  const errors = [];
  if (!provided.JWT_SECRET || cfg._jwtSecretIsDefault) {
    errors.push('JWT_SECRET');
  }
  if (!provided.ALLOWED_ORIGINS || cfg._corsIsWildcard) {
    errors.push('ALLOWED_ORIGINS');
  }
  if (!provided.SUPABASE_URL
      || !cfg.SUPABASE_URL
      || cfg.SUPABASE_URL === DEFAULTS.SUPABASE_URL
      || String(cfg.SUPABASE_URL).indexOf('REPLACE_ME') >= 0) {
    errors.push('SUPABASE_URL');
  }
  if (!provided.SUPABASE_ANON_KEY
      || !cfg.SUPABASE_ANON_KEY
      || String(cfg.SUPABASE_ANON_KEY).indexOf('REPLACE_ME') >= 0) {
