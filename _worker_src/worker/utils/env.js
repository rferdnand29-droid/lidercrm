...
const DEFAULTS = Object.freeze({
  ...
  // 2026-08-07 — checagem de atividades atrasadas (ver
  // controllers/cron-controller.js). Cloudflare Pages NÃO suporta Cron
  // Triggers diretamente (limitação confirmada da plataforma) — por
  // isso este endpoint é chamado por um Worker separado e pequeno,
  // configurado com Cron Trigger no painel do Cloudflare, que faz uma
  // requisição HTTP periódica pra cá. Sem usuário logado nesse
  // contexto (não é o cliente chamando), então a autenticação normal
  // por JWT não se aplica — este segredo compartilhado faz esse papel.
  // Default '' = endpoint recusa qualquer chamada até ser configurado.
  // Configurar via: wrangler secret put CRON_SECRET
  CRON_SECRET: '',
});
