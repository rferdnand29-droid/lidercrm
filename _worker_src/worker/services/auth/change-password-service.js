...
export async function adminResetPasswordService(cfg, sessionUser, params) {
  ...
  if (relUser) {
    try {
      await relUpdatePasswordHash(cfg, relUser.id, newPh);
      await relClearPasswordResetFlags(cfg, relUser.id);
    } catch (_e) { /* best-effort — fs_documents ainda é tentado abaixo */ }
  }
  ...
  return {
    ...
    passwordUpdatedAt: now,
    storage: relUser ? ('relational' + (fsUpdated ? '+fs' : '')) : 'fs_documents',
  };
}
