...
//   • fetchWithTimeout agora converte AbortError em UpstreamError(502)
//     em vez de vazar DOMException — antes, quando o Supabase demorava
//     >10s (cold start, DNS lento, projeto suspenso) o AbortController
//     abortava e o api-handler recebia um DOMException genérico, que
//     não é HttpError → virava 500 WORKER_ERROR no /api/v1/login.
//     Agora vira 502 UPSTREAM_ERROR com mensagem clara, e o
//     login-service ainda tem chance de fazer fallback.
//   • Erros de rede (TypeError: Failed to fetch, DNS, TLS) também são
//     convertidos em UpstreamError(502) — mesmo motivo.
// =====================================================================
...
function fetchWithTimeout(url, init) {
  ...
  return fetch(url, { ...init, signal: ctrl.signal })
    ...
    .catch(err => {
      clearTimeout(tid);
      // CORREÇÃO login-fix: normaliza qualquer falha de rede/abort para
      // UpstreamError, para que o api-handler devolva 502 (não 500).
      const name = err && err.name ? String(err.name) : '';
      const msg  = err && err.message ? String(err.message) : String(err);
      if (name === 'AbortError' || /aborted/i.test(msg)) {
        throw new UpstreamError('Timeout ao consultar Supabase (10s).', { cause: 'ABORT', url: String(url).split('?')[0] });
      }
      if (err instanceof UpstreamError || err instanceof UnauthorizedError
          || err instanceof BadRequestError || err instanceof NotFoundError) {
        throw err;
      }
      throw new UpstreamError('Falha de rede ao consultar Supabase: ' + msg, { cause: name || 'NETWORK', url: String(url).split('?')[0] });
    });
}
...
function buildHeaders(cfg, extra = {}) {
  ...
  if (cfg.SUPABASE_SERVICE_ROLE) {
    ...
    // Compat legado: quando ANON_KEY é um JWT (formato antigo do Supabase),
    // o PostgREST exige tanto `apikey` quanto `Authorization: Bearer <jwt>`.
...
async function ensureOk(response, contextLabel) {
  ...
  const message = (body && body.message) || (body && body.error) || (typeof body === 'string' ? body : ('Falha em ' + contextLabel));
...
// Chamada a uma função PostgreSQL. É usada para operações que precisam
// alterar mais de um documento como uma única transação (por exemplo, mover
// um card entre dois boards do Kanban).
export async function callRpc(cfg, functionName, payload = {}) {
  const safeName = String(functionName || '');
  if (!/^[a-z_][a-z0-9_]*$/i.test(safeName)) {
    throw new BadRequestError('Nome de função RPC inválido.');
  }
  const url = cfg.SUPABASE_URL.replace(/\/+$/, '') + '/rest/v1/rpc/' + encodeURIComponent(safeName);
  const headers = buildHeaders(cfg, { 'Prefer': 'return=representation' });
  const res = await fetchWithTimeout(url, {
    method: 'POST',
    headers,
  ...
  await ensureOk(res, 'rpc ' + safeName);
  return readBody(res);
}
