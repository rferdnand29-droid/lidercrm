...
// SEC-06 (2026-10-02): filtra os alvos pro mesmo departamento do
// remetente, a não ser que ele tenha escopo global (gerência) — sem
// isso, qualquer autenticado podia notificar QUALQUER outra pessoa da
// empresa, com conteúdo totalmente livre, sem nenhuma relação exigida
// entre remetente e destinatário (superfície de phishing/engenharia
// social). Best-effort: alvos fora do departamento são descartados,
// não vira erro pra requisição inteira — mesmo estilo já usado pro
// resto deste arquivo (device sem token, FCM fora do ar, etc.).
export async function scopeTargetsToDepartment(cfg, fromUserId, targets, caps) {
  if (caps && caps.escopo === 'global') {
    return { allowed: targets, blocked: [] };
  }
  const deptIds = await resolveDepartmentMemberIds(cfg, fromUserId);
  const deptSet = new Set(deptIds || []);
  const allowed = targets.filter((uid) => deptSet.has(uid));
  const blocked = targets.filter((uid) => !deptSet.has(uid));
...
export async function sendPushController(request, ctx) {
  ...
  const rawTargets = body.toUserIds
    .map((u) => sanitizeString(String(u || ''), 200))
    .filter((u) => u && u !== fromUserId); // nunca notifica quem mandou

  const { allowed: targets, blocked } = await scopeTargetsToDepartment(ctx.cfg, fromUserId, rawTargets, ctx.caps);
  ...
  // este endpoint).
  const results = blocked.map((uid) => ({ uid, ok: false, reason: 'OUT_OF_DEPARTMENT_SCOPE' }));
