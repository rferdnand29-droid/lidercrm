...
export async function uploadController(request, ctx) {
  ...
  // CORRIGIDO 2026-08-01: `attachments` usa nomes de coluna diferentes do
  // `record` acima (que continua igual — é o que a resposta da API/o
  // frontend espera). object_key/entity_type/entity_id não têm dado de
  // origem claro neste fluxo (upload avulso, sem lead/cliente associado
  // ainda) — ficam null; se precisar rastrear isso, precisa de contexto
  // adicional que esta rota hoje não recebe.
  const dbRecord = {
    file_name: filename, object_key: path, public_url: storage.publicUrl,
    content_type: contentType, size_bytes: decoded.bytes.length,
    uploaded_by: record.uploaded_by, created_at: record.created_at,
    entity_type: null, entity_id: null, provider: null, bucket: null,
  };
  try { await uploadsRepo.insert(ctx.cfg, dbRecord); } catch (_e) { /* opcional */ }
