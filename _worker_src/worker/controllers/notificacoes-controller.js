...
// LF-NOTIF-ROOTADM-ACCEPT-20260908: ADM raiz namespaced (adm_root_2026,
// adm_*) tem acesso à inbox de qualquer uid — mesmo predicado de
// _isRootAdm() em authz-cargo-only-dept-patch.js. Não amplia outros cargos.
function canAccessInbox(ctx, uid) {
  const me = ctx && ctx.user && ctx.user.sub;
  const role = ctx && ctx.user && ctx.user.role;
  const isRootAdm = role === 'adm' || /^adm(_|$)/i.test(String(me || ''))
                 || (ctx && ctx.user && ctx.user.raw && ctx.user.raw.adm_root === true);
  return !!uid && (isRootAdm || uid === me || role === 'gestor' || role === 'admin');
}
