...
import {
  getFsDocument, deleteFsDocument, listFsChildren,
...
export async function createAgendaSlot(request, ctx) {
  ...
  delete payload.id;
  delete payload._id;
  const saved = await saveVersionedDocument(ctx.cfg, slotPath(id), payload);
  return respondWithVersionedDocument(
    request,
    Object.assign({ _id: id }, payload),
    { endpoint: '/api/v1/agenda-slots' },
    saved.version,
    ctx.headers,
    201,
  );
...
export async function updateAgendaSlot(request, ctx) {
  ...
  delete merged.id;
  delete merged._id;
  const saved = await saveVersionedDocument(ctx.cfg, slotPath(id), merged, {
    version: expectedDocumentVersion(request, body),
  });
  return respondWithVersionedDocument(
    request,
    Object.assign({ _id: id }, merged),
    { endpoint: '/api/v1/agenda-slots', id },
    saved.version,
    ctx.headers,
  );
}
...
export async function deleteAgendaSlot(request, ctx) {
  ...
  const id = sanitizeString(url.searchParams.get('id'), 120);
  if (!id) throw new BadRequestError('id é obrigatório.');
  await deleteVersionedDocument(ctx.cfg, slotPath(id), {
