...
import { resolveTeamMemberIds } from '../utils/team-scope.js';
...
export async function getDashboard(request, ctx) {
  ...
  // (caps.adminUI) continua vendo a empresa toda — comportamento IDÊNTICO
  // ao de antes desta correção, nenhuma query nova pra esse grupo.
  // Supervisor/orientador (caps.supervisorUI, sem adminUI) passam a ver
  // só o próprio time (public.users.team_id) — filtro novo abaixo.
  // Consultor/funcionario/administrativo (nenhum dos dois): comportamento
  // também IDÊNTICO ao de antes — a intenção pra esse grupo não foi
  // confirmada, não mudei nada aqui pra não arriscar quebrar tela de
  // quem já depende do dashboard hoje.
  let ownerFilterIds = null; // null = sem filtro (mesmo comportamento de sempre)
  if (ctx.caps && ctx.caps.supervisorUI && !ctx.caps.adminUI) {
    const teamIds = ctx.user && ctx.user.sub ? await resolveTeamMemberIds(cfg, ctx.user.sub) : null;
    // Sem time resolvido (sem team_id cadastrado, ou erro): fail-closed
    // pro próprio uid só, nunca "sem filtro" = empresa toda.
    ownerFilterIds = teamIds && teamIds.length ? teamIds : (ctx.user && ctx.user.sub ? [ctx.user.sub] : []);
  }
  const ownerFilter = ownerFilterIds
    ? { owner_id: 'in.(' + ownerFilterIds.join(',') + ')' }
    : {};

  // ------- 1) Tenta o RELACIONAL primeiro -------
  const [relClients, relLeads, relBusiness] = await Promise.all([
    safeSelect(cfg, 'clients', { select: 'id,extra', limit: 2000, filters: ownerFilter }),
    safeSelect(cfg, 'leads',   { select: 'id,status,created_at', limit: 2000, filters: ownerFilter }),
    safeSelect(cfg, 'business',{ select: 'id,status,value', limit: 2000, filters: ownerFilter }),
  ]);
