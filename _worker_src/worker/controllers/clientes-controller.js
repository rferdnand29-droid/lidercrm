...
// [FIX 20260820] O cargo Administrativo tem negocios:'crud' (precisa, pra
// usar o board Negócios) e leads:'none'. O Bingo/clientes reusava a MESMA
// capacidade 'negocios' pra liberar mutação — como Administrativo já tem
// negocios:'crud' legitimamente, ele NUNCA era bloqueado por
// assertCanMutate, mesmo com a aba escondida no frontend
// (lf-administrativo-hide-tabs-v1-20260820.js). Ocultar a aba não impedia
// chamar a API direto (curl, devtools, app modificado). Esta função usa a
// MESMA assinatura de detecção já usada no frontend (leads:'none' +
// negocios:'crud') — mesmo critério dos dois lados, pra nunca divergir.
function assertNotAdministrativo(caps) {
  if (caps && caps.leads === 'none' && caps.negocios === 'crud') {
    throw new ForbiddenError('Bingo não está disponível para o cargo Administrativo.', {
      code: 'AUTHZ_FORBIDDEN',
      reason: 'administrativo_bingo_blocked',
      board: 'clientes',
    });
  }
}
...
export async function listClientes(request, ctx) {
  assertNotAdministrativo(ctx && ctx.caps);
...
export async function getClientesListDoc(request, ctx) {
  ...
  return respondWithVersionedDocument(
    request,
    doc || null,
    { endpoint: '/api/v1/clientes/list', uid },
...
export async function putClientesListDoc(request, ctx) {
  ...
  // Não aceitar que uma aba antiga substitua uma lista salva mais
  // recentemente em outro dispositivo. O cliente envia clientTs no início
  // de cada operação; chamadas legadas sem esse campo seguem funcionando.
  const incomingClientTs = Number(body.clientTs);
  const current = await getFsDocument(ctx.cfg, CLIENTES_LIST_PARENT + '/' + uid).catch(() => null);
  ...
  if (current
    ...
    return respondWithVersionedDocument(
      request,
      current,
      { endpoint: '/api/v1/clientes/list', uid, stale: true },
  ...
  const saved = await saveVersionedDocument(ctx.cfg, CLIENTES_LIST_PARENT + '/' + uid, payload, {
