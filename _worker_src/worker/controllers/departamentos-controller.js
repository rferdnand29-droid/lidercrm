...
/* MULTI-SUPERVISOR (2026-08-20): saneamento de listas de uids —
   strings não-vazias, dedup, limite de tamanho razoável. */
function sanitizeUidArray(input) {
  const arr = Array.isArray(input) ? input : [];
  const out = [];
  for (const raw of arr) {
    const v = sanitizeString(raw, 120);
...
export async function createDepartamento(request, ctx) {
  ...
  /* MULTI-SUPERVISOR (2026-08-20): aceita supervisorUids/adjuntoUids
     (arrays). O 1º de cada lista continua gravado nas colunas escalares
     legadas supervisor_uid/adjunto_uid (consumidores antigos / índices);
     a lista completa vai para as colunas JSONB supervisor_uids/
     adjunto_uids (ver sql/migrations/migration_dept_multi_supervisores_20260820.sql).
     O id pode vir do chamador (Estrutura gera 'dept_' + timestamp) pra
     manter o MESMO id no backend e no cache local. */
  const supIds = sanitizeUidArray(body && body.supervisorUids);
  const adjIds = sanitizeUidArray(body && body.adjuntoUids);
  const legacySup = sanitizeString(body && body.supervisorUid, 120);
  const legacyAdj = sanitizeString(body && body.adjuntoUid, 120);
  if (!supIds.length && legacySup) supIds.push(legacySup);
  if (!adjIds.length && legacyAdj) adjIds.push(legacyAdj);
  const callerId = sanitizeString(body && body.id, 120);

  const payload = {
    id: callerId || genDeptId(),
    nome,
    descricao: sanitizeString(body && body.descricao, 1000) || null,
    cor: sanitizeString(body && body.cor, 20) || '#3b82f6',
    status: 'ativo',
    supervisor_uid: supIds[0] || null,
    adjunto_uid: adjIds[0] || null,
    supervisor_uids: supIds,
    adjunto_uids: adjIds,
...
export async function updateDepartamento(request, ctx) {
  ...
  if (Array.isArray(body?.supervisorUids)) {
    const arr = sanitizeUidArray(body.supervisorUids);
    patch.supervisor_uids = arr;
    patch.supervisor_uid = arr[0] || null;
  } else if (typeof body?.supervisorUid === 'string') {
    const one = sanitizeString(body.supervisorUid, 120) || null;
    patch.supervisor_uid = one;
    patch.supervisor_uids = one ? [one] : [];
  }
  if (Array.isArray(body?.adjuntoUids)) {
    const arr = sanitizeUidArray(body.adjuntoUids);
    patch.adjunto_uids = arr;
    patch.adjunto_uid = arr[0] || null;
  } else if (typeof body?.adjuntoUid === 'string') {
    const one = sanitizeString(body.adjuntoUid, 120) || null;
    patch.adjunto_uid = one;
    patch.adjunto_uids = one ? [one] : [];
  }
