...
export async function claimLivreLead(request, ctx) {
  ...
  const board = 'leads';
  const originDoc = await getFsDocument(ctx.cfg, boardPath(board, fromUid));
  const originList = (originDoc && Array.isArray(originDoc.list)) ? originDoc.list : [];
  const idx = originList.findIndex((c) => c && String(c.id) === String(cardId));
  if (idx < 0) throw new BadRequestError('Lead não encontrado no board de origem.');
  const card = originList[idx];
  if (!card || card.col !== 'livre') {
    throw new ForbiddenError('Este Lead não está (mais) na etapa Livre.', {
      code: 'KANBAN_LIVRE_CLAIM_NOT_LIVRE',
    });
  }
  const claimed = Object.assign({}, card, { userId: sub, updatedAt: new Date().toISOString() });
  const newOriginList = originList.filter((c) => !(c && String(c.id) === String(cardId)));
  const destDoc = await getFsDocument(ctx.cfg, boardPath(board, sub));
  const destList = (destDoc && Array.isArray(destDoc.list)) ? destDoc.list : [];
  const destWithout = destList.filter((c) => !(c && String(c.id) === String(cardId)));
  destWithout.push(claimed);
  await setFsDocument(ctx.cfg, boardPath(board, sub), { list: destWithout, ts: Date.now() });
  await setFsDocument(ctx.cfg, boardPath(board, fromUid), { list: newOriginList, ts: Date.now() });
  return ok(claimed, { endpoint: '/api/v1/kanban/livre-claim', board }, ctx.headers);
}
