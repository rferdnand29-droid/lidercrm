...
/* [FIX 20260907-KANBAN409-RACE] Duas escritas concorrentes no MESMO
   documento (board,uid) liam a mesma versao e gravavam quase juntas:
   a segunda falhava no check otimista e virava 409
   DOCUMENT_VERSION_CONFLICT mesmo o cliente mandando a versao certa
   (era o loop infinito de PUT->409 no console: cada evento realtime/
   poll disparava novo PUT enquanto o anterior ainda estava em voo).
   Agora as escritas por documento sao SERIALIZADAS dentro do isolate
   do Worker (promise chain por chave): a segunda le a versao ja
   atualizada pela primeira e grava sem conflito. Requisicoes que
   chegariam juntas sao atendidas em sequencia, nenhuma vira 409.
   Idempotente: re-aplicar nao muda nada (o marcador e checado). */
const KANBAN_409_RACE_FIX = 'KANBAN_LIST_409_RACE_FIX_20260907';
const _kbWriteLocks = new Map();
function withKanbanWriteLock(key, fn) {
  const prev = _kbWriteLocks.get(key) || Promise.resolve();
  const next = prev.catch(function () {}).then(fn);
  _kbWriteLocks.set(key, next);
  const cleanup = function () {
    if (_kbWriteLocks.get(key) === next) _kbWriteLocks.delete(key);
  };
...
export function assertKanbanReadOwner(uid, user, caps) {
  ...
  // LF-KANBAN-ROOTADM-ACCEPT-20260908
  // Espelha _isRootAdm() de authz-cargo-only-dept-patch.js: sub do ADM raiz
  // pode vir namespaced (adm, adm_root_2026, adm_*). Sem isso, o ADM raiz
  // tomava 403 em kanban/list quando o front pedia ?uid=adm_root_2026 e o
  // JWT trazia sub diferente do uid solicitado. Não amplia acesso a outros
  // cargos — só reconhece o mesmo predicado já autorizado pelo authz.
  const _rawSub = String((user && user.raw && user.raw.sub) || sub || '');
  if (user && (user.role === 'adm' || /^adm(_|$)/i.test(_rawSub) || (user.raw && user.raw.adm_root === true))) {
    return true;
  }
...
export function assertKanbanWriteOwner(uid, user, caps) {
  ...
  // LF-KANBAN-ROOTADM-ACCEPT-20260908 (write): mesmo predicado do gate de
  // leitura — ADM raiz namespaced não fica preso no !== sub.
  const _rawSub = String((user && user.raw && user.raw.sub) || sub || '');
  if (user && (user.role === 'adm' || /^adm(_|$)/i.test(_rawSub) || (user.raw && user.raw.adm_root === true))) {
    return true;
  }
...
export async function putKanbanListDoc(request, ctx) {
  ...
  return withKanbanWriteLock(board + '/' + uid, async function () {
