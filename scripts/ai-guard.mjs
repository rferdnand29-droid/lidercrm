...
// --------- 4. R3 — tags <script> de patches replicadas em 4 HTMLs ---------
...
if (HTMLS.length === 4) {
  ...
  for (const patch of union) {
    ...
    if (missing.length && missing.length < 4) {
      fail('R3.HTML_SYNC', patch, `tag <script> presente em alguns HTMLs mas falta em: ${missing.join(', ')}`,
        'Adicione a mesma tag <script src="..."> nos 4 HTMLs (index/app na raiz e em www/).');
    }
  }
}
