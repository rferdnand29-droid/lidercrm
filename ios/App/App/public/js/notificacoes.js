...
function setAutoReminderOn(v){
  ss(AUTO_REMINDER_KEY,!!v);
  _lfLogSettingsChangeToFeed('Automação de lembrete',!!v);
}

/* ============================================================
   AUTO-MOVER LEAD PARADO (3 DIAS) PRA ETAPA LIVRE — liga/desliga
   Pedido 2026-08-17: opção nas Configurações, logo abaixo de
   Automação de Lembretes, pra cada consultor desativar o auto-move de
   _autoMoveStaleToLivre (js/kanban.js) nos PRÓPRIOS leads. Ligado por
   padrão (mesma filosofia da automação de lembrete) — quem não mexer
   não percebe diferença nenhuma.
   ============================================================ */
var LIVRE_AUTO_MOVE_KEY='lf_livre_auto_move_on';

/* BUG #6 (2026-08-19) — persistência no SERVIDOR da preferência
   "auto-mover lead parado pra Livre".

   Antes: só localStorage (sg/ss). Após um deploy, o Service Worker
   limpa caches / o app roda clear() por cota (lf-fix-raiz-token-quota),
   a chave some e isLivreAutoMoveOn() caía no default true — a opção
   desligada "voltava sozinha".

   Agora segue a MESMA trilha da logo/wallpaper (worker client
   getConfig/putConfig, chave por-usuário 'prefs_<uid>'):
     • setLivreAutoMoveOn grava local E sobe ao servidor;
     • hidrataLivreAutoMovePref (chamada no login/startApp) baixa a
       versão do servidor e espelha em S.prefs.livreAutoMoveOn;
     • isLivreAutoMoveOn usa S.prefs como fallback antes do
       localStorage e — enquanto a hidratação não completou —
       retorna null (adiar) em vez de assumir true às cegas. */

var _LIVRE_PREF_HYDRATED=false;  /* fica true quando o servidor respondeu (ok ou falhou) */

function _lfPrefsWorkerClient(){
  try{
  ...
  try{
    var rt=((window.LiderCRM||{}).modules||{}).configuracoes||{};
    var fn=rt.runtime&&rt.runtime._cfgWorkerClient;
    if(typeof fn==='function'){var w=fn();if(w&&typeof w.getConfig==='function')return w;}
  }catch(_e){}
  return null;
}

function _lfPrefsKey(uid){return 'prefs_'+String(uid||'');}

/* Baixa do servidor e espelha em S.prefs.livreAutoMoveOn.
   Chamar no login / startApp. Idempotente. */
function hidrataLivreAutoMovePref(uid){
  uid=uid||(S&&S.userId)||null;
  if(!uid){_LIVRE_PREF_HYDRATED=true;return;}
  var wc=_lfPrefsWorkerClient();
  if(!wc){_LIVRE_PREF_HYDRATED=true;return;}
  wc.getConfig(_lfPrefsKey(uid)).then(function(doc){
    var val=(doc&&typeof doc.livreAutoMoveOn!=='undefined')?!!doc.livreAutoMoveOn:null;
    if(val!==null){
      if(!S)S={};
      S.prefs=S.prefs||{};
      S.prefs.livreAutoMoveOn=val;
      /* espelha no localStorage pra leitura síncrona offline */
      try{ss(LIVRE_AUTO_MOVE_KEY+'_'+uid,val);}catch(_e){}
    }
    _LIVRE_PREF_HYDRATED=true;
  }).catch(function(e){
    console.warn('[notif] hidrataLivreAutoMovePref falhou',e);
    _LIVRE_PREF_HYDRATED=true; /* falhou: libera o gate com fallback local */
  });
}

function isLivreAutoMoveOn(uid){
  var u=uid||(S&&S.userId)||'';
  /* 1º) preferência hidratada do servidor */
  if(S&&S.prefs&&typeof S.prefs.livreAutoMoveOn!=='undefined')return !!S.prefs.livreAutoMoveOn;
  /* 2º) localStorage (cache) */
  var v=sg(LIVRE_AUTO_MOVE_KEY+'_'+u);
  if(v!==null&&v!==undefined)return !!v;
  /* 3º) servidor ainda não respondeu E nada local: ADIA (null) em vez
         de ligar a automação às cegas. O chamador (_autoMoveStaleToLivre)
         deve tratar null como "não rodar agora". */
  if(!_LIVRE_PREF_HYDRATED)return null;
  /* hidratou e não há pref em lugar nenhum: default ligado */
  return true;
}

function setLivreAutoMoveOn(v,uid){
  var u=uid||(S&&S.userId)||'';
  ss(LIVRE_AUTO_MOVE_KEY+'_'+u,!!v);
  if(S&&S.userId===u){S.prefs=S.prefs||{};S.prefs.livreAutoMoveOn=!!v;}
  /* sobe ao servidor (mesma trilha da logo/wallpaper) */
  try{
    var wc=_lfPrefsWorkerClient();
    if(wc){
      wc.getConfig(_lfPrefsKey(u)).catch(function(){return {};})
        .then(function(doc){
          var merged=(doc&&typeof doc==='object')?doc:{};
          merged.livreAutoMoveOn=!!v;
          return wc.putConfig(_lfPrefsKey(u),merged);
        })
        .catch(function(e){console.warn('[notif] setLivreAutoMoveOn sync falhou',e);});
    }
  }catch(_e){}
  _lfLogSettingsChangeToFeed('Auto-mover lead parado (3 dias) para Livre',!!v);
}

/* Ambos os toggles acima (e só eles, a pedido) aparecem pro ADM nas
   Movimentações daquele usuário específico — mesmo padrão de
   logFeedEvent já usado em todo o resto do CRM (bingo, importação em
   lote, etc.), só um type novo ('settings_change', já com rótulo em
   pt-BR no dicionário do feed em js/relatorios.js). */
function _lfLogSettingsChangeToFeed(label,on){
  try{
    if(!S||!S.userId||typeof logFeedEvent!=='function')return;
    logFeedEvent('settings_change',S.userId,label,on?'Ativado':'Desativado','config');
  }catch(_e){}
}
...
function _playNotifFallbackBeep(){
...
function _notifSoundMime(src){
  if(/\.mp3(?:$|\?)/i.test(src))return 'audio/mpeg';
  if(/\.wav(?:$|\?)/i.test(src))return 'audio/wav';
  if(/\.ogg(?:$|\?)/i.test(src))return 'audio/ogg';
  return '';
}
function _buildNotifAudio(kind){
  kind=_notifSoundPaths[kind]?kind:'geral';
  if(_notifSoundTemplates[kind])return _notifSoundTemplates[kind];
  var audio=document.createElement('audio');
  audio.preload='auto';
  audio.setAttribute('playsinline','');
  (_notifSoundPaths[kind]||_notifSoundPaths.geral).forEach(function(src){
    var source=document.createElement('source');
    source.src=src;
    var mime=_notifSoundMime(src);
    if(mime)source.type=mime;
    audio.appendChild(source);
  });
  _notifSoundTemplates[kind]=audio;
  return audio;
}
function _playNotifSound(kind){
  kind=_notifSoundPaths[kind]?kind:'geral';
  try{
    var audio=_buildNotifAudio(kind).cloneNode(true);
    var fallbackDone=false;
    var fallback=function(){
      if(fallbackDone)return;
      fallbackDone=true;
      try{_playNotifFallbackBeep();}catch(_e){}
    };
    ...
    try{_playNotifFallbackBeep();}catch(_e){}
    console.warn('[notif] som falhou',e);
  }
}
...
function _runAutomationEngineBoot(){
  ...
  // [FIX 20260823] Fase 3 (spec "Bitrix24 duplicados", item 2 — "Auto
  // find duplicates"): não existe infraestrutura de job agendado no
  // servidor neste app — todo "recorrente" daqui (automação acima
  // inclusive) já é client-side, rodando enquanto alguém está com o
  // app aberto (boot + a cada 5min, ver js/app.js). Reaproveita esse
  // MESMO gatilho já existente em vez de criar um setInterval novo —
  // a checagem de duplicados só de fato faz algo quando a frequência
  // configurada (diária/semanal) já passou, então rodar junto a cada
  // 5min é inofensivo (no-op na maioria das vezes). try/catch pra
  // nunca quebrar a automação normal se algo aqui falhar.
  try{if(typeof _dupScheduledCheck==='function')_dupScheduledCheck();}catch(_e){}
}
...
function _execAutomationAction(rule,c,board,ownerUid){
  ...
  if(act.tipo==='notify'){
    ...
    // FIX-AUTO-ID (mesma justificativa do edit acima): 8 chars para reduzir
    // risco de colisão quando várias regras disparam em paralelo no mesmo ciclo.
    var actObj={id:'auto_'+Date.now()+'_'+Math.random().toString(36).slice(2,10),type:p.actTipo||'task',desc:desc,scheduledAt:null,done:false,read:false,createdAt:new Date().toISOString(),userId:ownerUserId,clientId:c.id,clientNome:c.name,board:board};
    var list2=getActivitiesLocalFor(ownerUserId);list2.unshift(actObj);lfSaveActivitiesFor(ownerUserId,list2);
    if(!Array.isArray(c.activities))c.activities=[];c.activities.unshift({id:actObj.id,type:actObj.type,desc:desc,scheduledAt:null,done:false,by:'Automação',createdAt:actObj.createdAt});
...
function onAutoBoardChange(){
  ...
  var triggerOpts=allCols.map(function(c){return '<option value="'+_htmlAttr(c.id)+'">'+eH(c.label)+'</option>';}).join('');
  var moveCols=allCols.filter(function(c){return !(board==='leads'&&c.id==='conv');});
  var moveOpts=moveCols.map(function(c){return '<option value="'+_htmlAttr(c.id)+'">'+eH(c.label)+'</option>';}).join('');
...
function openAutoRuleEditor(id){
  ...
  if(uSel)uSel.innerHTML=nu.map(function(u){return '<option value="'+_htmlAttr(u.id)+'">'+eH(u.nome)+'</option>';}).join('');
...
function saveAutoRule(){
  ...
  if(editId){
    ...
    // FIX-AUTO-ID: antes era Date.now()+Math.random().toString(36).slice(2,5)
    // (~3 chars, ~46 mil combinações). Dois ADMs criando regras quase ao
    // mesmo tempo colidiam o id e a 2ª regra sobrescrevia a 1ª silenciosamente
    // (sem nenhum erro, porque .find() retornava o objeto errado). Agora são 8
    // chars (~2,8 bi combinações) — mesma escala dos ids de cliente seguro.
    rules.push({id:'auto_'+Date.now()+'_'+Math.random().toString(36).slice(2,10),nome:nome,board:board,trigger:{tipo:triggerTipo,params:triggerParams},action:{tipo:actionTipo,params:actionParams},ativo:ativo,createdAt:new Date().toISOString()});
  }
...
// ============================================================
/* RHUAN-FIX-20260729-B: som/inbox por tipo real */
var NTF_ICONS={transfer:'🔄',activity:'🔔',automation:'⚙️',chat:'💬'}

var NTF_IC_CLASS={transfer:'meet',activity:'task',automation:'note',chat:'meet'}

/* Mapa tipo-da-notificação -> kind de som usado por _playNotifSound.
   Mantém compatibilidade com os 3 arquivos já existentes em
   assets/sounds/: chat.mp3, atrasada.mp3, geral.mp3.
   - chat      -> som próprio de bate-papo
   - activity  -> "atrasada" (é a família de eventos temporais)
   - transfer  -> "geral" (aviso importante mas não é chat nem atraso)
   - automation-> "geral"
*/
var NTF_SOUND_KIND={chat:'chat',activity:'late',transfer:'geral',automation:'geral'}
...
/* ============================================================
 * MULTI-TAB-SYNC-20260818: um toque em qualquer aba zera o badge em todas.
 *
 * PROBLEMA original relatado pelo usuário:
 *   "ao abrir várias guias tenho que entrar ou marcar como lido em todas
 *   pra sumir a notificação, quero corrigir tocando só uma vez."
 *
 * CAUSA (auditada em saveNotifsFor / updateNotifBadge / _seenNotifIds):
 *   saveNotifsFor gravava só no localStorage da aba atual + Firestore/Worker.
 *   As outras abas do mesmo navegador só descobriam a mudança no próximo
 *   poll (ou quando o usuário abria o sino nelas). Cada aba mantinha seu
 *   próprio _seenNotifIds, então o badge continuava "n" até um evento local.
 *
 * SOLUÇÃO (sem backend novo, 100% APIs nativas do navegador):
 *   1) BroadcastChannel('lf_notif_sync') → canal em tempo real entre abas do
 *      mesmo browser (Chrome/Edge/Firefox/Safari 15.4+/Android WebView 79+).
 *      Toda gravação de notif emite {uid, list}; as outras abas atualizam o
 *      cache local, re-renderizam o painel se aberto, e atualizam o badge —
 *      tudo síncrono, sem esperar poll.
 *   2) window 'storage' event → fallback automático em navegadores antigos
 *      sem BroadcastChannel. ss() já grava no localStorage; o evento dispara
 *      nas OUTRAS abas por si só, sem código extra por chamada.
 *   Firestore/Worker continua funcionando pelo caminho antigo pra sync entre
 *   dispositivos diferentes — este patch fecha só o buraco entre abas do
 *   MESMO navegador (que é o sintoma que o usuário sentiu).
 * ============================================================ */
var _notifBC=null;
try{ if(typeof BroadcastChannel!=='undefined') _notifBC=new BroadcastChannel('lf_notif_sync'); }catch(_e){}

/* Aplica uma lista recebida de OUTRA aba: atualiza cache local (sem re-broadcast
   pra não ecoar), re-renderiza painel se aberto, e atualiza badge do sininho.
   Guarda os ids já vistos no _seenNotifIds da própria aba pra não repetir
   som/aviso nativo de itens que a outra aba já processou. */
function _notifApplyRemoteSync(uid,list){
  if(!S||!S.userId||uid!==S.userId)return; // só me interessa o meu inbox
  if(!Array.isArray(list))return;
  try{ ss(notifKey(uid),list.slice(0,200)); }catch(_e){}
  if(_seenNotifIds){ list.forEach(function(n){ if(n&&n.id)_seenNotifIds.add(n.id); }); }
  var panel=document.getElementById('ntf-panel');
  if(panel&&panel.classList.contains('open')){
    try{ renderNotifPanel(list); }catch(_e){}
  }
  try{ updateNotifBadge(); }catch(_e){}
}

if(_notifBC){
  _notifBC.onmessage=function(ev){
    var d=ev&&ev.data; if(!d||d.kind!=='notifs')return;
    _notifApplyRemoteSync(d.uid,d.list);
  };
}

/* Fallback pra WebViews antigas: o próprio localStorage já emite 'storage'
   nas outras abas quando ss() escreve. Detectamos por prefixo da chave. */
try{
  window.addEventListener('storage',function(ev){
    if(!ev||!ev.key)return;
    if(ev.key.indexOf('lf_notif_')!==0)return;
    if(!S||!S.userId)return;
    if(ev.key!==notifKey(S.userId))return; // ignora inbox de outros usuários
    var list; try{ list=JSON.parse(ev.newValue||'[]')||[]; }catch(_e){ list=[]; }
    _notifApplyRemoteSync(S.userId,list);
  });
}catch(_e){}

function saveNotifsFor(uid,list){
  uid=uid||S.userId;
  list=list.slice(0,200); // FIX 20260729: alinhado com pushNotif (200)
  ss(notifKey(uid),list);
  /* MULTI-TAB-SYNC-20260818: avisa as outras abas do mesmo navegador na hora,
     em vez de esperar elas fazerem o próximo poll ou o usuário tocar no sino
     de novo. As outras abas rodam _notifApplyRemoteSync -> updateNotifBadge. */
  try{ if(_notifBC) _notifBC.postMessage({kind:'notifs',uid:uid,list:list}); }catch(_e){}
  var svc=_notifService();
...
/* Compara a lista recebida com a última conhecida e dispara notificação nativa
   ...
   RHUAN-FIX-20260729-B: som/inbox por tipo real
   Corrigido em 2026-07-29:
   - Passa o `kind` correto pro fireNativeNotification (chat/late/geral) usando
     NTF_SOUND_KIND, senão TUDO tocaria geral.mp3 e o CRM já tem chat.mp3 e
     atrasada.mp3 na pasta assets/sounds/.
   - Na primeira chamada (boot), NÃO faz mais return cego: se a lista local
     já contém itens não lidos (ex.: transferência de responsável recebida
     enquanto o usuário estava fora), o aviso nativo dispara UMA vez por
     item. O Set _seenNotifIds ainda garante que polls subsequentes não
     repitam o mesmo aviso.
   - Título do aviso agora é específico do tipo, deixando claro o que chegou. */
function _alertNewNotifs(list){
  var firstBoot=!_seenNotifIds;
  if(firstBoot)_seenNotifIds=new Set();
  var appName=(window.LiderCRM&&window.LiderCRM.config&&window.LiderCRM.config.appShortName)||'LIDER CRM';
  (list||[]).forEach(function(n){
    if(!n||!n.id)return;
    var seen=_seenNotifIds.has(n.id);
    _seenNotifIds.add(n.id);
    if(seen)return;
    if(n.lida)return;
    var kind=(NTF_SOUND_KIND&&NTF_SOUND_KIND[n.type])||'geral';
    var title;
    if(n.type==='chat')title='💬 Nova mensagem — '+appName;
    else if(n.type==='transfer')title='🔄 Novo responsável — '+appName;
    else if(n.type==='activity')title='🔔 Nova atividade — '+appName;
    else if(n.type==='automation')title='⚙️ Automação — '+appName;
    else title='🔔 Novo aviso — '+appName;
    try{fireNativeNotification(title,n.text||'',n.id,kind);}catch(_e){}
  });
}
...
function pushNotif(toUid,type,text,opts){
  if(!toUid)return;
  /* RHUAN-FIX-20260729-B: som/inbox por tipo real
     Antes tocava _playNotifSound('geral') quando toUid === S.userId, mas na
     prática A envia pra B: A é o remetente e nunca deve ouvir som. Quem toca
     som é o polling do destinatário via _alertNewNotifs (com o kind correto
     do tipo). Deixar isso aqui só produzia beep errado no navegador do
     remetente e nunca no destinatário. */
  if(!S||!S.userId)return;
  opts=opts||{};
  var svc=_notifService();
  ...
  var entry={id:'ntf_'+Date.now()+'_'+Math.random().toString(36).slice(2,5),type:type,text:text,ts:new Date().toISOString(),lida:false,cardId:opts.cardId||null,board:opts.board||null,convId:opts.convId||null};
  var list=getNotifs(toUid);
  list.unshift(entry);
  if(list.length>200)list=list.slice(0,200);
  saveNotifsFor(toUid,list);
  // LF_PUSH_NOTIF_WORKER_FALLBACK_20260729: se o notif-service não subiu,
  // tenta gravar direto via Worker no doc do destinatário para que o
  // Realtime/poll do #ntf-bell dele veja a nova notificação.
  try{
    var root=window.LiderCRM,wc=root&&root.api&&root.api.workerClient;
    var cfg=root&&root.config;
    if(cfg&&cfg.useWorkerApi&&wc&&typeof wc.saveDocument==='function'){
      wc.saveDocument('notifications/'+toUid,{list:list,uid:toUid,ts:Date.now()});
    }
  }catch(_e){}
...
function notifItemClick(id){
  ...
  if(n.cardId&&n.board){
    ...
    // [FIX 20260822] notificação sem card específico (ex.: vários leads
    // importados de uma vez pra mesma pessoa) — antes não fazia nada ao
    // clicar. Agora pelo menos leva pro board certo, já que não tem como
    // apontar pra um card único.
    toggleNotifPanel();
    if(typeof isMobileView==='function'&&isMobileView()&&typeof mobileGoPage==='function')mobileGoPage(n.board);
    else if(typeof goPage==='function')goPage(n.board);
  }
}
...
/* 2026-08-05: marca como lida no sininho a notificação de chat cuja
   conversa acabou de ser aberta — sem isso, ler a mensagem dentro do
   Papo não refletia no sininho, que continuava contando como não-lida
   uma notificação que a pessoa já tinha visto. Chamada por
   openChatConv() (js/chat.js). Silenciosa se não achar nada pra marcar
   (comportamento normal — nem toda mensagem gera notificação de sino,
   ex.: quando o app já está com o Papo aberto). */
function notifAutoMarkReadForConv(convId){
  if(!convId||!S||!S.userId)return;
  var list=getNotifs(S.userId);
  var changed=false;
  list.forEach(function(n){
    if(!n.lida&&n.type==='chat'&&n.convId===convId){ n.lida=true; changed=true; }
  });
  if(!changed)return;
  saveNotifsFor(S.userId,list);
  updateNotifBadge();
  // Painel pode estar aberto ao fundo (raro, mas possível) — atualiza também.
  var panel=document.getElementById('ntf-panel');
  if(panel&&panel.classList.contains('open'))renderNotifPanel(list);
}
window.notifAutoMarkReadForConv=notifAutoMarkReadForConv;
