...
(function(global){
  'use strict';
  if (global.__lfFixDepartmentsCrudV1) return;
  global.__lfFixDepartmentsCrudV1 = true;
  ...
  /* MULTI-SUPERVISOR (2026-08-20): além do formato antigo, espelha as
     colunas JSONB novas (supervisor_uids/adjunto_uids) nos campos
     camelCase supervisorIds/supervisorAdjIds, com fallback pras colunas
     escalares legadas (supervisor_uid/adjunto_uid) quando o backend ainda
     não tiver as colunas novas (migration não rodada). */
  function _arr(x){ return Array.isArray(x) ? x.filter(Boolean) : []; }
  function _fromServerShape(row){
    ...
    var sups=_arr(row.supervisor_uids);
    if(!sups.length && row.supervisor_uid) sups=[row.supervisor_uid];
    var adjs=_arr(row.adjunto_uids);
    if(!adjs.length && row.adjunto_uid) adjs=[row.adjunto_uid];
    out.supervisorIds=sups;
    out.supervisorAdjIds=adjs;
    return out;
  }
  ...
  function create(input){
    ...
    /* Aceita tanto os arrays novos (supervisorIds/adjuntoIds) quanto os
       escalares legados (supervisor_uid/adjunto_uid). O id pode vir do
       chamador (Estrutura gera 'dept_' + Date.now()) pra manter o MESMO id
       no backend e no cache local da Estrutura. */
    var supIds=_arr(input.supervisorIds);
    if(!supIds.length && input.supervisor_uid) supIds=[input.supervisor_uid];
    var adjIds=_arr(input.adjuntoIds);
    if(!adjIds.length && input.adjunto_uid) adjIds=[input.adjunto_uid];
    var payload={
      nome:String(input.nome).trim(),
      descricao:input.descricao||'',
      cor:input.cor||'#3b82f6',
      status:input.status||'ativo',
      supervisorUids:supIds,
      adjuntoUids:adjIds,
      supervisorUid:supIds[0]||null,
      adjuntoUid:adjIds[0]||null
    };
    if(input.id) payload.id=String(input.id);
  ...
  function update(id, patch){
    ...
    if(patch && Array.isArray(patch.supervisorIds)){ body.supervisorUids=patch.supervisorIds.filter(Boolean); body.supervisorUid=body.supervisorUids[0]||null; }
    else if(patch && 'supervisor_uid' in patch) body.supervisorUid=patch.supervisor_uid;
    if(patch && Array.isArray(patch.adjuntoIds)){ body.adjuntoUids=patch.adjuntoIds.filter(Boolean); body.adjuntoUid=body.adjuntoUids[0]||null; }
    else if(patch && 'adjunto_uid' in patch) body.adjuntoUid=patch.adjunto_uid;
  ...
  global.LF_DEPARTMENTS = {
    ...
    diag:function(){
      ...
      return {
        ...
        semSupervisor:all.filter(function(d){return !(d.supervisor_uid||(Array.isArray(d.supervisor_uids)&&d.supervisor_uids.length));}).length,
        hasWorker:!!_wc(),
