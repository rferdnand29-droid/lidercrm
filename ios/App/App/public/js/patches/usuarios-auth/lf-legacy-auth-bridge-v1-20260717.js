...
(function(global){
  ...
  // CORREÇÃO (2026-08-02): a versão anterior só tentava a 1ª ponte aos
  // 800ms. Nessa janela, vários módulos (feed-runtime, preferences-
  // runtime, users-persist-cloudfirst, auth.js/clientesList, heartbeat/
  // online, kanban/list) já disparam suas chamadas de boot e tomam 401
  // "Bearer token ausente" — não é falha de rede, é a ponte não ter
  // rodado ainda (é essa onda de 401 que aparece como "usuário não
  // conectado na nuvem"). tryBridge() já é seguro de chamar cedo demais
  // (se sessão legada/usuário ainda não estiverem prontos, só
  // resolve(false) sem efeito colateral, protegido por _inFlight) —
  // então tentamos imediatamente e em intervalos curtos no início,
  // mantendo os agendamentos antigos como rede de segurança pro caso
  // de loadUsersDB ainda não ter terminado.
  function scheduleInitial(){
    tryBridge();
    setTimeout(tryBridge, 150);
    setTimeout(tryBridge, 400);
    setTimeout(tryBridge, 800);
    setTimeout(tryBridge, 1500);
