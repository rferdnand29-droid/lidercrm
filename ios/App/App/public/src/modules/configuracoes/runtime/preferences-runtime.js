(function(global){
  ...
  function _themeKey(uid){return 'lf_theme_mode'+(uid?'_'+uid:'');}
  function _readThemeLocal(uid){
    try{
      var scoped=localStorage.getItem(_themeKey(uid));
  ...
  function _writeThemeLocal(uid,mode){
    var safe=mode==='classic'?'classic':'light';
    try{localStorage.setItem(_themeKey(uid),safe);}catch(_e){}
...
function setAppThemeMode(mode,silent){
  ...
  _writeThemeLocal(_uid,classic?'classic':'light');
...
try{var _bootUid=(global.S&&global.S.userId)||'';setAppThemeMode(_readThemeLocal(_bootUid)==='classic'?'classic':'light',true);}catch(_e){}
...
function loadThemeRemote(uid,cb){
  var localMode='light';
  try{localMode=_readThemeLocal(uid||'');}catch(_e){}
...
function toggleAppTheme(){
  ...
  function _bgPreviewStyle(bg,photoUrl){if(bg.id==='photo'){
      if(!photoUrl) return 'background:rgba(255,255,255,.08)';
      /* FIX-WP-PREVIEW (2026-07-28): sobrepor o overlay glass da v2 para preview fiel */
      var isDark;
      try{ isDark=(typeof _lfIsThemeDark==='function')?_lfIsThemeDark():(document.body&&document.body.classList.contains('theme-classic')); }catch(_e){isDark=false;}
      var ovRgb=isDark?'20, 24, 32':'255, 255, 255'; var ovA=isDark?0.55:0.72;
      return "background:linear-gradient(rgba("+ovRgb+","+ovA+"),rgba("+ovRgb+","+ovA+")),url('"+photoUrl+"') center/cover no-repeat";
    }if(bg.id==='navy')return'background:linear-gradient(135deg,#0a1a2e,#0a0c10)';if(bg.id==='forest')return'background:linear-gradient(135deg,#0a1a10,#0a0c10)';if(bg.id==='purple')return'background:linear-gradient(135deg,#150a2a,#0a0c10)';if(bg.id==='gold')return'background:linear-gradient(135deg,#1a1200,#0a0c10)';if(bg.id==='slate')return'background:linear-gradient(135deg,#0f1218,#0a0c10)';return'background:var(--bg2)';}
...
function renderConfig(){
  ...
  var clm=document.getElementById('cfg-livre-auto-move');if(clm&&typeof isLivreAutoMoveOn==='function')clm.checked=isLivreAutoMoveOn(S.userId);
...
function compressImageFile(file,maxBytes,cb){
  ...
  /* ── FIX-WP-CONSOLIDATE (2026-07-28): transparência inteligente de wallpaper
     Mantida com helpers delegáveis, mas com fallback degradado no próprio
     bundle para evitar dependência de patch paralelo não registrado no HTML. */
  function _lfIsThemeDark(){
    if(global.__LF_WALLPAPER_TRANSP_V2__ && global._lfIsThemeDark) return global._lfIsThemeDark();
  ...
  /* R14-12: expor funções ao escopo global (delegam ao v2 quando disponível) */
