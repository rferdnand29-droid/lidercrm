(function(global){
  ...
  function kbCols(board){
    ...
    // [FIX 20260908-STAGE-PICKER-LEADS] Dupla proteção contra vazamento de
    // contexto entre boards: (1) se _kbDetBoard/_kbDetOwnerUid estiverem
    // inconsistentes (detalhe aberto de OUTRO board), ignora o contexto do
    // detalhe; (2) o resultado adm_* só se aplica a board==='negocios' —
    // Leads NUNCA resolve pipeline administrativo (IDs adm_* não existem lá,
    // e mover para eles fazia o card sumir da tela).
    var ownerUid=null;
    try{
    ...
    }catch(_e){ownerUid=null;}
    if(board!=='negocios') return KB_NEG_COLS;
