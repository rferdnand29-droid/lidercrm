(function(global){
  ...
  /* LF-NOTIF-PREREFRESH-20260908: pré-atualização da sessão antes do
     primeiro GET /notificacoes/inbox da sessão. middlewares/auth.js só
     concede janela de graça de 7 dias em POST /api/v1/session/refresh —
     sem este disparo, a 1ª bateria após o app voltar de suspenso colhe
     401 com o token expirado. Idempotente: roda 1x por sessão e nunca
     propaga erro. */
  var _preRefreshDone = false;
  function ensureFreshSession(){
    if (_preRefreshDone) return Promise.resolve();
    _preRefreshDone = true;
    var wc = root.api && root.api.workerClient;
    if (!wc || typeof wc.request !== 'function') return Promise.resolve();
    try {
      return wc.request('/session/refresh', { method: 'POST' }).then(function(){ return null; }, function(){ return null; });
  ...
  NotificationService.prototype.saveNotifs = function(uid, list){
    ...
    if(workerReady() && uid){
      return workerRequest('/notificacoes/inbox?uid=' + encodeURIComponent(uid), { method: 'PUT', body: { list: safe, clientTs: clientTs } }).then(function(){
        return safe;
      }).catch(function(){ return safe; });
    }
    if(!dbReady() || !uid) return Promise.resolve(safe);
    return notifDoc(uid).set({ list: safe, ts: Date.now(), clientTs: clientTs }).then(function(){
      return safe;
    }).catch(function(){ return safe; });
  };

  /* [FIX 20260820-2] Merge específico de notificações.
     _mergeKeepLocalOnly (kanban-helpers) desempata por updatedAt/createdAt —
     campos que notificação NÃO tem (só `ts`). Resultado: empate '' vs '' e
     a versão do SERVIDOR sempre vencia, descartando o `lida:true` local
     recém-gravado por markAllNotifsRead/notifItemClick — a notificação
     "voltava como não lida" a cada poll de 60s e o sino ficava piscando.
     Regra aqui: para o MESMO id, o estado local de leitura (lida) SEMPRE
     vence quando local está lida e o servidor ainda não; o resto dos campos
     vem da versão mais recente por `ts`. Itens só no servidor entram (foram
     gerados em outro dispositivo); itens só locais entram (upload pendente). */
  function _mergeNotifs(server, local){
    server = Array.isArray(server) ? server : [];
    local  = Array.isArray(local)  ? local  : [];
    var localById = {};
    local.forEach(function(n){ if(n && n.id) localById[n.id] = n; });
    var seen = {};
    var merged = server.map(function(n){
      if(!n || !n.id) return n;
      seen[n.id] = true;
      var l = localById[n.id];
      if(!l) return n; // só no servidor: chegou de outro dispositivo
      // Preserva leitura local: se marquei como lida aqui, o servidor
      // não pode reabrir. (lida:true local sempre vence.)
      var out = n;
      if(l.lida === true && n.lida !== true){
        out = {};
    ...
    local.forEach(function(n){
      if(n && n.id && !seen[n.id]) merged.push(n); // extra local: upload pendente
    });
  ...
  NotificationService.prototype.loadNotifs = function(uid, callback){
    ...
    if(typeof callback === 'function') callback(local);
    if(workerReady() && uid){
      // LF-NOTIF-PREREFRESH-20260908: refresh antes do 1º GET da sessão.
      return ensureFreshSession().then(function(){
        return workerRequest('/notificacoes/inbox?uid=' + encodeURIComponent(uid), { method: 'GET' });
      }).then(function(server){
        /* [FIX 20260820-2] antes sobrescrevia o local com a lista crua do
           servidor (sem merge) — mesma ressurreição de não-lidas. */
        var list = Array.isArray(server) ? _mergeNotifs(server, localNotifs(uid)) : local;
    ...
    return notifDoc(uid).get().then(function(snapshot){
      ...
      var merged = _mergeNotifs(server, localNotifs(uid));
