...
var toggleDetCard=__documentosRuntime.toggleDetCard||function(){};
...
/* [FIX 20260820] Comentários do lead — pedido: "poder deixar e escrever
   comentario sobre aquele lead" na aba Anexos. Cada comentário fica
   guardado em c.comments (array — igual ao padrão já usado pra
   c.attachments/c.historico), com autor e data. Qualquer um que possa
   editar anexos (_attCanEditCurrentCard — mesma trava de Vídeo/Loja/
   somente-leitura) pode comentar; só quem escreveu ou um ADM pode
   apagar. Aparece no feed do ADM igual anexo (logAttEvent), pra dar
   visibilidade sem precisar abrir o card. */
function renderLeadComments(c){
  var list=document.getElementById('att-comments-list');if(!list)return;
  var comments=(c&&c.comments)||[];
  // [FIX 20260822] REDESIGN: subtítulo do cartão recolhível "Comentários"
  // — contagem, dado que já existe (c.comments), nenhuma lógica nova.
  var detComentSub=document.getElementById('det-coment-sub');
  if(detComentSub)detComentSub.textContent=comments.length?(comments.length+' comentário'+(comments.length>1?'s':'')):'Nenhum comentário';
  if(!comments.length){list.innerHTML='<div class="act-empty">Nenhum comentário ainda.</div>';return;}
  var meId=(S&&S.userId)||null;
  var canDelAny=(typeof hasAdminAccess==='function')&&hasAdminAccess();
  list.innerHTML=comments.slice().reverse().map(function(cm){
    var dt=cm.ts?new Date(cm.ts).toLocaleString('pt-BR'):'';
    var canDel=canDelAny||(meId&&cm.byId===meId);
    return '<div class="hist-item att-comment-item">'
      +'<div class="hist-txt">'+eH(cm.text)+'</div>'
      +'<div class="hist-meta">'+eH(cm.by||'')+(dt?' · '+dt:'')
      +(canDel?' · <a href="javascript:void(0)" onclick="delLeadComment(\''+_jsSq(cm.id)+'\')" style="color:var(--rl)">excluir</a>':'')
      +'</div></div>';
  }).join('');
}
...
function addLeadComment(){
  ...
  var inp=document.getElementById('att-comment-input');
  var text=(inp&&inp.value||'').trim();
  if(!text){toast('⚠ Escreva um comentário antes de adicionar');return;}
  if(!S||!S.userId){toast('Sessão expirada.');return;}
  if(!Array.isArray(c.comments))c.comments=[];
  var comment={id:'cmt_'+Date.now()+'_'+Math.random().toString(36).slice(2,5),text:text,by:S.nome||S.userId,byId:S.userId,ts:new Date().toISOString()};
  c.comments.push(comment);
  c.updatedAt=new Date().toISOString();
  var ok=saveKBFor(board,uid,arr);
  logAttEvent('comment',c.name,text.length>60?text.slice(0,60)+'…':text,board);
  if(inp)inp.value='';
  renderLeadComments(c);
  toast(ok?'💬 Comentário adicionado':'⚠️ Comentário pode não ter sido salvo — armazenamento local cheio.');
}

function delLeadComment(commentId){
  var board=(typeof _kbDetBoard!=='undefined'?_kbDetBoard:null),id=(typeof _kbDetId!=='undefined'?_kbDetId:null);if(!board||!id)return;
  var uid=((typeof _kbDetOwnerUid!=='undefined'?_kbDetOwnerUid:null)||(typeof activeUID==='function'?activeUID(board):null));
  var arr=getKBFor(board,uid);var c=arr.find(function(x){return x.id===id;});if(!c||!Array.isArray(c.comments))return;
  var cm=c.comments.find(function(x){return x.id===commentId;});if(!cm)return;
  var meId=(S&&S.userId)||null;
  var canDel=((typeof hasAdminAccess==='function')&&hasAdminAccess())||(meId&&cm.byId===meId);
  if(!canDel){toast('Só quem escreveu ou um ADM pode excluir este comentário.');return;}
  c.comments=c.comments.filter(function(x){return x.id!==commentId;});
  c.updatedAt=new Date().toISOString();
  saveKBFor(board,uid,arr);
  logAttEvent('comment_delete',c.name,cm.text.length>60?cm.text.slice(0,60)+'…':cm.text,board);
  renderLeadComments(c);
...
function _deleteFromStorage(path,cb){
  ...
  if(typeof b2DeleteFile==='function'&&b2IsAvailable&&b2IsAvailable()){
    return b2DeleteFile(path,null,function(err){
      ...
      if(cb)cb(err);
    /* R5-6: propaga o erro para o caller — cb() sem argumento era interpretado como sucesso */
    }).catch(function(e){ if(cb)cb(e||new Error('b2-delete-rejected')); });
  }
  var gw=_storageGateway();
...
var _lastDownloadBlobUrl=null; /* AUDITORIA-FINAL-10 (2026-08-01, item 2.4): rastreia
  a última blob: URL de download pra poder revogar e não vazar memória a cada
  anexo baixado. Variável própria (não reaproveita _attViewBlobUrl, usada pelo
  preview) pra não revogar uma preview aberta se o usuário clicar "baixar" logo
  em seguida. */
function _attDownloadHref(a,cb){
  if(a.data){cb(a.data);return;}
  if(a.url){
    fetch(a.url).then(function(r){return r.blob();}).then(function(blob){
      if(_lastDownloadBlobUrl){try{URL.revokeObjectURL(_lastDownloadBlobUrl);}catch(e){}}
      _lastDownloadBlobUrl=URL.createObjectURL(blob);
      cb(_lastDownloadBlobUrl);
...
function processAttFiles(files,c,arr,board,uid){
  ...
  valid.forEach(function(file){
    ...
    if(DB_MODE==='firebase'&&_canUseManagedStorage()){
      ...
      // AUDITORIA-FINAL-10 (2026-08-02, sincronização PC/mobile/Capacitor):
      // antes, TODO anexo (inclusive foto) virava base64 embutido dentro
      // do documento kanban/list inteiro (compartilhado pela empresa toda,
      // reescrito por completo a cada salvamento) — o mesmo problema que
      // já tinha sido corrigido pra áudio em 2026-07-20 (ver comentário
      // "CORREÇÃO ÁUDIO" em src/repositories/storage-repository.js) nunca
      // foi replicado pra anexos gerais. Em celular isso pesa mais: menos
      // memória pra segurar uma string base64 grande, rede mais lenta pra
      // sincronizar um documento gigante, maior chance de falha a meio
      // caminho numa foto de vários MB tirada na hora pela câmera.
      // Agora usa o MESMO storage-repository.js que o áudio já usa —
      // upload binário direto (Supabase Storage, com fallback automático
      // pro Worker e depois legado já embutido no próprio repo) — com
      // fallback pro base64 antigo só se o repo não estiver disponível,
      // pra nunca regredir se algo não carregou.
      var storageRepo = (window.LiderCRM && window.LiderCRM.repositories && window.LiderCRM.repositories.storage);
      if (storageRepo && typeof storageRepo.upload === 'function' && typeof storageRepo.canUpload === 'function' && storageRepo.canUpload()) {
        var stPath = 'documentos/' + board + '/' + uid + '/' + c.id + '/' + attId + '_' + _safeStorageName(file.name);
        storageRepo.upload(file, stPath, function(err, res) {
          if (err || !res || !res.url) {
            // Não deixa o anexo simplesmente falhar — cai pro caminho
            // antigo (base64), que sempre funcionou até aqui.
            var readerFb = new FileReader();
            readerFb.onload = function(e) {
              c.attachments.push({
                id: attId, name: file.name, type: file.type,
            ...
            readerFb.onerror = function() { toast('❌ Erro ao ler o arquivo: ' + file.name); finishOne(); };
            readerFb.readAsDataURL(file);
            return;
          }
          c.attachments.push({
            id: attId, name: file.name, type: file.type,
...
function processAdmDocFiles(files){
  ...
  valid.forEach(function(file){
    ...
    if(DB_MODE==='firebase'&&_canUseManagedStorage()){
      ...
      // AUDITORIA-FINAL-10 (2026-08-02, sincronização PC/mobile/Capacitor):
      // mesmo problema e mesma correção de processAttFiles logo acima —
      // ver o comentário completo lá. Documentos ADM é um repositório
      // compartilhado por toda a empresa (ver docs/permissions.md), então
      // o custo de embutir base64 é ainda maior aqui: todo mundo
      // resincroniza o documento inteiro a cada arquivo novo de qualquer
      // gerente.
      var storageRepoAdm = (window.LiderCRM && window.LiderCRM.repositories && window.LiderCRM.repositories.storage);
      if (storageRepoAdm && typeof storageRepoAdm.upload === 'function' && typeof storageRepoAdm.canUpload === 'function' && storageRepoAdm.canUpload()) {
        var stPathAdm = 'adm_docs/' + docId + '_' + _safeStorageName(file.name);
        storageRepoAdm.upload(file, stPathAdm, function(err, res) {
          if (err || !res || !res.url) {
            var readerFb = new FileReader();
            readerFb.onload = function(e) {
              docs.push({
                id: docId, name: file.name, type: file.type, data: e.target.result, size: file.size,
                uploadedAt: new Date().toISOString(), uploadedBy: (S && S.nome) || '?', pinned: false,
              });
              ok++; finishOne();
            };
            readerFb.onerror = function() { toast('❌ Erro ao ler o arquivo: ' + file.name); finishOne(); };
