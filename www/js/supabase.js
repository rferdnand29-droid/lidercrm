...
var _fltNicho='',_fltDate='';

var _nshCid=null,_nshOpt=null,_nshOpts=null;
...
function _currentDashFilters(){
  return {nicho:_fltNicho||'',date:_fltDate||'',ts:Date.now()};
}
...
function initDB(){
  ...
  /* CORREÇÃO 2026-08-17 (tela de carregamento perceptível ao abrir link em
     nova guia): antes, a splash só escondia depois do round-trip completo
     de rede (signIn + preflight, mais 500ms de propósito) — mesmo quando
     o aparelho já tinha sessão válida E já esteve conectado à nuvem há
     pouco (cache de 30min, _lfReadAnonCache). Com os dois sinais batendo,
     a UI já pode aparecer quase na hora — a conexão de verdade continua
     em segundo plano exatamente como antes (DB_MODE começa 'local' e vira
     'firebase' assim que _finishCloudOk() resolver, sem o usuário
     perceber — mesmo fallback gracioso que já existia pra rede lenta). */
  try{
    if(sg('lf6_s')&&_lfReadAnonCache()){
      setTimeout(_lfRevealAppEarly,80);
    }
  }catch(_e){}
  ...
  var _authConfig = (window.LiderCRM && window.LiderCRM.config && window.LiderCRM.config.auth) || {};
  var _SDK_POLL_LIMIT = _LF_CAPACITOR.native
    ? (Number(_authConfig.sdkPollLimitNative) || 40)
    : (Number(_authConfig.sdkPollLimitWeb) || 12);
  var _SIGNIN_LIMIT = _LF_CAPACITOR.native ? 2 : 2;
  var _signinTimeoutMs = _LF_CAPACITOR.native
    ? (Number(_authConfig.signInTimeoutMsNative) || 18000)
    : (Number(_authConfig.signInTimeoutMsWeb) || 7000);
  ...
  function _connectSupabase(){
    // [FIX 20261012] Guarda de idempotência — se um cliente já foi criado
    // (por qualquer caminho, mesmo um não previsto por startBoot/
    // __bootStarted em configuracoes.js), reaproveita em vez de criar
    // outro. Elimina a causa mais comum do aviso "Multiple GoTrueClient
    // instances" do SDK, sem custo nem risco (não achei uma segunda
    // chamada real no código — isso é defesa em profundidade, não
    // correção de um bug confirmado).
    ...
    try{
      ...
      /* FIX 2026-08-04: expõe o CLIENTE criado globalmente. O chat
         (_chatStartPresence em js/chat.js) procura window.supabase.channel(),
         mas window.supabase é o SDK (só tem createClient) — sem este alias a
         presença nunca inicializava ("Supabase indisponível"). */
      try{ window.supabaseClient=_sbClient; }catch(_e){}
    ...
    var t=setTimeout(function(){
      ...
      if(_signinAttempts < _SIGNIN_LIMIT){
        _signinAttempts++;
        clearTimeout(t);
    ...
    function _doSignin(){
      ...
      try{
        _sbClient.auth.signInAnonymously().then(function(res){
          ...
          clearTimeout(t);
          if(_signinAttempts < _SIGNIN_LIMIT){
            console.warn((_LF_CAPACITOR.native?'initDB Capacitor':'initDB web')+': retry #' + _signinAttempts);
            setTimeout(_doSignin, 800*_signinAttempts);
            return;
          }
...
/* CORREÇÃO 2026-08-17 (splash perceptível ao abrir em nova guia): hideSplash()
   é o que dispara bootApp() — antes só era chamado depois do round-trip
   completo de conexão (signIn + preflight + 500/600ms de propósito). Ver
   _lfRevealAppEarly em initDB(), que agora pode chamar isto bem mais cedo
   quando já há sessão local + cache de conexão recente. Guard idempotente:
   qualquer um dos caminhos (early reveal, sucesso normal, fallback local)
   pode chegar aqui primeiro — só o primeiro realmente executa. */
var _lfSplashRevealed=false;
function _lfRevealAppEarly(){
  if(_lfSplashRevealed)return;
  hideSplash();
}
function hideSplash(){
  if(_lfSplashRevealed)return;
...
// ============================================================
// IDENTIDADE VISUAL OFICIAL — Líder CRM
// ============================================================
...
(function(){try{
  ...
  var img=new Image();
  /* AUDITORIA-FINAL-10 (2026-08-01, item 4.5): rastreia a blob: URL do
     manifest pra poder revogar antes de criar a próxima — vazamento
     pequeno e fixo (no máx. 1 por execução deste bloco), mas sem custo
     revogar. Mantém start_url/scope relativos como já eram (comportamento
     de manifest inalterado — ver docs/troubleshooting.md sobre blob:). */
  var _pwaManifestBlobUrl=null;
  function _setManifestBlobUrl(el,blob){
    if(!el)return;
    if(_pwaManifestBlobUrl){try{URL.revokeObjectURL(_pwaManifestBlobUrl);}catch(e){}}
    _pwaManifestBlobUrl=URL.createObjectURL(blob);
    el.href=_pwaManifestBlobUrl;
  }
  img.onload=function(){
    try{
      ...
      var appShortName=(window.LiderCRM&&window.LiderCRM.config&&window.LiderCRM.config.appShortName)||'LIDER CRM';
      var m={name:appShortName+' - '+appName,short_name:appShortName,start_url:window.location.href.split('?')[0],display:'standalone',background_color:'#0A0C10',theme_color:'#0A0C10',icons:[{src:pngUrl,sizes:'192x192',type:'image/png'},{src:pngUrl,sizes:'512x512',type:'image/png'}]};
      _setManifestBlobUrl(document.getElementById('pwa-manifest'),new Blob([JSON.stringify(m)],{type:'application/json'}));
    }catch(e){
      var appName2=(window.LiderCRM&&window.LiderCRM.config&&window.LiderCRM.config.appName)||'Líder CRM';
      var appShortName2=(window.LiderCRM&&window.LiderCRM.config&&window.LiderCRM.config.appShortName)||'LIDER CRM';
      var m2={name:appShortName2+' - '+appName2,short_name:appShortName2,start_url:window.location.href.split('?')[0],display:'standalone',background_color:'#0A0C10',theme_color:'#0A0C10',icons:[{src:iconDataUrl,sizes:'480x480',type:'image/jpeg'}]};
      _setManifestBlobUrl(document.getElementById('pwa-manifest'),new Blob([JSON.stringify(m2)],{type:'application/json'}));
    }
  };
  img.onerror=function(){
    var appName3=(window.LiderCRM&&window.LiderCRM.config&&window.LiderCRM.config.appName)||'Líder CRM';
    var appShortName3=(window.LiderCRM&&window.LiderCRM.config&&window.LiderCRM.config.appShortName)||'LIDER CRM';
    var m3={name:appShortName3+' - '+appName3,short_name:appShortName3,start_url:window.location.href.split('?')[0],display:'standalone',background_color:'#0A0C10',theme_color:'#0A0C10',icons:[]};
    _setManifestBlobUrl(document.getElementById('pwa-manifest'),new Blob([JSON.stringify(m3)],{type:'application/json'}));
  };
  img.src=iconDataUrl;
  }catch(e){}
