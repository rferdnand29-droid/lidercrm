  ...
  if(_timeViewUid){
    ...
    if(typeof loadCli==='function'){
      loadCli(_timeViewUid,function(list){
        ...
        if(typeof drawNegKPIs==='function')drawNegKPIs(_timeViewUid,'time-krow2',(typeof _analyticsDateRange==='function')?_analyticsDateRange('time-krow'):null,'time-krow');
      });
    }
  } else {
    // -------- MODO "TODOS" --------
...
function confirmDeleteKBReason(){
    ...
    try{
      ...
      if(_acts){
        var _nextActs=_acts.filter(function(a){
          var _match=(a&&a.clientId===cardId&&(!a.board||a.board===board));
          if(_match&&a.id&&typeof window._lfMarkRecentlyDeleted==='function')window._lfMarkRecentlyDeleted(a.id);
          return !_match;
        });
  ...
  // [FIX 20260905] Excluir/descartar um Lead na etapa Livre (pool
  // compartilhado entre consultores, com cache próprio — ver
  // _lfLivrePoolServerCache) nunca invalidava esse cache específico —
  // outros consultores continuavam vendo o lead removido até a próxima
  // sondagem de 15s (ou nunca, se ninguém mais interagisse com nada).
  if(affected.leads&&typeof _lfRefreshLivrePoolFromServer==='function')_lfRefreshLivrePoolFromServer(true);
...
function convertToNeg(cardId,ownerUid,prevCol,silent,opts,noAuto){
  ...
  if(!already)_pushHistorico(c,'Convertido em Negócio (etapa: "'+_colLabel('negocios',targetCol)+'")');
  ...
  /* [FIX 20260908-CONV-NEGOCIO-2] Se a gravação do negócio falhou (storage cheio),
     REVERTE a marcação do lead: antes o lead ficava conv (sumia da aba Leads) e o
     negócio não existia em lugar nenhum — perda silenciosa exata do sintoma relatado. */
  if(!okConv&&!already){
    negArr.pop();
    c.col=(c.colAntesConv!==undefined?c.colAntesConv:oldCol);
    delete c.colAntesConv;
    saveKBFor('leads',uid,arr);
    toast('⚠️ Não foi possível criar o Negócio — armazenamento local cheio. O lead foi mantido na etapa anterior.');
    renderKBLocal('leads');
    return;
  }
  ...
  /* [FIX 20260908-CONV-NEGOCIO-3] Limpa filtros residuais da aba Negócios
     (chip/etapa via stage-summary-bar, filtros avançados nicho/valor/dias/usuário
     e "só atrasadas") antes de renderizar — senão o card recém-convertido caía
     fora do filtro ativo e parecia não ter sido criado. */
  if(typeof _mbStageFilter!=='undefined')_mbStageFilter['negocios']=null;
  if(typeof _kbFilter!=='undefined')_kbFilter['negocios']={nicho:'',valorMin:'',valorMax:'',dias:'',usuario:'',repetido:false};
  if(typeof _kbOnlyLate!=='undefined')_kbOnlyLate['negocios']=false;
  if(typeof renderMobileChips==='function')renderMobileChips('negocios');
  if(typeof renderStageSummaryBar==='function')renderStageSummaryBar('negocios');
  /* [FIX 20260908-CONV-NEGOCIO-1] Push imediato ao Worker/Firestore: sem isto, o
     negócio novo só subia no próximo ciclo periódico (15–20s) — e um poll/SSE do
     servidor (sem o card) podia sobrescrever o cache local antes disso, apagando
     o card recém-criado (corrida documentada no lf-fix-cards-missing-own-board-v2). */
  try{
    var _wcC=(typeof _kbWorkerClient==='function')?_kbWorkerClient():null;
    if(_wcC||(typeof DB_MODE!=='undefined'&&DB_MODE==='firebase'&&typeof db!=='undefined'&&db)){
      if(typeof _kbRemoteEnqueue==='function')_kbRemoteEnqueue('negocios',uid,_wcC,function(){});
    }
  }catch(_eC){}
  var _scrollSnap=(typeof _kbCaptureScrollSnapshot==='function')?_kbCaptureScrollSnapshot():null;
