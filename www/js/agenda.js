...
var ligKeyForDate=__agendaRuntime.ligKeyForDate||function(){};
var getLigForDate=__agendaRuntime.getLigForDate||function(){};
...
function loadAllActivitiesAdmin(cb){
  // FIX-403-ATIVIDADES (2026-08-26): o Worker recusa com 403 a leitura da
  // lista de OUTRO uid quando o logado não tem adminUI/supervisorUI
  // (canAccessUid, utils/team-scope.js). Esta função é chamada também pelo
  // painel do sino (toggleActPanel) para QUALQUER usuário — sem esta
  // guarda, cada colega ativo gerava um GET 403. Sem permissão de visão
  // global, carregamos apenas a lista do próprio usuário (e o cache local
  // dos demais, sem rede). ADM/supervisor mantêm o comportamento anterior.
  var _meId=(S&&S.userId)||null;
  var _canAll=false;
  try{
    var _api=(window.LiderCRM&&window.LiderCRM.api)||null;
    if(_api&&typeof _api.canReadForeignActivities==='function')_canAll=!!_api.canReadForeignActivities();
    else if(typeof hasAdminAccess==='function')_canAll=!!hasAdminAccess();
  }catch(_e){_canAll=false;}
  var users=getUsers().filter(function(u){return u.ativo!==false;});
  if(!_canAll&&_meId){users=users.filter(function(u){return String(u.id)===String(_meId);});}
  ...
  users.forEach(function(u){
    if(wc){
      wc.atividadesList(u.id).then(function(doc){
        var serverList=(doc&&doc.list)?doc.list:null;
        var local=getActivitiesLocalFor(u.id)||[];
        var list=serverList?_mergeActivitiesServerFetch(serverList,local):local;
      ...
      db.collection('activities').doc(u.id).get().then(function(d){
        var serverList=(d.exists&&d.data().list)?d.data().list:null;
        var local=getActivitiesLocalFor(u.id)||[];
        var list=serverList?_mergeActivitiesServerFetch(serverList,local):local;
        ss(actKeyFor(u.id),list);
        results=results.concat(list);done();
...
// [FIX 20260909] Bug grave, pedido explícito: atividades concluídas
// voltando como "atrasadas" depois de atualizar o CRM — a causa era
// que os pontos que buscam atividades do servidor (abrir o painel do
// sino, ver atividades de outro usuário) sobrescreviam o cache local
// DIRETO com a resposta do servidor, sem merge nenhum. Se o servidor
// ainda não tivesse processado uma conclusão recente (a conclusão em
// si é otimista/local-first — grava local na hora, envia pro servidor
// depois), essa sobrescrita "desfazia" a conclusão visualmente. Esta
// função central corrige isso: qualquer busca no servidor passa por
// aqui antes de ir pro cache local.
function _mergeActivitiesServerFetch(serverList,localList){
  serverList=Array.isArray(serverList)?serverList:[];
  localList=Array.isArray(localList)?localList:[];
  var localById={};
  localList.forEach(function(a){if(a&&a.id)localById[a.id]=a;});
  var merged=serverList.filter(function(a){
    // Protege contra item excluído recentemente "ressuscitando".
    return !(a&&a.id&&typeof window._lfIsRecentlyDeleted==='function'&&window._lfIsRecentlyDeleted(a.id));
  }).map(function(a){
    if(!a||!a.id)return a;
    var loc=localById[a.id];
    // Protege contra conclusão local recente sendo revertida por uma
    // resposta do servidor que ainda não processou essa conclusão.
    // [FIX 20260907-KANBAN409-CIRURGICO] Ampliado: item com _pending:true (edicao local ainda nao
    // confirmada pelo servidor — a flag sobrevive a reload porque fica
    // gravada no cache) tambem vence a versao do servidor. Sem isto,
    // qualquer fetch intermediario (abrir o painel do sino, sync de
    // outro uid) podia regravar a versao velha por cima de uma conclusao
    // pendente e a atividade "voltava atrasada".
    if(loc&&((loc.done&&!a.done)||loc._pending===true))return loc;
    return a;
  });
  // Mantém itens que só existem localmente ainda (criados agora,
  // não sincronizados com o servidor).
  var mergedIds={};
  merged.forEach(function(a){if(a&&a.id)mergedIds[a.id]=true;});
  localList.forEach(function(a){
    if(a&&a.id&&!mergedIds[a.id]&&!(typeof window._lfIsRecentlyDeleted==='function'&&window._lfIsRecentlyDeleted(a.id))){
      merged.push(a);
    }
  });
  return merged;
...
function createActivity(){
  ...
  var actNow=new Date().toISOString();
  var act={id:'act_'+Date.now()+'_'+Math.random().toString(36).slice(2,5),type:_actType,desc:desc,scheduledAt:sched,done:false,read:false,createdAt:actNow,updatedAt:actNow,userId:targetUid,clientId:null,clientNome:null};
  ...
  function _afterCreate(){
    ...
    // FIX 2026-08-18: registra criação de agendamento no feed ADM ("Movimentações da equipe").
    try{
      if(typeof logFeedEvent==='function' && S && S.userId){
        var _tgtNome=(function(){var u=(typeof getUser==='function')?getUser(targetUid):null;return u?(u.nome||u.email||targetUid):targetUid;})();
        var _detalhe=(sched?('Para '+(new Date(sched).toLocaleString('pt-BR',{day:'2-digit',month:'2-digit',hour:'2-digit',minute:'2-digit'}))):'Sem horário')+(isSelf?'':' · Responsável: '+_tgtNome);
        logFeedEvent('act_create',S.userId,desc,_detalhe,'agenda');
      }
    }catch(_e){}
    renderActPanel();
...
/* Excluir atividade vinculada individual (chamado pelo modal mo-act-edit e pelo
 * botao do cartao). Segue o mesmo padrao do deleteActBulk: tombstone para o
 * merge do Worker nao "ressuscitar" o item + limpeza do espelho card.activities
 * para o badge de atrasada nao ficar fantasma. */
function deleteLinkedActivity(){
  if(!_actEditId||!_actEditOwnerUid){toast('Atividade não selecionada.');return;}
  var uid=_actEditOwnerUid, id=_actEditId;
  _confirmModal({
    title:'🗑 Excluir atividade?',
    msg:'Excluir esta atividade permanentemente?',
    okLabel:'Excluir', okClass:'bd',
    onOk:function(){
      _loadActivitiesForOwner(uid,function(list){
        list=(Array.isArray(list)?list:[]).filter(function(x){return x.id!==id;});
        // tombstone: sem isso o merge conservador do Worker a "ressuscita" — mesmo padrão do deleteActBulk
        if(typeof window._lfMarkRecentlyDeleted==='function'){try{window._lfMarkRecentlyDeleted(id);}catch(_e){}}
        if(uid===(S&&S.userId)){ saveActivities(list); }
        else if(typeof lfSaveActivitiesFor==='function'){ lfSaveActivitiesFor(uid,list); }
        // purga o espelho no card (evita badge fantasma de atrasada)
        try{
          if(_actEditBoard&&_actEditCardId){
            var arr=getKBFor(_actEditBoard,uid);
            var c=arr.find(function(x){return x.id===_actEditCardId;});
            if(c&&Array.isArray(c.activities)){
              c.activities=c.activities.filter(function(x){return x.id!==id;});
              saveKBFor(_actEditBoard,uid,arr);
            }
          }
        }catch(_e){}
        try{ if(typeof logFeedEvent==='function'&&S&&S.userId) logFeedEvent('act_delete',S.userId,'atividade','Excluída',_actEditBoard||'agenda'); }catch(_e){}
        closeM('mo-act-edit');
...
/* Variante para o botao 🗑 direto no cartao da atividade (sem abrir o editor):
 * seta o contexto do editor e reutiliza o fluxo de confirmacao acima. */
function deleteLinkedActivityById(id,uid,board,cardId){
  if(!id||!uid){toast('Atividade não selecionada.');return;}
  _actEditId=id;_actEditOwnerUid=uid;_actEditBoard=board||'';_actEditCardId=cardId||'';
...
function saveLinkedActivityEdit(){
  ...
  _loadActivitiesForOwner(uid,function(list){
    ...
    // FIX 2026-08-18: registra edição de agendamento no feed ADM.
    try{
      if(typeof logFeedEvent==='function' && S && S.userId){
        var _detEd=act.scheduledAt?('Para '+(new Date(act.scheduledAt).toLocaleString('pt-BR',{day:'2-digit',month:'2-digit',hour:'2-digit',minute:'2-digit'}))):'Sem horário';
        logFeedEvent('act_edit',S.userId,desc,_detEd,'agenda');
      }
    }catch(_e){}
    closeM('mo-act-edit');
    renderActPanel();updateActBadge();refreshLinkedActivitySummaries();
...
function saveQuickActivity(){
  ...
  var actNow=new Date().toISOString();
  var act={id:'qa_'+Date.now()+'_'+Math.random().toString(36).slice(2,5),type:_qaType,desc:desc,scheduledAt:sched,done:false,read:false,createdAt:actNow,updatedAt:actNow,userId:uid,clientId:id,clientNome:c?c.name:'',board:board};
  function _finishSave(){
    ...
    // FIX 2026-08-18: registra lembrete rápido (do card) no feed ADM.
    try{
      if(typeof logFeedEvent==='function' && S && S.userId){
        var _cn=(c&&c.name)||'card';
        var _det='Lembrete em '+(new Date(sched).toLocaleString('pt-BR',{day:'2-digit',month:'2-digit',hour:'2-digit',minute:'2-digit'}))+' · '+desc;
        logFeedEvent('act_create',S.userId,_cn,_det,board);
      }
    }catch(_e){}
...
function renderActPanel(){
  ...
  // [FIX 20260820] Atividade vinculada a um card já excluído, ou já
  // descartado/fechado/convertido, nunca deve aparecer aqui como pendência
  // — mesma regra já aplicada no painel de Atividades do ADM/Time
  // (_admAtivIsOrphan, js/relatorios.js). Este painel (o "sino"/alarme que
  // todo usuário vê) usava uma lista separada sem essa proteção, então um
  // lead/negócio já excluído continuava aparecendo "atrasado" aqui mesmo
  // depois de já não existir mais em lugar nenhum.
  if(typeof _isActivityOrphanOrInactive==='function'){
    acts = acts.filter(function(a){ return a && !_isActivityOrphanOrInactive(a, a._ownerId); });
  }
  ...
  if(typeof _actPanelOnlyLate==='undefined')window._actPanelOnlyLate=false;
  if(_actPanelOnlyLate) acts = acts.filter(function(a){return a && !a.done && a.scheduledAt && _isScheduledExpired(a.scheduledAt);});
  _actPanelLastList = acts; // pra actItemClick/actConfirmDone acharem o item certo, seja de quem for
  var _actHdr='<div style="padding:6px 14px;border-bottom:1px solid var(--b1);display:flex;gap:6px;align-items:center">'
    +'<button class="bc" style="font-size:.68rem;padding:4px 10px;width:auto" onclick="event.stopPropagation();openActBulkEdit()">Editar em lote</button>'
    +'<button class="bc" style="font-size:.68rem;padding:4px 10px;width:auto;'+(_actPanelOnlyLate?'border-color:var(--rl);color:var(--rl);background:rgba(220,60,60,.14);':'')+'" onclick="event.stopPropagation();toggleActLateFilter()" title="Mostrar somente atrasadas">\u23f0 Atrasadas</button></div>';
  if(!acts.length){el.innerHTML=_actHdr+'<div class="act-empty">'+(_actPanelOnlyLate?'Nenhuma atividade atrasada.':'Nenhuma atividade ainda.')+'</div>';return;}
  var im={call:'📞',meet:'📅',task:'✅',note:'📋'};
  el.innerHTML=_actHdr
...
function actItemClick(id){
  ...
  if(a.clientId&&a.board&&typeof openKBDet==='function'){
    try{
      var arr=getKBFor(a.board,ownerId)||[];
      if(arr.some(function(x){return String(x.id)===String(a.clientId);})){
        var ap=document.getElementById('act-panel');if(ap)ap.classList.remove('open');
        openKBDet(a.clientId,a.board,ownerId);
        return;
      }
    }catch(_e){}
    toast('O card vinculado a esta atividade n\u00e3o foi encontrado.');
    return;
  }
  toast('Esta atividade n\u00e3o est\u00e1 vinculada a um lead.');
}



function toggleActLateFilter(){
  window._actPanelOnlyLate=!window._actPanelOnlyLate;
  renderActPanel();
}

function actDoneFromPanel(id){
  var _a=null;
  try{_a=(_actPanelLastList||[]).find(function(x){return x.id===id;});}catch(_e){}
  var ownerId=(_a&&_a._ownerId)||(S&&S.userId);
  var canManage=(ownerId===(S&&S.userId))||(typeof hasAdminAccess==='function'&&hasAdminAccess());
  if(!canManage){toast('\ud83d\udccb S\u00f3 o respons\u00e1vel ou um ADM pode concluir.');return;}
  actConfirmDone(id,ownerId);
}
...
function actConfirmDone(id,ownerId){
  ...
  _loadActivitiesForOwner(ownerId,function(list){
    ...
    a.done=true;a.doneAt=new Date().toISOString();a.updatedAt=new Date().toISOString();
    // [FIX 20261008] registra no cinto de segurança persistente — ver
    // _lfMarkRecentlyDone/_lfIsRecentlyDone em js/utils.js. Protege
    // contra um ciclo de sincronização do kanban, logo em seguida,
    // trazer de volta uma cópia desatualizada (done:false) do espelho
    // card.activities, vinda de uma resposta do servidor que ainda não
    // processou esta conclusão (o PUT é assíncrono, a conclusão em si
    // é local-first).
    if(typeof window._lfMarkRecentlyDone==='function')window._lfMarkRecentlyDone(id);
    // FIX: propaga "done" para o espelho legado dentro de card.activities no
    // kanban, para que o filtro "somente atrasadas" (_kbHasOverdueLinkedActivity)
    // enxergue imediatamente a atividade como concluída sem depender de reload.
    try{
      if(a.clientId && a.board){
        var kbUid = ownerId || (S && S.userId);
        var arr = getKBFor(a.board, kbUid) || [];
        ...
        if(card){
          if(Array.isArray(card.activities)){
            ...
            if(legacy){
              legacy.done = true;
              legacy.doneAt = a.doneAt;
            }
          }
          // [FIX 20260829] Pedido explícito: conclusão de atividade agora
          // fica registrada pra sempre no histórico do card, não só na
          // lista separada de Atividades (que pode não ter o mesmo tipo de
          // garantia de permanência que o histórico do card já tem).
          if(typeof _pushHistorico==='function'){
            _pushHistorico(card,'Atividade concluída: "'+(a.desc||a.tipo||'atividade')+'"');
          }
          saveKBFor(a.board, kbUid, arr);
        }
      }
    }catch(_e){}
    ...
    // FIX 2026-08-18: registra conclusão de agendamento no feed ADM.
    try{
      if(typeof logFeedEvent==='function' && S && S.userId){
        logFeedEvent('act_done',S.userId,(a&&a.desc)||'atividade','Concluída',(a&&a.board)||'agenda');
      }
    }catch(_e){}
    renderActPanel();updateActBadge();refreshLinkedActivitySummaries();toast('Concluída!');
    // [FIX 20261012] refreshLinkedActivitySummaries só atualiza o resumo
    // dentro de um modal aberto — nunca redesenha o quadro do Kanban em
    // si. Sem isto, o card continuava mostrando "atrasada" (indicador
    // vermelho + filtro) até o próximo ciclo de sincronização, mesmo já
    // concluído localmente. Só redesenha se a página do board estiver
    // visível — mesma checagem já usada em outros pontos deste arquivo.
    try{
      if(a && a.board){
        var _pg=document.getElementById('pg-'+a.board);
        if(_pg&&_pg.classList.contains('on')&&typeof renderKBLocal==='function')renderKBLocal(a.board);
      }
    }catch(_e){}
  });
}
...
function updateActBadge(){
  ...
  // [FIX 20260820] mesma exclusão de órfãs — sem isso o sino continuava
  // "tocando"/contando pra atividades de leads/negócios já excluídos.
  var uid0=(S&&S.userId)||null;
  var unread=acts.filter(function(a){
    if(!a||a.read||a.done)return false;
    if(typeof _isActivityOrphanOrInactive==='function'&&_isActivityOrphanOrInactive(a,uid0))return false;
    return true;
  }).length;
...
function fireNativeNotification(title,body,tag,kind){
  try{
    var notifKind=kind;
    if(notifKind!=='chat'&&notifKind!=='late'&&notifKind!=='geral'){
      if(/^💬/.test(title||''))notifKind='chat';
      else if(/^⏰/.test(title||''))notifKind='late';
      else notifKind='geral';
    }
    if(typeof _playNotifSound==='function')_playNotifSound(notifKind);/* R15-03: som na notificação nativa */
...
function showActAlert(act){
  /* LF-FIX-3BUGS-v1-20260819 #2: nao dispara alerta/bolinha vermelha para atividade ja
     concluida — cobre a janela em que o servidor devolveu versao velha
     (done:false) enquanto o PUT da conclusao ainda nao foi confirmado. */
  try{
    if(!act||act.done||act._doneLocalAt)return;
    var _al=(typeof getActivities==='function'?getActivities():[]).find(function(x){return x.id===act.id;});
    if(_al&&(_al.done||_al._doneLocalAt))return;
  }catch(_e){}
  ...
  fireNativeNotification('⏰ Atividade atrasada',body,'act-'+act.id,'late');
}
...
function checkUpcomingActs(){
  ...
  acts.forEach(function(a){
    if(!a)return;
    /* BUG #3 (2026-08-19): a guarda passa a ser POR ITEM, não só por
       pendência global do uid. Antes, quando o PUT ao Worker limpava
       _pending antes do badge reagrupar, abria-se uma janela em que
       item já concluído localmente disparava som/vibração. Agora
       pula qualquer item concluído, com gravação em voo ou com
       conclusão local ainda não confirmada. */
    if(a.done||a._pending||a._doneLocalAt)return;
    // [FIX 20260820] nunca dispara alerta pra atividade cujo card já foi
    // excluído/descartado/fechado — mesma proteção de renderActPanel.
    if(typeof _isActivityOrphanOrInactive==='function'&&_isActivityOrphanOrInactive(a,uid))return;
...
function scheduleNativeNotif(act){
  ...
  var appName=(window.LiderCRM&&window.LiderCRM.config&&window.LiderCRM.config.appShortName)||'LIDER CRM';
  setTimeout(function(){if(Notification.permission==='granted'){var ic=(ACT_TYPES[act.type]||{ic:'🔔'}).ic;new Notification(appName+' - '+ic,{body:(act.clientNome?act.clientNome+': ':'')+act.desc,tag:act.id,requireInteraction:true});}},diff);
}
...
function saveTlActivity(){
  ...
  /* [FIX 20261101] Bug 2 causa 2.2 — timeline do cliente (Bingo) gravava
     a atividade SEM o campo board, e o filtro "Atrasadas" tratava
     "a.board vazio" como "casa com qualquer quadro", fazendo cards
     aparecerem como atrasados no quadro errado. Aqui derivamos o board
     do próprio registro do Bingo (sourceOriginalLeadId => leads,
     sourceCardId => negocios) — mesma precedência já usada em
     clientes.js/openKBDet. Se não houver vínculo (registro avulso do
     Bingo), fica null, e aí o novo guarda em _kbHasOverdueLinkedActivity
     também impede o match acidental. */
  var tlBoard=null;
  if(c.sourceOriginalLeadId)tlBoard='leads';
  else if(c.sourceCardId)tlBoard='negocios';
  var tlClientId=(c.sourceOriginalLeadId||c.sourceCardId||_tlCid);
  var act={id:'a_'+Date.now(),type:_tlActType,desc:desc,scheduledAt:sched||null,done:false,by:(S&&S.nome)||'?',byId:(S&&S.userId)||null,createdAt:new Date().toISOString(),board:tlBoard||undefined};
  c.activities.unshift(act);saveCli(uid,list);
  var acts=getActivities();
  var storeAct={id:act.id,type:act.type,desc:act.desc,scheduledAt:sched||null,done:false,read:false,createdAt:act.createdAt,userId:(S&&S.userId)||null,clientId:tlClientId,clientNome:c.nome};
  if(tlBoard)storeAct.board=tlBoard;
  acts.unshift(storeAct);saveActivities(acts);
...
function markTlActDone(cid,actId){
  ...
  var a=c.activities.find(function(x){return x.id===actId;});if(!a)return;
  var doneAtISO=new Date().toISOString();
  a.done=true;a.doneAt=doneAtISO;a.updatedAt=doneAtISO;
  /* [FIX 20261101] conclusão pela timeline do cliente — mesmo cinto de
     segurança já aplicado em actConfirmDone: marca no _lfMarkRecentlyDone
     (persistente, 7 dias) e no _doneLocalAt (flag respeitada por
     checkUpcomingActs/showActAlert), impedindo que um ciclo de sync do
     kanban traga de volta a versão desatualizada (done:false) do espelho
     card.activities e reviva a atividade como "atrasada". */
  if(typeof window._lfMarkRecentlyDone==='function')window._lfMarkRecentlyDone(actId);
  a._doneLocalAt=Date.now();
  saveCli(uid,list);
  var acts=getActivities();var ga=acts.find(function(x){return x.id===actId;});
  if(ga){
    ga.done=true;
    ga.doneAt=doneAtISO;
    ga.updatedAt=doneAtISO;
    ga._doneLocalAt=Date.now();
    saveActivities(acts);
  }
...
function _actBulkRenderList(){
  ...
  el.innerHTML=acts.map(function(a){
    ...
    return '<label class="act-bulk-sel"><input type="checkbox" value="'+_htmlAttr(a.id)+'" onchange="toggleActBulkSel(\''+_jsSq(a.id)+'\')"><div class="act-bulk-sel-info"><div class="act-bulk-sel-desc">'+ic+' '+eH(a.desc)+'</div><div class="act-bulk-sel-meta">'+(a.clientNome?eH(a.clientNome)+' · ':'')+(boardLbl?boardLbl+' · ':'')+late+dt+'</div></div></label>';
...
function applyActBulkDate(){
  ...
  var acts=getActivities();_actBulkSel.forEach(function(id){var a=acts.find(function(x){return x.id===id;});if(a){a.scheduledAt=dt;a.read=false;a.updatedAt=new Date().toISOString();}});
...
function applyActBulkDesc(){
  ...
  _actBulkSel.forEach(function(id){var a=acts.find(function(x){return x.id===id;});if(a){a.desc=desc;a.updatedAt=new Date().toISOString();n++;}});
...
function applyActBulkDone(){
  ...
  var acts=getActivities();
  var touchedCards={}; // board|uid|cardId -> {arr, card} — evita reler/regravar o mesmo card várias vezes
  _actBulkSel.forEach(function(id){
    var a=acts.find(function(x){return x.id===id;});
    if(!a)return;
    var doneAtISO=new Date().toISOString();
    a.done=true;a.doneAt=doneAtISO;a.updatedAt=doneAtISO;
    /* [FIX 20261101] conclusão em lote — mesmo cinto de segurança já
       aplicado em actConfirmDone: marca no _lfMarkRecentlyDone
       (persistente, 7 dias) e no _doneLocalAt (respeitado por
       checkUpcomingActs/showActAlert). Sem isso, um ciclo de sincronização
       do kanban logo em seguida podia trazer de volta o espelho legado
       card.activities com done:false e o filtro "Atrasadas" revivia a
       atividade que acabou de ser concluída em lote. */
    if(typeof window._lfMarkRecentlyDone==='function')window._lfMarkRecentlyDone(id);
    a._doneLocalAt=Date.now();
    // [FIX 20260829] Mesma correção de actConfirmDone — grava a conclusão
    // permanentemente no histórico do card, não só na lista de Atividades.
    try{
      if(a.clientId&&a.board){
        var key=a.board+'|'+(S&&S.userId)+'|'+a.clientId;
        var t=touchedCards[key];
        if(!t){
          var arr=getKBFor(a.board,S&&S.userId)||[];
          var card=arr.find(function(x){return String(x.id)===String(a.clientId);});
          if(card){t=touchedCards[key]={arr:arr,card:card};}
        }
        if(t){
          if(Array.isArray(t.card.activities)){
            var legacy=t.card.activities.find(function(x){return String(x.id)===String(a.id);});
            if(legacy){legacy.done=true;legacy.doneAt=a.doneAt;}
          }
          if(typeof _pushHistorico==='function')_pushHistorico(t.card,'Atividade concluída: "'+(a.desc||a.tipo||'atividade')+'"');
        }
      }
    }catch(_e){}
  });
  Object.keys(touchedCards).forEach(function(key){
    var t=touchedCards[key];
    saveKBFor(t.card.board||key.split('|')[0],(S&&S.userId),t.arr);
  });
...
function deleteActBulk(){
  ...
  _confirmModal({
    ...
    okClass:'bd',
    onOk:function(){
      // FIX 2026-08-18: registra exclusão em lote no feed ADM ANTES de filtrar (senão perdemos os nomes).
      try{
        if(typeof logFeedEvent==='function' && S && S.userId){
          var _all=getActivities();
          var _removidas=_all.filter(function(a){return _actBulkSel.indexOf(a.id)>=0;});
          _removidas.forEach(function(a){
            try{ logFeedEvent('act_delete',S.userId,(a&&a.desc)||'atividade','Excluída',(a&&a.board)||'agenda'); }catch(_e){}
          });
        }
      }catch(_e){}
      var acts=getActivities().filter(function(a){return _actBulkSel.indexOf(a.id)<0;});
      /* [FIX 20260907] Propagação de exclusões multi-dispositivo: marca
         cada id excluído no tombstone compartilhado (mapa de 7 dias de
         js/utils.js). O activities-store coleta esses ids como
         deletedIds no próximo PUT, e o Worker (merge por id) os remove
         do documento do servidor — sem isso, uma exclusão feita aqui
         "voltava" em qualquer outro dispositivo, porque o merge é
         conservador e não deleta por omissão. */
      if(typeof window._lfMarkRecentlyDeleted==='function'){
        _actBulkSel.forEach(function(id){try{window._lfMarkRecentlyDeleted(id);}catch(_eDel){}});
      }
...
/* Acumulador do dia (total de ligações + nº de rodadas de bingo completas),
   nativo desde 2026-08-17 — antes vivia só como wrapper externo
   (js/patches/kanban-leads/lf-lig-counter-rounds-v1-20260728.js) por cima
   de toggleLig/resetLig. Absorvido pra dentro porque o comportamento do
   contador mudou nesta rodada (reinício automático ao bater 10, sem
   precisar mais do botão manual) — manter como wrapper por fora ia exigir
   reescrever o mesmo wrapper de novo. A API pública LiderCRM.ligCounterRounds
   é mantida (o patch de calendário do feed ADM ainda a consome). O patch de
   sincronia em nuvem (lf-lig-counter-sync-cloud-v1) continua rodando por
   fora, sem mudança — ele só observa saveLigToday, que não mudou de forma. */
function _ligTotalKey(uid,date){return 'lf13_lig_total_'+(uid||(S&&S.userId)||'')+'_'+(date||today());}
function _ligRoundsKey(uid,date){return 'lf13_lig_rounds_'+(uid||(S&&S.userId)||'')+'_'+(date||today());}
function getLigTotal(uid,date){var v=sg(_ligTotalKey(uid,date));var n=(typeof v==='number')?v:parseInt(v,10);return (Number.isFinite(n)&&n>=0)?n:0;}
function getLigRounds(uid,date){var v=sg(_ligRoundsKey(uid,date));var n=(typeof v==='number')?v:parseInt(v,10);return (Number.isFinite(n)&&n>=0)?n:0;}
function _setLigTotal(uid,date,v){ss(_ligTotalKey(uid,date),Math.max(0,v|0));}
function _setLigRounds(uid,date,v){ss(_ligRoundsKey(uid,date),Math.max(0,v|0));}
window.LiderCRM=window.LiderCRM||{};
window.LiderCRM.ligCounterRounds={
  getLigTotal:getLigTotal,
  getLigRounds:getLigRounds,
  resetDay:function(uid,date){_setLigTotal(uid,date,0);_setLigRounds(uid,date,0);}
};
...
function initLigWidget(){
  ...
  // FIX-PERF-LIG-DRAG: antes o mousemove/touchmove atribuía style.left/top em CADA
  // movimento do mouse (a 60-120Hz em hardware bom), cada write forçando reflow do
  // viewport inteiro — o dedo "ganhava vassourinha" e o relógio da página batia 30fps.
  // Agora guardamos só a posição mais recente e escrevemos de fato 1 vez por animation
  // frame (best case 60fps, pior caso mais liso porque coalesce bursts).
  var _ligRAF=null,_ligLastX=0,_ligLastY=0;
  function _ligApply(){
    _ligRAF=null;
    if(!dragging)return;
    var vw=window.innerWidth,vh=window.innerHeight;
    var ww=w.offsetWidth||180,wh=w.offsetHeight||(w._lfH||220);if(w._lfH===undefined){var rr=w.getBoundingClientRect();w._lfH=rr.height;wh=rr.height;}
    w.style.left=Math.max(0,Math.min(vw-ww,_ligLastX-ox))+'px';
    w.style.top=Math.max(56,Math.min(vh-wh,_ligLastY-oy))+'px';
  }
  ...
  document.addEventListener('mousemove',function(e){if(!dragging)return;_ligLastX=e.clientX;_ligLastY=e.clientY;if(_ligRAF)return;_ligRAF=requestAnimationFrame(_ligApply);});
  document.addEventListener('mouseup',function(){dragging=false;if(_ligRAF){cancelAnimationFrame(_ligRAF);_ligRAF=null;}});
  hd.addEventListener('touchstart',function(e){var t=e.touches[0];var r=w.getBoundingClientRect();ox=t.clientX-r.left;oy=t.clientY-r.top;dragging=true;},{passive:true});
  hd.addEventListener('touchmove',function(e){var t=e.touches[0];_ligLastX=t.clientX;_ligLastY=t.clientY;if(_ligRAF)return;_ligRAF=requestAnimationFrame(_ligApply);e.preventDefault();},{passive:false});
  // Reseta o último offsetHeight cacheado em resize (evita layout com números velhos)
  window.addEventListener('resize',function(){w._lfH=undefined;},{passive:true});
}

function buildLigGrid(){
  var g=document.getElementById('lig-grid');if(!g)return;
  /* CORREÇÃO (2026-08-17): antes esta função fazia g.innerHTML='' e
     recriava os 10 <button> a CADA toque — o botão recém-clicado ficava
     órfão (fora do documento) no instante em que o listener global de
     "fechar ao clicar fora" (configuracoes.js) checava
     w.contains(e.target): como o nó antigo já não pertencia mais a
     #lig-widget (foi substituído por um novo), a checagem dava falso e
     o widget fechava sozinho a cada marcação. Agora os botões são
     criados 1 vez só; toques seguintes apenas alternam classes nos
     mesmos nós, sem nunca desconectá-los do documento. */
  if(g.children.length!==10){
    g.innerHTML='';
    for(var i=1;i<=10;i++){(function(n){var cell=document.createElement('button');cell.className='lig-cell';cell.textContent=n;cell.addEventListener('click',function(){toggleLig(n);});g.appendChild(cell);})(i);}
  }
  Array.prototype.forEach.call(g.children,function(cell,idx){
    var n=idx+1;
    cell.classList.toggle('marked',_ligMarked.indexOf(n)>=0);
  });
  ...
  if(cnt===10)Array.prototype.forEach.call(g.children,function(c){c.classList.add('bingo');});
  else Array.prototype.forEach.call(g.children,function(c){c.classList.remove('bingo');});
}
...
function toggleLig(n){
  ...
  var beforeLen=_ligMarked.length;
  ...
  saveLigToday(list);
  var afterLen=_ligMarked.length;
  var delta=afterLen-beforeLen;
  if(delta>0){
    var _novoTotalHoje=getLigTotal(uid,day)+1;
    _setLigTotal(uid,day,_novoTotalHoje);
    // [FIX 20260820] Antes só o BINGO (rodada fechada, 10/10) gerava
    // evento no feed — qualquer dia com ligações avulsas (sem fechar
    // rodada completa) não deixava rastro nenhum no Feed do ADM/Time,
    // mesmo o filtro de data já funcionando certinho (conferido: o bug
    // nunca foi no filtro, era ausência de dado pra filtrar). Agora
    // toda ligação marcada grava um evento leve — some com o "detail"
    // vazio (não precisa detalhe extra) pra não pesar a lista, mas
    // aparece na busca/filtro por data e por canal="chamada".
    if(typeof logFeedEvent==='function'){
      logFeedEvent('lig_call',uid,'Ligação '+_novoTotalHoje+' do dia','','agenda','chamada');
    }
    // BINGO: transição limpa de <10 pra 10 (marcar a 10ª célula).
    if(beforeLen<10&&afterLen===10){
      var rounds=getLigRounds(uid,day)+1;
      _setLigRounds(uid,day,rounds);
      var total=getLigTotal(uid,day);
      // Duração da rodada: da 1ª marcação até a 10ª (ordena por horário,
      // não por ordem de clique — cobrir o caso de desmarcar/remarcar
      // fora de ordem antes de completar).
      var horas=list.map(function(r){return r.hora;}).filter(Boolean).sort();
      var durTxt='';
      if(horas.length>=2){
        var ms=new Date(horas[horas.length-1]).getTime()-new Date(horas[0]).getTime();
        if(ms>=0){
          var mins=Math.floor(ms/60000),secs=Math.round((ms%60000)/1000);
          durTxt=mins>0?(mins+'min '+secs+'s'):(secs+'s');
        }
      }
      if(typeof logFeedEvent==='function'){
        logFeedEvent('lig_bingo',uid,rounds+'ª rodada (10/10)',
          'Acumulado do dia: '+total+' ligações em '+rounds+' rodada'+(rounds!==1?'s':'')+(durTxt?' · Levou '+durTxt+' da 1ª à 10ª ligação':''),
          'agenda','chamada');
      }
      buildLigGrid();
      setTimeout(function(){
        toast('BINGO! 10 ligações!');
        // Reinício automático — o botão manual "Reiniciar" deixou de
        // existir a pedido: o acumulador do dia (total/rounds) NÃO zera
        // aqui, só a rodada visível no grid.
        _ligMarked=[];saveLigToday([]);buildLigGrid();
      },900);
      return;
    }
  }else if(delta<0){
    var t=getLigTotal(uid,day);if(t>0)_setLigTotal(uid,day,t-1);
  }
  buildLigGrid();
}
...
var AGD_POLL_MS=(window.LiderCRM&&window.LiderCRM.config&&window.LiderCRM.config.sync&&Number(window.LiderCRM.config.sync.agendaPollMs))||6000;
...
function agdFillConsultorFilter(){
  ...
  sel.innerHTML='<option value="">Todos os consultores</option>'+users.map(function(u){
    /* AUDITORIA-FINAL-10 (2026-08-01, item 2.5): u.id agora escapado com
       _htmlAttr(), mesmo padrão já usado em agdFillConsultorSelect() logo
       abaixo — antes era o único dos dois que não escapava. */
    return '<option value="'+_htmlAttr(u.id)+'">'+_agdEsc(u.nome||'?')+'</option>';
  }).join('');
  sel.value=cur||'';
}
...
function agdFillConsultorSelect(){
  ...
  var users=getUsers().filter(function(u){return u.ativo!==false;});
  sel.innerHTML=users.map(function(u){return '<option value="'+_htmlAttr(u.id)+'">'+_agdEsc(u.nome||'?')+'</option>';}).join('');
}
...
/* [FIX 20260822] Indicador de Comparecimento (Show/No-Show) — pedido
   explícito, com fórmulas exatas fornecidas:
     TOTAL = agendamentos no período que já deveriam ter acontecido
             (data+hora no passado — agendamento futuro não conta,
             ainda não teve chance de comparecer ou faltar).
     SHOW = quantos têm status "Atendido".
     NO-SHOW = TOTAL - SHOW (inclui Remarcar e Agendado vencido nunca
               atualizado — não é só quem tem status "No-Show" marcado
               manualmente, é TUDO que não foi "Atendido").
     Taxa Show (%) = SHOW ÷ TOTAL × 100.
     Taxa No-Show (%) = NO-SHOW ÷ TOTAL × 100 (= 100 − Taxa Show).
   Arredondamento: SHOW/NO-SHOW são sempre inteiros (contagem de
   pessoas) — não tem arredondamento aí, são contagens exatas. Só as
   taxas percentuais são arredondadas (1 casa decimal). */
function _agdNowIso(){
  var n=new Date();
  return today()+'T'+String(n.getHours()).padStart(2,'0')+':'+String(n.getMinutes()).padStart(2,'0');
}
function _agdShowNoShowMetrics(startDate,endDate){
  var consF=(document.getElementById('agd-filter-cons')||{}).value||'';
  var nowIso=_agdNowIso();
  var base=(_agdCache||[]).filter(function(a){
    if(!a||!a.data)return false;
    if(consF&&a.consultorId!==consF)return false;
    if(!agdInRange(a.data,startDate,endDate))return false;
    var dataHora=a.data+'T'+(a.hora||'23:59');
    return dataHora<=nowIso; // só conta o que já deveria ter acontecido
  });
  var total=base.length;
  var show=base.filter(function(a){return a.status==='atendido';}).length;
  var noShow=total-show;
  var showRate=total>0?Math.round((show/total)*1000)/10:0;
  var noShowRate=total>0?Math.round((noShow/total)*1000)/10:0;
  return {total:total,show:show,noShow:noShow,showRate:showRate,noShowRate:noShowRate};
}
function agdToggleShowRatePanel(force){
  var panel=document.getElementById('agd-showrate-panel');if(!panel)return;
  var btn=document.getElementById('agd-showrate-btn');
  var open=(force!==undefined)?!!force:!panel.classList.contains('open');
  panel.classList.toggle('open',open);
  if(btn)btn.classList.toggle('agd-showrate-btn-on',open);
  if(open)agdRenderShowRatePanel();
}
if(!window.__lfAgdShowRateOutsideClick){
  window.__lfAgdShowRateOutsideClick=true;
  document.addEventListener('click',function(e){
    var panel=document.getElementById('agd-showrate-panel');
    var btn=document.getElementById('agd-showrate-btn');
    if(!panel||!panel.classList.contains('open'))return;
    if(panel.contains(e.target)||(btn&&btn.contains(e.target)))return;
    agdToggleShowRatePanel(false);
  },{capture:true});
}
function agdRenderShowRatePanel(){
  var tbody=document.getElementById('agd-showrate-tbody');if(!tbody)return;
  var t=today();
  var weekStart=agdAddDays(t,-6);
  var monthPrefix=t.slice(0,7);
  var yearPrefix=t.slice(0,4);
  var fromEl=document.getElementById('agd-showrate-from');
  var toEl=document.getElementById('agd-showrate-to');
  var customFrom=(fromEl&&fromEl.value)||null;
  var customTo=(toEl&&toEl.value)||null;
  var rows=[
    {label:'Hoje',m:_agdShowNoShowMetrics(t,t)},
    {label:'Semana',m:_agdShowNoShowMetrics(weekStart,t)},
    {label:'Mês',m:_agdShowNoShowMetrics(monthPrefix+'-01',t)},
    {label:'Ano',m:_agdShowNoShowMetrics(yearPrefix+'-01-01',t)}
  ];
  if(customFrom&&customTo){
    rows.push({label:'Intervalo',m:_agdShowNoShowMetrics(customFrom,customTo)});
  }
  function _rateColor(rate){return rate>=60?'var(--ok,#2ecfa0)':(rate>=35?'var(--al,#c39a2d)':'var(--rl,#e05c5c)');}
  tbody.innerHTML=rows.map(function(r){
    var m=r.m;
    return '<tr><td>'+eH(r.label)+'</td>'
      +'<td>'+m.show+'</td>'
      +'<td>'+m.noShow+'</td>'
      +'<td>'+m.total+'</td>'
      +'<td style="color:'+_rateColor(m.showRate)+'">'+m.showRate.toFixed(1).replace('.',',')+'%</td>'
      +'<td style="color:'+_rateColor(100-m.noShowRate)+'">'+m.noShowRate.toFixed(1).replace('.',',')+'%</td>'
      +'</tr>';
  }).join('');
}
...
function agdRenderKPIs(){
  ...
  var monthPrefix=t.slice(0,7);
  /* FIX (2026-08-20): KPIs agora respeitam os filtros de CONSULTOR e STATUS
     do topo da agenda. Ex.: ao selecionar "Rhuan" + status "Atendido", o card
     "Este m\u00eas" passa a mostrar somente os atendidos do Rhuan no m\u00eas atual,
     e assim por diante para Hoje / \u00daltimos 7 dias / 7 dias anteriores. */
  var consF=(document.getElementById('agd-filter-cons')||{}).value||'';
  var stF=(document.getElementById('agd-filter-status')||{}).value||'';
  function _agdKpiMatch(a){
    if(consF&&a.consultorId!==consF)return false;
    if(stF&&a.status!==stF)return false;
    return true;
  }
  var base=_agdCache.filter(_agdKpiMatch);
  var hoje=base.filter(function(a){return a.data===t;}).length;
  var semana=base.filter(function(a){return agdInRange(a.data,weekStart,t);}).length;
  var semanaAnt=base.filter(function(a){return agdInRange(a.data,lastWeekStart,lastWeekEnd);}).length;
  var mes=base.filter(function(a){return (a.data||'').indexOf(monthPrefix)===0;}).length;
...
function agdRenderList(){
  ...
  el.innerHTML=items.map(function(a){
    var st=Object.prototype.hasOwnProperty.call(AGD_STATUS_LBL,a.status)?a.status:'agendado';
...
function agdSave(){
  /* R5-5: null guards — se o modal for fechado inesperadamente enquanto o JS
     está rodando (edge case em Capacitor ao minimizar o app), getElementById
     pode retornar null e o acesso direto a .value lançaria TypeError, travando
     o handler sem feedback para o usuário. */
  var _fData=document.getElementById('agd-f-data');
  var _fHora=document.getElementById('agd-f-hora');
  var _fCons=document.getElementById('agd-f-cons');
  var _fCli=document.getElementById('agd-f-cli');
  var _fNicho=document.getElementById('agd-f-nicho');
  var _fStatus=document.getElementById('agd-f-status');
  var _fObs=document.getElementById('agd-f-obs');
  var _fId=document.getElementById('agd-edit-id');
  if(!_fData||!_fHora||!_fCons||!_fCli||!_fNicho||!_fStatus){toast('⚠️ Formulário não está pronto. Tente fechar e abrir novamente.');return;}
  var data=_fData.value;
  var hora=_fHora.value;
  var consultorId=_fCons.value;
  var cliente=_fCli.value.trim();
  var nicho=_fNicho.value;
  var status=_fStatus.value;
  var obs=_fObs?_fObs.value.trim():'';
  var id=_fId?_fId.value:'';
...
function agdDelete(){
  ...
  _confirmModal({
    ...
    onOk:function(){
      syncBusy();
      // FIX 2026-08-18: registra exclusão de slot de agenda no feed ADM.
      try{
        if(typeof logFeedEvent==='function' && S && S.userId){
          var _slot=(_agdCache||[]).find(function(a){return a._id===id;});
          var _lbl=_slot?((_slot.clienteNome||_slot.titulo||'agendamento')+(_slot.data?(' · '+_slot.data+(_slot.hora?(' '+_slot.hora):'')):'')):'agendamento';
          logFeedEvent('act_delete',S.userId,_lbl,'Slot de agenda excluído','agenda');
        }
      }catch(_e){}
      var wc=_agdWc();
      if(wc){
