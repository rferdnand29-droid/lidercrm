...
// ============================================================
// EVOLUTION API — proxy server-side
// ============================================================
// A URL, a chave e a instância agora ficam no Worker. O navegador chama
// apenas /api/v1/whatsapp/* e nunca recebe a credencial do provedor.
var EVOLUTION_INSTANCE = "lidercrm";

function _evoApi(){
  try {
    return window.LiderCRM && window.LiderCRM.api &&
      typeof window.LiderCRM.api.request === 'function'
      ? window.LiderCRM.api.request : null;
  } catch(_e) { return null; }
}
function evoInstanceReady(){return !!(EVOLUTION_INSTANCE&&_evoApi());}
...
function evoSendText(tel,text,cb){
  ...
  _evoApi()('/api/v1/whatsapp/send',{
    method:'POST',
    body:{number:number,text:text}
  }).then(function(r){
    if(!r||!r.ok)throw new Error((r&&r.message)||'Evolution indisponível');
    cb(null,r.data);
  }).catch(function(e){console.error('evoSendText',e);cb(e);});
}
...
function evoCheckConnection(cb){
  cb=cb||function(){};
  if(!evoInstanceReady()){cb('evolution-nao-configurado');return;}
  _evoApi()('/api/v1/whatsapp/status',{
    method:'GET'
  }).then(function(r){
    if(!r||!r.ok)throw new Error((r&&r.message)||'Evolution indisponível');
    cb(null,r.data);
