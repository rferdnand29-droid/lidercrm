...
/* =====================================================================
 * R7 — LF-NOTIF-401-NOLOGOUT-20260908
 * 401 em /api/v1/notificacoes/inbox NÃO é sessão inválida: devolve a
 * lista local e dispara /session/refresh em background (única rota com
 * janela de graça de 7 dias em middlewares/auth.js). O logout duro fica
 * reservado a 401 em /api/v1/session* — é o que impede o "fechou sozinho".
 * Idempotente: guard window.__LF_NOTIF_401_NOLOGOUT__.
 * ===================================================================== */
(function (global) {
  'use strict';
  if (global.__LF_NOTIF_401_NOLOGOUT__) return;
  global.__LF_NOTIF_401_NOLOGOUT__ = true;
  var TAG = '[lf-fix-raiz-definitivo:R7]';
  function warn(){ try{ console.warn.apply(console,[TAG].concat([].slice.call(arguments))); }catch(_){} }
  function log(){ try{ console.log.apply(console,[TAG].concat([].slice.call(arguments))); }catch(_){} }
  var origFetch = global.fetch;
  if (!origFetch || origFetch.__lfInbox401) return;
  var wrapped = function(input, init){
    var url = typeof input === 'string' ? input : (input && input.url) || '';
    var p = origFetch.apply(this, arguments);
    if (!/\/api\/v1\/notificacoes\/inbox/.test(url)) return p;
    return p.then(function(resp){
      if (!resp || resp.status !== 401) return resp;
      try {
        warn('inbox 401 — fallback local + refresh em background (sem logout):', url);
        try { origFetch.call(global, '/api/v1/session/refresh', { method:'POST' }); } catch(_){}
        var local = [];
        try {
          var m = url.match(/[?&]uid=([^&]+)/);
          var uid = m ? decodeURIComponent(m[1]) : ((global.S && global.S.userId) || '');
          var key = typeof global.notifKey === 'function' ? global.notifKey(uid) : ('lf_notif_' + uid);
          var parsed = JSON.parse(global.localStorage.getItem(key) || '[]');
          local = Array.isArray(parsed) ? parsed : [];
        } catch(_){}
        var body = JSON.stringify({ ok:true, data: local, meta:{ endpoint:'/api/v1/notificacoes/inbox', fallback:'401->local', marker:'LF-NOTIF-401-NOLOGOUT-20260908' } });
        return new Response(body, { status:200, headers:{'Content-Type':'application/json'} });
      } catch(_){ return resp; }
    });
  };
  wrapped.__lfInbox401 = true;
  global.fetch = wrapped;
  log('R7 ativo — 401 na inbox não derruba mais a sessão');
})(window);
