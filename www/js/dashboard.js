...
function renderDash(){
  ...
  var _uid=S.userId;
  loadCli(_uid,function(list){
    /* [FIX 20260903] O callback é assíncrono: se a sessão for encerrada
       (logout, 401 limpando a sessão) enquanto a lista carrega, `S` vira
       null e o acesso a S.userId estourava
       "Cannot read properties of null (reading 'userId')". Sai em silêncio
       — não há painel para desenhar sem sessão. */
    if(!S)return;
    var u=getUser(_uid);
    var e;
...
/* [FIX 20260909] CALIBRACAO — pedido explicito do usuario: "Agendado"
   deve contar SOMENTE quem, dependendo do dia, foi posto na etapa
   AG Video ('agvid') ou Presencial ('presencial') do Kanban de
   Negocios (ou etapa posterior). Nao basta ter sido convertido pra
   Negocio: quem esta em 'retag' (Retornar — negocio criado mas ainda
   sem data/agendamento) NAO conta como agendado. */
var KB_NEG_COLS_NAO_AGENDADO=['retag'];
function _countAgendadoNegocios(uids,range){
  if(typeof getKBFor!=='function')return 0;
  return (uids||[]).reduce(function(sum,uid){
    var arr=getKBFor('negocios',uid)||[];
    return sum+arr.filter(function(c){
      if(!c||KB_NEG_COLS_NAO_AGENDADO.indexOf(c.col)>=0)return false;
      if(!range)return true;
...
/* [FIX 20260909] Novas metricas do FUNIL, no lugar das etapas manuais
   "Audio 30s", "Conf. 24h" e "Conf. 2h" (pedido explicito do usuario):
   - "Convertido p/ Negocio": leads do board de Leads na etapa 'conv'
     (mesma regra da Taxa de Conversao, ja existente).
   - "Lead Cartela": Negocios na etapa 'cart' (Cartela) do Kanban.
   - "Retornar": Negocios na etapa 'retag' (Retornar — leads que
     pediriam retorno / precisam ser retornados). */
function _countCartelaNegocios(uids,range){
  if(typeof getKBFor!=='function')return 0;
  return (uids||[]).reduce(function(sum,uid){
    var arr=getKBFor('negocios',uid)||[];
    return sum+arr.filter(function(c){
      if(!c||c.col!=='cart')return false;
      if(!range)return true;
      var ref=c.stageEnteredAt||c.updatedAt||c.createdAt||c.data||null;
      return _isDateWithinRange(ref,range);
    }).length;
...
function _countRetornarNegocios(uids,range){
  if(typeof getKBFor!=='function')return 0;
  return (uids||[]).reduce(function(sum,uid){
    var arr=getKBFor('negocios',uid)||[];
    return sum+arr.filter(function(c){
      if(!c||c.col!=='retag')return false;
      if(!range)return true;
      var ref=c.stageEnteredAt||c.updatedAt||c.createdAt||c.data||null;
      return _isDateWithinRange(ref,range);
    }).length;
...
function _getClosingReconciliationMetrics(list,opts){
  ...
  /* [FIX 20260909] "Agendado" vem SOMENTE do Kanban de Negocios
     (quem foi posto em AG Video/Presencial ou alem, no periodo) —
     o toggle manual do Bingo (steps[0]) nao entra mais, pra nao
     contar lead apenas convertido pra negocio sem agendamento. */
  var ag=agKB;
  steps[0]=ag;
  ...
  steps[4]=com;
  /* [FIX 20260909] substitui as barras "Audio 30s", "Conf. 24h" e
     "Conf. 2h" por metricas reais do funil (pedido explicito). */
  steps[1]=_countLeadsConvertidos(opts.uids||[],range);
  steps[2]=_countCartelaNegocios(opts.uids||[],range);
  steps[3]=_countRetornarNegocios(opts.uids||[],range);
...
/* [FIX 20260909] Rotulos do FUNIL do Analytics (ETP continua sendo
   usado pelos toggles manuais da tela de Clientes/Bingo). */
var ETP_DASH=['Agendado','Convertido p/ Negócio','Lead Cartela','Retornar','Compareceu','Virou Ficha','Fechou'];
...
function drawAnal(list,kid,fid,svid,lgid){
  ...
  if(fEl){var max=Math.max(ag,1);fEl.innerHTML=ETP_DASH.map(function(l,i){var v=steps[i]||0;var w=Math.round(v/max*100);return '<div class="fi"><div class="fil">'+l+'</div><div class="fibw"><div class="fib" style="width:'+w+'%;background:'+ECR[i]+'"><span>'+v+'</span></div></div><div class="fip">'+w+'%</div></div>';}).join('');}
...
var _ANALYTICS_DRILL_DEFS={
  ...
  agendados:{board:'negocios',title:'Leads Agendados',pred:function(c){return KB_NEG_COLS_NAO_AGENDADO.indexOf(c.col)<0;},ref:function(c){return c.stageEnteredAt||c.updatedAt||c.createdAt||c.data||null;}},
