(function(global){
  ...
  function _linkedActCardHTML(a,done,opts){
    ...
    if(opts&&opts.canEdit&&!done){
      var _ownerForBtn=opts.ownerUid||global.S.userId;
      actions='<div class="qa-act-actions" style="display:flex;gap:6px;margin-top:8px">'
        ...
        +'<button class="bp" type="button" style="padding:6px 10px;font-size:.68rem;min-height:auto;width:auto;background:var(--ok);border-color:var(--ok)" onclick="actConfirmDone(\''+global._jsSq(a.id)+'\',\''+global._jsSq(_ownerForBtn)+'\')">✅ Concluir atividade</button>'
        +'<button class="bc" type="button" style="padding:6px 10px;font-size:.68rem;min-height:auto;width:auto;border-color:var(--rl);color:var(--rl)" onclick="deleteLinkedActivityById(\''+global._jsSq(a.id)+'\',\''+global._jsSq(opts.ownerUid||global.S.userId)+'\',\''+global._jsSq(opts.board||'')+'\',\''+global._jsSq(opts.cardId||'')+'\')">🗑 Excluir</button>'
        +'</div>';
    }
