(function(global){
  ...
  /* [FIX 20260907] Tombstones compartilhados: o mapa de "excluído
     recentemente" (7 dias) já existia em js/utils.js
     (window._lfMarkRecentlyDeleted/_lfIsRecentlyDeleted) como cinto de
     segurança do lado do cliente. Agora ele também alimenta o campo
     deletedIds do PUT /atividades/list: como o Worker passou a fazer
     MERGE por id (atividades-controller.js, mesmo fix), uma exclusão
     local deixava de ser propagada — "sumiu da lista" é ambíguo num
     modelo de merge e o servidor conservador não deleta por omissão.
     A chave de storage (lf_recently_deleted_ids_v1) é a mesma lida por
     _lfReadRecentlyDeletedMap em utils.js. Como o mapa é global (não
     por uid), filtramos pelo prefixo 'a_' usado nos ids de atividade
     (agenda.js) pra não vazar ids de leads/cards. Falha silenciosa é
     proposital: sem tombstone, o pior caso é o comportamento anterior
     (a exclusão não propaga — o merge conservador a mantém no servidor). */
  var LF_ACT_DEL_KEY = 'lf_recently_deleted_ids_v1';
  function _actTombstones(){
    try {
      var map = JSON.parse(global.localStorage.getItem(LF_ACT_DEL_KEY) || '{}') || {};
      var out = [];
      Object.keys(map).forEach(function(id){
        if (typeof id === 'string' && id.indexOf('a_') === 0) out.push(id);
      });
      return out;
    } catch(_e){ return []; }
  }
  function _unmarkActTombstone(id){
    if (!id) return;
    try {
      var map = JSON.parse(global.localStorage.getItem(LF_ACT_DEL_KEY) || '{}') || {};
      if (map[id]) { delete map[id]; global.localStorage.setItem(LF_ACT_DEL_KEY, JSON.stringify(map)); }
    } catch(_e){}
  }
  /* Auto-limpeza: um id que EXISTE na lista local não pode continuar
     marcado como excluído — senão o próximo PUT mandaria um deletedIds
     que apagaria o item no servidor (ex.: atividade recriada com o
     mesmo id, ou ressuscitada propositalmente pelo merge). Roda antes
     de montar cada payload remoto. */
  function _syncActTombstones(list){
    if (!Array.isArray(list)) return;
    for (var i = 0; i < list.length; i++){
      var a = list[i];
      if (a && a.id) _unmarkActTombstone(a.id);
    }
  }
  ...
  // ---------- (4) Retry automatico de itens _pending ---------- LF-FIX-3BUGS-v1-20260819 #2
  // Antes: se o PUT falhava, o item ficava com _pending preso e o servidor
  // devolvia a versao velha (ex.: done:false) sem que ninguem tentasse de
  // novo — a atividade concluida "voltava a ficar atrasada". Agora cada
  // falha agenda uma nova tentativa com backoff (3s/6s/9s/12s/15s), sem
  // reinicializar nada: re-grava a lista local atual via o proprio save.
  var _retryTimers = Object.create(null); // uid -> timeoutId
  var _retryCount  = Object.create(null); // uid -> tentativas
  function _schedulePendingRetry(uid){
    if (!uid || _retryTimers[uid]) return;
    var tries = (_retryCount[uid] || 0);
    /* [FIX 20260907] Bug 1 causa 1.3 — antes o retry PARAVA após 5
       tentativas (~45s no total). Qualquer falha de rede mais longa
       que isso (celular sem sinal, túnel, avião) deixava o item com
       _pending:true pra sempre no cache, sem NUNCA confirmar no
       servidor — e qualquer leitura em outro contexto (painel ADM,
       sino de colega, outro dispositivo) trazia a versão velha do
       servidor (ex.: done:false), fazendo a atividade concluída
       "voltar". Agora, ao esgotar as 5 tentativas rápidas:
       1) enfileira a lista atual em LF.enqueueActivities — o
          SyncManager (retry-queue-sync.js) drena a fila a cada ~15s,
          em 'online' e em 'visibilitychange', com até 30 tentativas
          e backoff exponencial próprio;
       2) continua re-agendando localmente a cada 60s, SEM TETO, até
          _hasPending(uid) ficar falso (ou seja: até o servidor
          confirmar). O céu é o limite de propósito — o custo é um
          timer ocioso por uid, e ele para sozinho assim que a fila
          esvazia. */
    if (tries >= 5) {
      if (tries === 5) {
        _retryCount[uid] = tries + 1; // marca que o handoff já foi feito (só uma vez)
        try {
          if (global.LF && typeof global.LF.enqueueActivities === 'function') {
            var _q = (global.S && uid === global.S.userId) ? getActivities() : getActivitiesLocalFor(uid);
            global.LF.enqueueActivities(uid, _q);
          }
        } catch(_e0){}
      }
      _retryTimers[uid] = global.setTimeout(function(){
        _retryTimers[uid] = null;
        try {
          if (!_hasPending(uid)) { _retryCount[uid] = 0; return; }
          if (global.SyncManager && typeof global.SyncManager.drain === 'function') {
            try { global.SyncManager.drain(); } catch(_e1){}
          }
          _schedulePendingRetry(uid); // mantém o laço de 60s vivo
        } catch(_e2){}
      }, 60000);
      return;
    }
    _retryCount[uid] = tries + 1;
    _retryTimers[uid] = global.setTimeout(function(){
      _retryTimers[uid] = null;
      try {
        if (!_hasPending(uid)) { _retryCount[uid] = 0; return; }
        if (global.S && uid === global.S.userId) saveActivities(getActivities());
        else lfSaveActivitiesFor(uid, getActivitiesLocalFor(uid));
      } catch(_e){}
    }, 3000 * _retryCount[uid]);
  }
  ...
  function saveActivities(list){
    ...
    _enqueueSave(uid, function(){
      ...
      /* [FIX 20260907] antes de montar o payload, remove tombstones de
         ids que voltaram a existir na lista (auto-limpeza) e coleta os
         tombstones restantes como deletedIds — o Worker (merge por id)
         só propaga exclusões via deletedIds explícito. */
      _syncActTombstones(payloadList);
      var deletedIds = _actTombstones();
      var savePromise = wc
        ? wc.saveAtividadesList(uid, payloadList, undefined, { deletedIds: deletedIds })
      ...
      return savePromise.then(function(){
        ...
        if(!_hasPending(uid))_retryCount[uid]=0; /* LF-FIX-3BUGS-v1-20260819 #2 */
        try {
        ...
        try{_schedulePendingRetry(uid);}catch(_e2){} /* LF-FIX-3BUGS-v1-20260819 #2 */
        global.syncErr(err);
      });
    });
  }
  ...
  function lfSaveActivitiesFor(uid, list){
    ...
    if (wc || hasFirebase) {
      ...
      return _enqueueSave(uid, function(){
        ...
        /* [FIX 20260907] mesmo par (auto-limpeza + deletedIds) do
           caminho saveActivities acima — ver comentário lá. */
        _syncActTombstones(payloadList);
        var deletedIds = _actTombstones();
        var savePromise = wc
          ? wc.saveAtividadesList(uid, payloadList, undefined, { deletedIds: deletedIds })
