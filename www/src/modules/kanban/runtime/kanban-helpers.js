(function(global){
  ...
  /* ===== Registro curto de "excluido recentemente" (2026-08-16) =====
     Protege QUALQUER merge feito via _mergeKeepLocalOnly (clientes/Bingo
     E kanban leads/negocios) contra a corrida descrita acima. TTL de 5
     minutos: cobre folgadamente qualquer fetch em voo (inclusive em
     conexao ruim/mobile) sem precisar de lapide permanente no servidor.
     Guardado em localStorage (nao so memoria) pra sobreviver a um reload
     no meio da janela de corrida. */
  var LF_DELETED_TTL_MS = 5*60*1000;
  var LF_DELETED_KEY = 'lf6_recently_deleted_ids';
  function _lfRecentlyDeletedMap(){
    try{ return JSON.parse(localStorage.getItem(LF_DELETED_KEY))||{}; }catch(_e){ return {}; }
  }
  function _lfPruneRecentlyDeleted(map){
    var now=Date.now(),changed=false;
    Object.keys(map).forEach(function(id){
      if(!map[id]||(now-map[id])>LF_DELETED_TTL_MS){delete map[id];changed=true;}
    });
    return changed;
  }
  function _lfMarkRecentlyDeleted(id){
    if(!id)return;
    try{
      var map=_lfRecentlyDeletedMap();
      _lfPruneRecentlyDeleted(map);
      map[id]=Date.now();
      localStorage.setItem(LF_DELETED_KEY,JSON.stringify(map));
    }catch(_e){}
  }
  function _lfIsRecentlyDeleted(id){
    ...
    try{
      var map=_lfRecentlyDeletedMap();
      if(_lfPruneRecentlyDeleted(map)){
        try{localStorage.setItem(LF_DELETED_KEY,JSON.stringify(map));}catch(_e2){}
      }
      return !!map[id];
    }catch(_e){ return false; }
  }
  global._lfMarkRecentlyDeleted = _lfMarkRecentlyDeleted;
  global._lfIsRecentlyDeleted = _lfIsRecentlyDeleted;
