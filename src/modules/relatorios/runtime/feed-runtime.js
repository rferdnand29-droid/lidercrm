(function(global){
  ...
  // FIX 2026-08-19: ADM agora enxerga o histórico completo de movimentações
  // (antes o feed era truncado em 200 eventos em vários pontos). Mantemos um
  // teto grande no cache local (só p/ evitar estourar sessionStorage do
  // navegador em contas muito antigas), mas 20k já é ordens de grandeza
  // maior que os 200 do limite antigo e cobre "todo o tempo" do CRM.
  var _cfg=(global.LiderCRM&&global.LiderCRM.config)||{};
  var FEED_CACHE_HARD_CAP=Number(_cfg.cache&&_cfg.cache.feedMaxItems)||20000;
  global.FEED_CACHE_HARD_CAP=FEED_CACHE_HARD_CAP;

function getFeed(){return (typeof global.sg==='function')?global.sg(FEED_KEY)||[]:[];}

function saveFeed(list){
  // FIX 2026-08-19: removido o corte de 200; guarda todo o histórico (com um
  // teto de segurança bem alto p/ não estourar sessionStorage). O documento
  // do Firebase (branch legado) também deixa de ser truncado — quem lê
  // esse ramo é apenas fallback de leitura, o gravador real hoje é o Worker
  // (feed-controller.js), onde cada evento já é um doc próprio (sem limite).
  var trimmed=Array.isArray(list)?list.slice(0,FEED_CACHE_HARD_CAP):[];
  if(typeof global.ss==='function')global.ss(FEED_KEY,trimmed);
  if(global.DB_MODE==='firebase'&&global.db){global.db.collection('config').doc('feed').set({list:trimmed,ts:Date.now()}).catch(function(e){console.warn("[feed] saveFeed firebase falhou",e);});}
}
...
function logFeedEvent(type,byId,itemName,detail,board,canal){
  ...
  // FIX 2026-08-19: cache local passa a manter o histórico completo
  // (com teto de segurança FEED_CACHE_HARD_CAP) — o slice(0,200) antigo
  // fazia com que o próprio dispositivo perdesse eventos mais antigos
  // mesmo quando o backend ainda tinha tudo.
  var localFeed=getFeed();localFeed.unshift(entry);ss(FEED_KEY,localFeed.slice(0,FEED_CACHE_HARD_CAP));
  ...
  if(root&&root.config&&root.config.useWorkerApi&&wc&&typeof wc.logFeedEventRemote==='function'){
    wc.logFeedEventRemote(entry).catch(function(e){
      console.warn("[feed] logFeedEvent worker falhou",e);
      // [FIX 20260820] antes, uma falha aqui só ia pro console — o evento
      // ficava SÓ no cache local deste aparelho, sem nenhuma tentativa
      // futura de reenviar. Ver lf-fix-lig-feed-retry-queue-v1-20260820.js.
      if(typeof global._lfEnqueueFeedRetry==='function')global._lfEnqueueFeedRetry(entry);
    });
  }else if(DB_MODE==='firebase'&&db){
    db.collection('config').doc('feed').set({list:firebase.firestore.FieldValue.arrayUnion(entry),ts:Date.now()},{merge:true}).catch(function(e){
      console.warn("[feed] logFeedEvent firebase falhou",e);
      if(typeof global._lfEnqueueFeedRetry==='function')global._lfEnqueueFeedRetry(entry);
    });
  }else if(typeof global._lfEnqueueFeedRetry==='function'){
    // [FIX 20260820] nem Worker nem Firebase disponíveis agora (ex.: boot
    // ainda inicializando) — antes o evento nunca subia pra nuvem nesse
    // caso. Enfileira pra tentar assim que a conexão/sessão estiver pronta.
    global._lfEnqueueFeedRetry(entry);
  }
}

// Helpers puros extraídos de js/relatorios.js na rodada 2026-07-17 (parte 3).
function _kbDeleteReasonLabel(reason){
  var labels={duplicado:'Duplicado',bug_teste:'Bug ou Teste',numero_nao_existe:'Número não existe',numero_errado:'Número errado',lead_duplicado:'Lead duplicado',em_tratativa:'Em tratativa',ja_comprou:'Já comprou',sem_interesse:'Sem interesse',sem_contato:'Sem contato'};
