...
async function _syncFetchWithTimeout(url, init, timeoutMs) {
  ...
  // -----------------------------------------------------------------
  // CORREÇÃO 2026-08-07 (relatado: "mensagem existe no Capacitor mas
  // não no PC — ficou presa só no aparelho"): _chatSyncMsg() (js/chat.js)
  // salva a mensagem localmente na hora (otimista, pra aparecer
  // instantâneo na tela), mas se o PUT pro servidor falhar (rede
  // instável — bem mais comum no celular que no PC), o código só
  // registrava um aviso no console e desistia — a mensagem ficava
  // "presa" só naquele aparelho, invisível em qualquer outro lugar,
  // pra sempre. Mesma classe de bug que este arquivo já resolveu pra
  // atividades/ligações/agenda — só faltava estender pro chat. Mesmo
  // endpoint que wc.putConfig() já usa (PUT /usuarios/config?name=...),
  // só que passando pela fila persistente em vez de uma tentativa
  // única e silenciosa.
  // -----------------------------------------------------------------
  function _workerConfigUrl(name){
    var cfg = (global.LiderCRM && global.LiderCRM.config) || {};
    var base = cfg.workerBaseUrl || '/api';
    var ver  = cfg.workerVersion || 'v1';
    return base + '/' + ver + '/usuarios/config?name=' + encodeURIComponent(name);
  }
  function _enqueueChatMsg(convId, payload){
    if(!convId) return null;
    return RetryQueue.enqueue({
      type: 'chat_msg',
      url: _workerConfigUrl('chat_conv_' + convId),
      method: 'PUT',
      headers: _workerAuthHeaders(),
      body: payload,
      localId: convId,
  ...
  NS_LF.enqueueChatMsg = _enqueueChatMsg;
