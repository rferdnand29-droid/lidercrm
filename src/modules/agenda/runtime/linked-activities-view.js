  ...
  function _linkedActCardHTML(a,done,opts){
    ...
    if(opts&&opts.canEdit&&!done){
      actions='<div class="qa-act-actions" style="display:flex;gap:6px;margin-top:8px">'
        ...
        /* 2026-08-06 (pedido do usuário): faltava um jeito de concluir a
           atividade direto daqui — só dava pra editar. Reaproveita o
           mesmo fluxo de confirmação (toast, não confirm() bloqueante —
           falha em iOS PWA) já usado no painel de Atividades, só que
           passando o ownerUid certo direto (o painel original resolvia
           o dono a partir de uma lista que não existe aqui dentro). */
        +'<button class="bc" type="button" style="padding:6px 10px;font-size:.68rem;min-height:auto;width:auto;background:rgba(27,138,94,.15);border-color:rgba(27,138,94,.35);color:var(--ok)" onclick="confirmLinkedActDone(\''+global._jsSq(a.id)+'\',\''+global._jsSq(opts.ownerUid||global.S.userId)+'\')">✅ Concluir</button>'
        +'</div>';
    }
