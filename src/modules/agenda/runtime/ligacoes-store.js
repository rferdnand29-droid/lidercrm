...
(function(global){
  ...
  /* [FIX 20260820] getLigToday()/ligKey() sempre apontam pro dia de HOJE —
     é assim que a contagem em tempo real (clique a clique) funciona, e não
     dá pra mudar isso sem arriscar a gravação do dia atual. Pra ver um dia
     ANTERIOR sem tocar em como os dados são gravados, esta função lê a
     MESMA chave por data (lf13_lig_<uid>_<data>) — só parametrizada, em vez
     de sempre hoje. Nenhuma escrita nova, nenhuma duplicação: só uma leitura
     a mais do que já existe em localStorage/servidor. */
  function ligKeyForDate(uid,dateStr){ return 'lf13_lig_' + (uid || global.S.userId) + '_' + dateStr; }
  function getLigForDate(uid,dateStr){
    if(!dateStr || dateStr === global.today()) return getLigToday(uid);
    return global.sg(ligKeyForDate(uid,dateStr)) || [];
  }
  ...
  function saveLigToday(list, uid){
    ...
    if (useWorker) {
      ...
      return _enqueueSave(qkey, function(){
        ...
        return savePromise.then(global.syncOk).catch(function(e){
          global.syncErr(e);
          // [FIX 20260820] antes, uma falha aqui só passava pro
          // indicador de sync com erro — o total/rounds ficavam só no
          // aparelho, sem nenhuma tentativa futura de reenviar. Ver
          // lf-fix-lig-feed-retry-queue-v1-20260820.js.
          if (typeof global._lfEnqueueLigRetry === 'function') {
            global._lfEnqueueLigRetry(finalUid, today, freshList, total, rounds);
          }
        });
      });
      ...
      return global.db.collection('ligacoes').doc(finalUid + '_' + today)
        ...
        .then(global.syncOk).catch(function(e){
          global.syncErr(e);
          if (typeof global._lfEnqueueLigRetry === 'function') {
            global._lfEnqueueLigRetry(finalUid, today, list, total2, rounds2);
          }
        });
    } else if (typeof global._lfEnqueueLigRetry === 'function') {
      // [FIX 20260820] nem Worker nem Firebase disponíveis agora (ex.:
      // boot ainda inicializando) — antes o total/rounds nunca subiam
      // pra nuvem nesse caso. Enfileira pra tentar assim que a
      // conexão/sessão estiver pronta.
      var totalF = _accumulatedTotal(finalUid, today, (list || []).length);
      var roundsF = _accumulatedRounds(finalUid, today);
      global._lfEnqueueLigRetry(finalUid, today, list, totalF, roundsF);
    }
  }
  ...
  agenda.runtime.ligKeyForDate = ligKeyForDate;
  agenda.runtime.getLigForDate = getLigForDate;
  /* R14-10b: expor funções ao escopo global */
  global.ligKey = ligKey;
  global.getLigToday = getLigToday;
  global.saveLigToday = saveLigToday;
  global.ligKeyForDate = ligKeyForDate;
  global.getLigForDate = getLigForDate;
