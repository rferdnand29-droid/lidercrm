...
(function(global){
  ...
  function set(token, expiresInSec, user){
    ...
    persist();
    // Op-4 (2026-07-23): sempre que set() rodar (login inicial OU
    // silent refresh), sincroniza os campos autoritativos do JWT
    // (cargoCodigo, admExtra, userUuid, cargo) para dentro de S.
    // A funcao é aditiva: se lfSyncSessionFromAuthUser ainda não
    // foi carregada (ordem de <script>), noop.
    try{
      if(user && typeof global.lfSyncSessionFromAuthUser==='function'){
        global.lfSyncSessionFromAuthUser(user);
      }
    }catch(_e){}
  }
  function isValid(){
    ...
    if(body != null && typeof body !== 'string' && !(body instanceof FormData) && !(body instanceof Blob) && !(body instanceof ArrayBuffer)) body = JSON.stringify(body);
      ...
      return fetch(path, {
        ...
        return response.text().then(function(text){
          ...
          /* [FIX 20260903] Antes, QUALQUER 401 apagava a sessão gravada.
             Como o token expirado é justamente o insumo do refresh com
             janela de graça, apagá-lo tornava a recuperação impossível e
             obrigava login manual. Agora só limpamos quando não há token
             renovável; caso contrário, mantemos e acionamos a renovação. */
          if(response.status === 401){
            var _renewable = false;
            try {
              _renewable = !!(session.token && global.LF_AUTH_GRACE
                && global.LF_AUTH_GRACE.status
                && global.LF_AUTH_GRACE.status().renovavelPorGraca);
            } catch(_e){}
            if (_renewable) { try { global.LF_AUTH_GRACE.kick(); } catch(_e2){} }
            else { clear(); }
          }
          return {
            ok: response.ok,
